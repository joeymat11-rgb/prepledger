...
| v7.55.9 | **the SCALE round** (SCHEMA_V 59, ten legs, Sol closure passes 1–8) — began as one live question ("I only logged in the morning, but it's saying night?"): a typed intake number was closing the morning read window, so three attested morning weigh-ins filed as evening reads and never reached the trend (patchV59 re-classes them on the owner's word and replays the chain). The sync law then extended to everything that question touched: one op-keyed receipt per read, the analyst DECISIONS and their EFFECTS (targets, adjustments, receipts) as derivations of the decision log, the trend/pt/weekly chain as the replay of `reads[]`, the attestation as a monotone store plus op-keyed FACT lines no old client can unsay, the orphan undo re-materialized from its own evidence, and the undo door claiming only the order it can prove. The durability guard learned to count records, not receipts — it had been refusing the app's own migration. Gate: suite 2,951 · 17 laws · 56 seeds · 95 declared mutations · explore ×4. | 1177055 |

**SHIPPED FROM THE SCALE ROUND, and what it parked.** Two limitations are named and deferred to STAMP rather than papered over: cross-device same-day order carries no causal provenance (every writer stamps its own device's wall clock, so `d` and `at` come from the same clock and neither witnesses order — the undo door now says "Latest dated move on file" wherever more than one move is on file, instead of claiming "Last move applied"), and the `calibratedAt` / `s.sync` residues. A finite skew window was tried and retired: it moves the false-certainty boundary, it cannot close it.
...
**Sol's blind leg is in-repo: [MINIPASS-SOL-AUDIT.md](MINIPASS-SOL-AUDIT.md)** (345 lines,
verbatim; it arrived mid-build after a first paste without it — filed the moment it
landed, and the consolidated M1–M7 directive below is filed verbatim beside it).
