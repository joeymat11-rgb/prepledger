...
2. **Curate `ledger/suggestions.json`** — start from the engine's draft, keep the few genuinely high-leverage, well-supported suggestions, sharpen each into your voice, and make sure every one has all three rationale parts (science / your-data / relationship) plus a `predict` and a `confidence`. Drop weak ones. **Never keep or emit a calorie-band change (`kind: "cal"`) — the engine owns the band (constitution, Article VIII); drop any the engine drafts, and put a band-adjacent point in the brief as an observation instead.** Keep the JSON shape exactly as the engine wrote it.
