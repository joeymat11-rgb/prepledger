...
**PENDING acceptance-contract amendment:** the owner authorized preparation of proportionate review, validated receipt reuse and two conditional first-owner deferrals. `rebuild/process/BRIEF-REVIEW-EFFICIENCY-PILOT.md` §§1–5 defines the concrete changes; independent technical acceptance and an integrated receipt are still required. Until then the existing review, execution and pre-port rules remain effective. This proposal changes no model, spending authority, private-import permission or current verdict.
