/* rig185 — ENGINE-TRACK witnesses executed on the OLD engine (a0009c3, pre-PROGRESSION-1): Sol's PACK 3 counterexamples.
...
const ENGINE = process.env.ENGINE_OLD || require("node:path").join(__dirname, "engines", "engine-old.cjs"); const out = [];
const log = (s) => { out.push(s); console.log(s); };
if (!fs.existsSync(ENGINE)) { log("rig185: ENGINE_OLD absent — SKIPPED (engine-track only)"); process.exit(0); }
...
log(`W1 same-day set-count maximum on the old engine: +1 current 4 → ${plus} (required 4) · −1 current 3 → ${minus} (required 4) · balanced +1/−1 current 4 → ${balanced} (required 5) ⇒ ${plus === 4 && minus === 4 ? "PASS" : "FAIL — end-of-day + same-day INCREASES double-counts the push; the maximum is end-of-day + same-day DECREASES"}`);
