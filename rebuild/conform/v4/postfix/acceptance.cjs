...
function verifyReceipts(root,a,e,bytes){
  ...
  if(!match||match[2]!==e.acceptanceFile||match[3]!==e.acceptanceSha256)fail('REVIEW-EXACT-VERDICT');const reviewed=a.packageId==='M2-SET-ONE-ERA'?require('./helpers/set-one-era-git-bytes.cjs').object(root,match[1],match[2]):L.object(root,match[1],match[2]);if(!reviewed.equals(bytes))fail('REVIEW-ARTIFACT-GIT-PIN');return true;
}
