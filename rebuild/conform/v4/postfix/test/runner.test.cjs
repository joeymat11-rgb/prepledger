...
test('preserved version1 cannot claim a repair because successor cases now exist',()=>{const mod=require('../laws/import-guards.cjs');assert.equal(mod.laws.length,3);assert.deepEqual(manifest.selectedApprovedFixIds,[]);const proposed=structuredClone(manifest);proposed.phase='PACKAGE';assert.ok(R.packagePending(proposed,{}).some(reason=>reason.includes('not implemented')));for(const law of mod.laws){assert.ok(law.requiredCases.length);assert.ok(law.requiredMutants.length);assert.equal(Object.hasOwn(law,'control'),false);}});
