...
function currentRate(s) {
  const rate = require('./scale.cjs').scaleRate(s, { weeksBetween, timeOf: r => mk(r.d).getTime(), DAY });
  const { scale, ...rest } = rate;
  return { scale, fat: rate.measured ? +(scale + s.model.drip).toFixed(2) : 1.25, ...rest };
}
