...
const { createEngine } = require("./b1b2-public-engine.cjs");
...
cell("B1-D24-empty-dailyLogs-does-not-invent-an-owed-yesterday", ["D24-2 empty-dailyLogs-is-owed"], () => {
  const T = engine("2026-09-03"), s = state();
  assert.deepEqual(s.dailyLogs, {});
  assert.equal(T.nowFocus(s, 12).owed.some((x) => x.k === "yesterday"), false);
