...
function volumePush(s) {
  ...
  if (rec.band === "WATCH" || rec.band === "LOW") return { mode: "WITHHELD", veto: "recovery", band: rec.band,
    why: (rec.sleepEvidence ? "Current sleep evidence is incomplete; observed flags establish this restriction. " : "") + "recovery is " + rec.band + " (" + rec.flags.map((f) => f.k).join(", ") + ") — an added set spends recovery, and the instrument says there is nothing spare. This is the ceiling working, not the lever failing" };
...
function runAdaptive(state, todayISO, raOpts) {
     ...
     would teach him to read neither. A decline buys the WEEK for the whole lever. */
  {
    ...
    const recoveryEvidence9 = recoveryIndex(s);
  ...
  if (rec.band !== "LOW") {
    s.proposals.filter((p) => p.rid && p.rid.indexOf("recovery_") === 0 && !p.resolved).forEach((p) => {
      p.resolved = true; p.stoodDown = true;
      s.feed.unshift({ d: todayISO, t: "RECOVERY CARD STOOD DOWN", how: rec.sleepEvidence ? "Current sleep evidence is incomplete; the observed flags do not support a LOW recovery card. Stored observations are unchanged." : `the signals that raised it have cleared — ${rec.flags.length} of ${rec.watched} still up, which is below the line that holds structural changes` });
    });
  }
  if (rec.band === "LOW") {
    ...
    const why = [
      ...
      rec.sleepEvidence ? "Current sleep evidence is incomplete; these independently observed flags still establish LOW recovery." : "",
