...
const root=path.resolve(__dirname,'../../../..'),C='c4716edad91453e74ba17f70ab076781ff5367de',M='100820aa47a4f8729642033499eaec0f0ee282e1';
assert.match(root.replaceAll('\\','/'),/\/work\/pm-caretaker\/review-b1b2-r2$/);assert.equal(process.version,'v22.23.2');
...
assert.equal(sha(source),'de8d1074a0614c006ce0913b793b021540e4563f5396b146c4e4163887a447fe');assert.equal(source.split(boundary).length,2);
const manifest=JSON.parse(fs.readFileSync(path.join(root,'.tmp/er-b1b2-r2/tooling-manifest.json')));for(const row of manifest.files)assert.equal(sha(fs.readFileSync(path.join(root,row.file))),row.sha256,row.file);
...
fs.writeFileSync(path.join(root,'.tmp/er-b1b2-r2/results/authority-evidence.json'),JSON.stringify(evidence,null,2)+'\n');console.log(JSON.stringify(evidence));
