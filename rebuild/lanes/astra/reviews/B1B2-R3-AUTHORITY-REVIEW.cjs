...
const root=path.resolve(__dirname,'../../../..'),C='8e65805481091daacda3266ad9113b49553cf3f4',M='100820aa47a4f8729642033499eaec0f0ee282e1';
assert.match(root.replaceAll('\\','/'),/\/work\/pm-caretaker\/review-b1b2-r3$/);assert.equal(process.version,'v22.23.2');
...
const manifest=JSON.parse(fs.readFileSync(path.join(root,'.tmp/er-b1b2-r3/tooling-manifest.json')));for(const row of manifest.files)assert.equal(sha(fs.readFileSync(path.join(root,row.file))),row.sha256,row.file);
...
fs.writeFileSync(path.join(root,'.tmp/er-b1b2-r3/results/authority-evidence.json'),JSON.stringify(evidence,null,2)+'\n');console.log(JSON.stringify(evidence));
