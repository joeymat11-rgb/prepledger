...
assert.match(root.replaceAll('\\','/'),/\/work\/pm-caretaker\/review-b1b2-r3$/);
assert.equal(process.version,'v22.23.2');
const C='8e65805481091daacda3266ad9113b49553cf3f4',M='100820aa47a4f8729642033499eaec0f0ee282e1',R='797e4cf39fac39148ccae784116999669b82caf3';
const base=path.join(root,'.tmp/er-b1b2-r3'),pub=path.join(base,'public'),results=path.join(base,'results');
...
if(action==='stage-tooling'){
 ...
 if(action==='recovery-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R3-RECOVERY-REVIEW-ANNEX.cjs')];
 if(action==='identity-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R3-IDENTITY-REVIEW-RUN.cjs')];
 if(action==='today-clearance')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R3-TODAY-CLEARANCE-REVIEW.cjs')];
 if(action==='clock-boundary')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R3-CLOCK-BOUNDARY-REVIEW.cjs')];
 let cwd=pub;
