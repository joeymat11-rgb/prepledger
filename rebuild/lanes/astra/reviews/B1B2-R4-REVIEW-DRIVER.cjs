...
assert.match(root.replaceAll('\\','/'),/\/work\/pm-caretaker\/review-b1b2-r4$/);
assert.equal(process.version,'v22.23.2');
const C='f2dea2ec8426d12e95483e180821839fbf1165f7',M='100820aa47a4f8729642033499eaec0f0ee282e1',R='cbcebb3d26bf8892adbe84fbbe0a7c5e85856013';
const base=path.join(root,'.tmp/er-b1b2-r4'),pub=path.join(base,'public'),results=path.join(base,'results');
...
if(action==='stage-tooling'){
 ...
 if(action==='recovery-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R4-RECOVERY-REVIEW-ANNEX.cjs')];
 if(action==='identity-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R4-IDENTITY-REVIEW-RUN.cjs')];
 if(action==='today-clearance')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R4-TODAY-CLEARANCE-REVIEW.cjs')];
 if(action==='clock-boundary')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-R4-CLOCK-BOUNDARY-REVIEW.cjs')];
 let cwd=pub;
