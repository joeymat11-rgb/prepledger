...
const root=path.resolve(__dirname,'../../../..'),base=path.join(root,'.tmp/er-b1b2-r4'),C='f2dea2ec8426d12e95483e180821839fbf1165f7',old='8e65805481091daacda3266ad9113b49553cf3f4';
...
const M=a.sourceBase,R=a.runtimeSource,S=a.successor.runtimeSource,T=a.repair.runtimeSource,U=a.r9.runtimeSource;
assert.equal(S,'48a3063a23528ed240eb2356226d237d2793a9da');assert.equal(T,'797e4cf39fac39148ccae784116999669b82caf3');
const {r9,...historical}=a;const repair=a.repair;assert.deepEqual(historical,previous,'every original map field and R/S record unchanged');
assert.equal(a.runtime.flatMap(r=>r.hunks).length,65);assert.equal(a.successor.runtime.flatMap(r=>r.hunks).length,10);assert.equal(repair.runtime.flatMap(r=>r.hunks).length,3);assert.equal(r9.runtime.flatMap(r=>r.hunks).length,1);assert.equal(U,'cbcebb3d26bf8892adbe84fbbe0a7c5e85856013');
...
const original=reconstruct(M,R,a.runtime),successor=reconstruct(R,S,a.successor.runtime),repaired=reconstruct(S,T,repair.runtime),current=reconstruct(T,U,r9.runtime),h3rows=[];
...
for(const [from,to,table]of [[R,S,a.successor],[S,T,repair],[T,U,r9]]){
...
assert.equal(git('rev-parse',C+':rebuild/engine').toString(),git('rev-parse',U+':rebuild/engine').toString());
const changed=git('diff','--name-only',old,C).toString().trim().split('\n');assert.equal(changed.length,17);
const engineInventory=git('ls-tree','-r',U,'rebuild/engine').toString().trim().split('\n').map(line=>{const [metadata,file]=line.split('\t');const oldBlob=git('rev-parse',T+':'+file).toString().trim(),blob=metadata.split(' ')[2];return{file,blob,oldBlob,changed:blob!==oldBlob,custody:'Git object metadata'};});
assert.deepEqual(engineInventory.filter(x=>x.changed).map(x=>x.file).sort(),[...r9.runtime,...r9.engineEvidence].map(x=>x.file).sort());
...
const profileNotes={mechanicalCitations:[profile.authorizations.owner.ledgerLine,profile.authorizations.contract.ledgerLine,profile.authorizations.theme.ledgerLine,profile.brief.acceptedLedgerLine.ledgerLine,232],amended246_248_R7_282_288MechanicalBinding:false,constructionCommission:'292 explicitly permits current review; final binding/composition remains held'};
const result={candidate:C,source:U,changed,original,successor,repaired,current,h3,entireHistoricalMapEqual:true,engineInventory,engineEvidence:r9.engineEvidence,pins,registration:{dIds:profile.dIds,parent:profile.parent,coverage:profile.coverage,children:profile.children,runnerSHA256:sha(runner),workflowSHA256:sha(workflow),todayFiles},profileNotes,nativeComparisons:[a.nativeFieldDeltas,a.successor.nativeComparison,repair.nativeComparison,r9.nativeComparison],builderReportRead:false};
fs.writeFileSync(path.join(base,'results/source-independent.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({paths:changed.length,originalHunks:65,successorHunks:10,repairHunks:3,r9Hunks:1,engineEvidence:4,h3Hunks:4,pins:pins.length,children:26,original9_3_7CoverageUnchanged:true,profileNotes}));
