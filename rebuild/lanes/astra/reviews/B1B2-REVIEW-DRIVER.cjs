...
assert.match(root.replaceAll('\\','/'),/\/work\/pm-caretaker\/review-b1b2-complete$/);
assert.equal(process.version,'v22.23.2');
const C='cc6a1315003adc7f3f96d635980cfdf32b5a7a74',M='100820aa47a4f8729642033499eaec0f0ee282e1',R='6c9248e695a4478abdbaae0f9f48395ac56000fa';
const base=path.join(root,'.tmp/er-b1b2'),pub=path.join(base,'public'),results=path.join(base,'results');
...
if(action==='stage-tooling'){
 ...
 if(action==='recovery-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-RECOVERY-REVIEW-ANNEX.cjs')];
 if(action==='identity-annex')argv=['--test','--test-reporter=tap',path.join(__dirname,'B1B2-IDENTITY-REVIEW-RUN.cjs')];
 let cwd=pub;
 if(action==='tooling-cohort'){
  const p=JSON.parse(fs.readFileSync(path.join(root,'rebuild/lanes/b/tooling/packages/B1-B2.json')));argv=p.children.find(c=>c.name==='tooling-cohort').argv;assert.equal(argv.length,11);assert.equal(argv[0],'--test');assert.equal(argv[1],'--test-reporter=tap');cwd=root;
 ...
 for(const name of Object.keys(env))if(/^(NODE_OPTIONS|NODE_PATH|NODE_V8_COVERAGE|V8_COVERAGE|NODE_TEST_CONTEXT|NODE_TEST_WORKER_ID)$/i.test(name))delete env[name];
