...
## September 11 proposed UNKNOWN amendment — DECISIONS:110 (candidate, not accepted)

FILE ASTRA-B1-UNKNOWN-RECOVERY.md authorizes this bounded successor from fe03ed0d5c4fa347807d8df7a611f408a4e10397, previously scoped-reviewed by 6529af024a5e61425f34773dda036e529438c56f. This section adds the PM's recorded UNKNOWN interpretation and narrowly authorized D8 freshness correction. Every prior B1 obligation, mutant, source-policy requirement, downstream fidelity budget and normal engine qualification requirement remains. The new delta is not covered by the historical four-module/hunk inventory below: this successor additionally owns direct consumers in volume.cjs and writers.cjs. It requires independent review and normal cumulative-parent qualification; this text does not confer acceptance or source-policy authority.

**Evidence contract.** recoveryIndex retains existing named flags, factors, lever and thresholds. A finite actual sleep observation on the prior calendar date or current calendar date satisfies existing D21 two-anchor presence semantics; a future-only row does not. With neither, score is null and sleepEvidence is {state:'UNKNOWN', expectedDate:<yesterday>, lastDate:<latest nonfuture date or null>}. When existing flags do not reach WATCH, band is UNKNOWN. Independently observed flags sufficient for existing WATCH/LOW keep that severity and existing restrictions, with score still null and incomplete sleep evidence explicit. Missing evidence never creates GREEN/100, an observation, a cost or a new restriction. Known-current outputs retain their old shape, with no sleepEvidence addition. No records are rewritten.

**Direct semantic mappings and changed output surfaces.** sleep.cjs recoveryIndex and its internal presence helper implement the representation. liftCall diagnoses recovery fatigue only for WATCH/LOW; missing evidence leaves an otherwise earned stall review/target standing. Its affected review/reset and post-refeed wording no longer calls absence GREEN or slept; historical night/bedtime receipts identify the actual recorded date instead of claiming last night. policy.cjs phaseProposal counts WATCH/LOW as the recovery cluster signal, and labels incomplete sleep on an otherwise supported break proposal. volume.cjs volumeConversion does not subtract for UNKNOWN, still subtracts for actual pain, repeated falling performance or WATCH/LOW, and identifies incomplete evidence in its receipt. writers.cjs volumePush preserves observed WATCH/LOW vetoes and qualifies partial evidence; runAdaptive's earned-volume, LOW note and stood-down recovery-card text distinguish missing sleep from proven GREEN/clean recovery. Decisions remain proposals and existing records/history remain. These are changed behavioral and textual outputs requiring downstream/full fidelity accounting, not a byte-identical presentation delta.

**Consumer inventory and screens handoff.** Actual engine band consumers are sleep.liftCall, policy.phaseProposal, volume.volumeConversion, writers.volumePush, writers.runAdaptive and energy.stepPush. The proposal sweep is named runAdaptive in this source (not evaluateProposals). stepPush already checks LOW explicitly, so energy.cjs remains unchanged; WATCH and UNKNOWN alone do not acquire a new step veto. No numeric recovery-score comparison or recovery cache was found in these engine consumers. recoveryIndex remains exported through the engine factory/browser build. The bounded m3/m4 source search finds context-compatibility.cjs and the existing native source-delta export binding, not an implemented recovery presentation join. Frozen src/app.jsx exposes the legacy bar at 17503 and a measured tag at 21798; it remains untouched. Future screens must show UNKNOWN as “no reading” in the approved secondary style, never zero/100 or a green status; omit the numeric bar when score is null. For partial WATCH/LOW, show the actual adverse flag receipts and “Current sleep: no reading”; do not label the whole recovery assessment complete/measured. Preserve individual actual observations and no automatic target reduction from absence. Screens/source-policy ownership must qualify that join separately.

**Separate D8 freshness omission.** Before this amendment, sleepMean3At accepted an old consecutive three-night short run as current debt even though cleanAtDate already refused a non-last-night restriction. Add only the same last-night calendar guard to sleepMean3At; do not change its existing mean threshold or erase history. This is a pre-existing omission discovered during consumer audit, not caused by UNKNOWN. Actual stale-three-short-night volumePush changes from a sleep veto to its earned offer; stepPush was already clear under cleanAtDate and stays clear. Current consecutive short nights still restrict both. Historical short sleep still exempts its own training session from a stall comparison.

**Focused authored evidence.** New committed engine/test/b1-unknown-recovery.test.cjs exercises 17 tests: missing/stale/future-only evidence; exact known adequate/short model values; partial WATCH/LOW; actual lift/phase/conversion/volume/step/adaptive consumers; historic preservation; current-vs-stale three-night debt; four old non-GREEN mappings plus removal of the new freshness guard as real in-memory behavioral counterexamples. Existing b1-delta-cells.cjs still reports 25/25. An ignored synthetic probe loaded the four original factories directly from exact fe03ed0: 42 complete known-current outputs across seven model/consumer reads matched the candidate, and original missing/stale GREEN100 was reproduced. No full campaign, law, witness, receipt, golden or source-policy artifact was changed/run for this amendment. The normal engine-tier public/private, affected-platform and second-gate work remains after the proper cumulative BNTC parent.
...
## 0.0 v1.2 AMENDMENT — what the independent review r1 changed in this contract

The review's verdict was **ACCEPT WITH CHANGES**, with C1, C2 and C7 blocking. C1 and C2 are contract changes and live here. C3, C4 and C5 are evidence corrections and live here. C6 is settled here. C7 (`rebuild/m4/spec`) is **not** authored by lane B — it stays a PM/tooling item, unchanged. C8 (`@noble/*`) is documented, never fixed here.

### A1. D21's hunk changes — the two-anchor `sleepInfo` (review C1, BLOCKING)

v1.1 §2 D21 published a one-line hunk. **That hunk is withdrawn.** Its consequence was found by the builder against its own interest (BUILD-REPORT §4.1) and confirmed with stronger evidence by the reviewer: with `cleanAtDate`'s D8 guard demanding `last.d === plusDays(iso, −1)` and `sleepInfo` asking at `tomorrow`, the question becomes "is there a night bed-dated **today**?" — and the product's own dating rule (frozen `src/app.jsx:12334`: *"A night belongs to the evening it began … never the one you haven't slept yet"*; `booksToday` files today's expected row at **yesterday**) means such a row is absent **by definition**. `recoveryIndex`'s named `sleep` flag therefore went silent in every state the engine can normally produce, taking a 2 h night from WATCH/70 to GREEN/100, against the owner's own D8 words at `DECISIONS.md:60` — *"…and no sleep restriction is applied today; **every other recovery check still applies**"*.

**The rejected remedy.** BUILD-REPORT §4.1's proposal (`clean: cleanAtDate(s, plusDays(<newest night date>, 1))`) is **NOT** adopted. With `iso = newest.d + 1`, `plusDays(iso, −1) === newest.d` identically, so the D8 guard can never trip: it switches D8 off at this call site and reinstates the carry-forward the owner's ruling removes (*"(not carry-forward of the last logged night)"*). The reviewer built it as `rv1/rem1` and measured a 3-day-old 2 h night reading `clean=false` — the opposite D8 violation.

**The adopted hunk** (the reviewer's executed `rem2`; `rebuild/engine/sleep.cjs`, `sleepInfo`, frozen counterpart `fe516c1:14453-14458`). D8's own hunk in `cleanAtDate` is **unchanged** — its `iso − 1` guard is right for the eleven "day in question" callers (`energy.cjs:1219`, `progression.cjs:91,515,704`, `sleep.cjs:54,258,261`, `writers.cjs:539,935,940`) and matches D8's own ratified repair control in `laws-clock-and-as-of.cjs`.

```js
// before (v1.1's published D21 hunk, as shipped at ffa4243 / db9fe59)
...
// after (v1.2 — ask the question at BOTH anchors)
function sleepInfo(s) {
  const n = s.sleep.nights;
...
`cleanAtDate(s, today9)` asks D8's question at the anchor D8 was written for (the current night is the one bed-dated yesterday). `cleanAtDate(s, tomorrow)` keeps D21's repair, because `nightsBefore`'s strict `n.d < iso` filter means only `iso = tomorrow` can see a night dated today — the fall-back row D21 exists to include. Moving `sleepInfo` to `today9` **alone** fails: D21's repair evaporates. Narrowing D8's guard also fails: it breaks eleven correct callers.

**Measured consequences, on the fixed branch, all three variants side by side (frozen bundle `180de4e2…`; `TZ=America/New_York`, `MEASURED_TEST_NOW=2026-09-03`):**

| `sleepInfo(s).clean` | base `acd3b67` | shipped one-anchor `db9fe59` | **v1.2 two-anchor** |
|---|---|---|---|
| newest night bed-dated **yesterday**, `h = 2` | `false` | **`true`** ← the regression | **`false`** |
| a night bed-dated **today**, `h = 2` (D21's case) | `false` | `false` | `false` |
| newest night **3 days old**, `h = 2` (D8 clause 1) | `false` | `true` | `true` |
| D21 witness state (`today=2026-11-01`, night `2026-11-01 h=1`) | `true` | `false` | `false` |
| empty history | `true` | `true` | `true` |

**D22's row (the acceptance bar that failed on the shipped candidate):**

| | base `acd3b67` | shipped `db9fe59` | **v1.2** |
|---|---|---|---|
| `D22 E-D22-recovery-reader-preserves-indexed-sleep-facts…` | RED/RED/**mutant-DETECTED** · framesParity **true** | RED/RED/**AUDIT-FAIL** · framesParity **false** | RED/RED/**mutant-DETECTED** · framesParity **true** |
| 45-law rows that move vs base | — | **11** | **10** |

Full 45-law totals on the fixed candidate: `TOTAL 45 laws · 45 RED-frozen · 29 RED-candidate · 89 GREEN repair controls · 87/104 mutant executions DETECTED · 0 HARNESS_ERROR · AUDIT RED-FIRST FAIL`. Row-by-row against base: **10 moved, 35 byte-identical**; the moved ids are exactly `D8, D10, D16, D17, D19, D21, D23, D24, D25, D27`. `AUDIT RED-FIRST FAIL` is the pristine terminal too, so it is not a B1 regression.

**Trace-safety note, measured rather than argued.** The second `cleanAtDate` adds **no** traced frame: D21's frame list is `[sleepInfo] → [sleepInfo>cleanAtDate]` on the **shipped** candidate *and* on v1.2 — identical. The only frames-parity movement anywhere in the 45 is D22's, and it moves back to `true`.

**Carrier impact: none.** The 18 successor substitutions in `legacy-b1-carriers.cjs` are **unchanged**. Re-measured on a non-throwing assert observer over all three witness programs against base `acd3b67`: **85 assertions compared, 18 flips, at exactly the same 18 lines** as v1.1 §3's table — `defect-witnesses.cjs:61,71,72`; `defect-witnesses-2.cjs:93,94,103,127,128,159`; `defect-witnesses-3.cjs:48,49,50,58,59,72,100,101,102`. Both predicted non-flips still hold (`defect-witnesses-2.cjs:95`, `defect-witnesses-3.cjs:41`), and the two pre-existing D12 failures at `:51`/`:53` still fail on both engines. Carrier re-run: **6/6 PASS**, 3/8/9 edits per file, witness files byte-identical to their pins.

### A2. The D8×D21 cross-case, with a purpose-written check (review C2, BLOCKING)

The review's central finding is that **no law and no witness encoded the cross-case**, which is exactly why the defect passed all ten B1 laws and surfaced only through a *neighbouring* law's trace comparison. v1.2 closes that with a named, committed cell.

**Cell name:** `B1-D8xD21-a-short-last-night-still-restricts-recovery`
**Home:** `rebuild/engine/test/b1-delta-cells.cjs` (new file; no frozen law, golden or witness byte touched)
**The assertion, in one line:**

```js
assert.equal(T.sleepInfo(withNights([...recent, { d: "2026-09-02", h: 2 }])).clean, false);   // today = 2026-09-03, h < DEBT_LAST_H
```

with four controls in the same cell so it cannot pass for the wrong reason: `recoveryIndex(...)` must read `band === "WATCH"` with a `sleep reset…` factor (the flag must reach the product, per D8's "every other recovery check still applies"); a **3-day-old** 2 h night must read `clean === true` and empty history `clean === true` (D8 clause 1, no carry-forward); a **same-date** night must read `clean === false` at both `2026-09-03` and the fall-back `2026-11-01` (D21's repair survives).

**Executed proof that the cell bites** (each hunk reverted one at a time, cell file re-run):

| tree | cells that hold | first failure |
|---|---|---|
| v1.2 repaired candidate | **3/3** | — (exit 0) |
| base `acd3b67` | 0/3 | cell 1 — `T.plusDays is not a function` (the primitive does not exist pre-B1) |
| revert only the **second anchor** (= the shipped `db9fe59` form) | 2/3 | **cell 3** |
| revert **D8's guard** | 2/3 | **cell 3** |

The same revert is also caught as a fourteenth single-hunk bite (§A5) and as a thirty-third source mutant, `D21-4 drop-the-today-anchor`, which the shipped candidate could not have carried because the shipped candidate *was* it.

### A3. The two millisecond→calendar cells B1 had no detector for (review C5)

The reviewer's own bite proved that `REVERT D16 dueISO calendar` and `REVERT D24 yISO calendar` move **no v4 law and no witness carrier** — those two hunks rested entirely on un-committed scratch cells. v1.2 commits them, in the same new file, on dates where a 24-hour step and a calendar step **disagree**:

| cell name | fixture | asserted |
|---|---|---|
| `B1-D16-due-date-is-seven-calendar-days-not-168-hours` | forecast `2026-11-01` (US fall-back week), one read, `GRADE_LAG = 7` | `plusDays("2026-11-01",7) === "2026-11-08"`; read `2026-11-07` → `{graded:0,hit:null}` (the 6th calendar day is **UNGRADED**); `2026-11-08` (due) → `{graded:1,hit:true}`; `2026-11-09` (the one grace date) → `{graded:1,hit:true}`; `2026-11-10` → `{graded:0,hit:null}` |
| `B1-D24-yesterday-is-the-previous-calendar-date-not-24-hours-ago` | `today = 2026-03-09` (the day after spring-forward), logs `{2026-03-07: steps-only, 2026-03-08: cal}` | `plusDays(isoOf(todayStart()),−1) === "2026-03-08"`; `nowFocus(s,12).owed` = `["night"]` — **not** `["night","yesterday"]`; plus a control that strips the 03-08 calories and re-owes yesterday |
...
**Consequence for the package artifact (C-r2-2(a), the PM's).** `rebuild/engine/test/b1-delta-cells.cjs` is not a convenience: remove it and B1's committed evidence falls from 32/33 to 13/33. It must be listed as a **required** artifact of B1's closed profile beside the carrier. **Re-measured on the 23-cell file by review r3 §4 and independently on the 25-cell file by the r4 pass (BUILD-REPORT §10.4), both with the cell file removed from the detector set: 12 of 33 CAUGHT · 21 NOT CAUGHT** (10 by carrier, 2 by law, one of those two only via D22's frames parity). The `13/33` above was the 14-cell measurement and is superseded by that pair; the consequence is unchanged and stronger — the cell file carries twenty-one of the thirty-three kills single-handedly. See §A9.

### A6. Residual risks carried from the review, unclosed (the PM owns each)

1. **The D8×D21 interaction is a class, not an instance.** Any B-package that changes a shared helper's contract while another hunk preserves a caller's idiom can reproduce it. A2's cell closes this instance; the §A5 bar is what closes the class.
2. **No gate can say PASS for B1 yet.** The parent profile refuses correctly (`Unchanged parent pin: rebuild/engine/dates.cjs`, re-measured on the fixed branch), B1's own closed profile does not exist (C7, PM/tooling), `native-carriers-package --ci` is unrunnable from a clean `npm ci`, and `load-write-package --ci` / `conform/run.cjs` / `second-gate --candidate` all fail **pre-existingly and identically on base**. Acceptance rests on the v4 runner, the carriers, the delta cells and the PM's own `--full` on the PC.
3. **`--full` and the private census are unexecuted by builder, reviewer and fixer, by design** (`DECISIONS:92`, `:93` C4). D16 is B1's only LIVE-TRIGGERED defect and its census change is anticipated; a census change on any of the other nine is a RED stop.
4. **`@noble/*` is undeclared** (C8) — see §A7.
...
6. **Parent-chain order still unsettled** (`PLAN…v1.md:146` says B2→B1 while `:113` and `DECISIONS:94` say "B2 ∥ B1"). Every pre-image sha in this brief assumes `acceptance-native-carriers.json 295762f0…` as B1's immutable parent. If the PM rules B2 first, B1 rebases and every pin is re-taken.
7. **`2026-08-12` is a derivation, not a constant** — express D27-3's fixture as `START + 63 days` (§A4 item 4).
8. **"No sleep restriction" still reads GREEN/100, not UNKNOWN.** The owner's D8 word was *UNKNOWN*. In all four variants (base, shipped, rem1, v1.2) a missing or stale last night makes `recoveryIndex` read **GREEN / 100 with zero flags** — a positive claim of perfect recovery on a day the engine knows nothing. The flag model has no third state (`sleep.cjs:220-236`). v1.2 does **not** close this; it is beyond B1's hunks and needs a PM ruling on whether "no restriction" may present as GREEN/100 or must present as unknown.
9. **A1's batched CI re-seal rides on B1** (`DECISIONS.md:99`): the lockfile-only A1 test step, retiring the memory-only preview and its `# pass 19` child, re-pinning `rebuild.yml`. `.github/` is not lane B's and is not in this brief, but B1's closed profile must budget for it.
10. **H1** (`today.cjs:92 e.id === "hack"`) stays out of B1; v1.1 §4's recommendation is unchanged.
11. **`D24-2 drop-the-empty-ledger-guard` is caught only by a FIXTURE COINCIDENCE** (review r3 §4, re-measured by the r4 pass at BUILD-REPORT §10.4). Its only detectors are the **three D27 cells**, and they detect it only because the shared `stalled` D27 fixture happens to carry an empty `dailyLogs`, so dropping the guard makes yesterday owed and pushes `theOneFix` down to rung `logging`. **No D24 cell catches it.** The brief's own named killer — the empty-`dailyLogs` control in D24's *Must NOT change* list — is committed nowhere. If B2 or B3 ever gives that fixture a `dailyLogs` row, `D24-2` goes uncaught silently. One `pro([])`-style empty-ledger line in the D24 cell would close it; the r4 pass left it open because it is outside review r3's three required changes, and records it here so it is a decision rather than an oversight.
12. **`D21`'s fall-back coverage is now three years deep, but it is still enumeration, not a rule.** §A9 item 2 commits `2025-11-02`, `2026-11-01` and `2027-11-07`. A mutant that special-cased all three would still pass. The structural fix — asserting the *primitive* rather than the dates — is the source/alias cell's `plusDays` clause, which is already committed; this risk is recorded so a future pass does not mistake three dates for a proof.

### A7. `@noble/*` — documented, NOT fixed here (review C8)

Re-verified on this branch: `package.json` declares dependencies `react`, `react-dom` and devDependencies `esbuild`, `jsdom`, `yaml` — **`@noble/hashes` and `@noble/ciphers` appear nowhere in it**, while **11 files under `rebuild/` import them** (`rebuild/m3/w5/reconciliation/codec.cjs:4`, `rebuild/m3/w5/source/codec.cjs:5`, `rebuild/m3/w6/frame-crypto.mjs:1`, `frame-format.mjs:1-2`, `node-sha256-browser.mjs:1-3`, `test/frame-browser-entry.mjs:2`, `test/frame-codec.test.mjs:4-5`, and three build/test guards). `package-lock.json` mentions `@noble/hashes` only as another package's `^1.8.0 || ^2.0.0` requirement, and `npm ci --include=dev` installs 44 packages with **no `node_modules/@noble` at all** (`Test-Path` → `False`). So the **accepted parent package's own CI gate is unreproducible from a clean `npm ci`** — on this branch and on pristine `acd3b67` alike. That is a reproducibility hole in an *accepted* artifact, and `DECISIONS:98`'s recorded `native-carriers --ci PASS` therefore depended on a pre-existing `node_modules`.

**`package.json` is deliberately NOT changed by B1** — it is pinned by the accepted NATIVE-CARRIERS artifact and a dependency declaration is an integrator/tooling decision, not an engine-package one. Proposed cross-lane request line, for the PM to file verbatim in `rebuild/lanes/REQUESTS.md` (lane B did not write it there because that file is cross-lane and would collide):
...
### A8. What v1.2 does NOT change

The other nine hunks (D10, D8, D19, D16, D17, D24, D25, D27, D23) are **unchanged, byte for byte**, and so is `legacy-b1-carriers.cjs`. C7's `rebuild/m4/spec` half (`acceptance-b1-grading-time-window.json`, `b1-grading-package.cjs`, `b1-inherited-carriers.cjs`, the `witnesses-1`/`witnesses-3` `coverage.run → coverage.covered` move) remains **unauthored by lane B on lane-boundary grounds** (`LANES.md` ownership table) — the carrier already exports `COVERS` and `runWitness` for that child, and `b1-delta-cells.cjs` must be added to its required-artifact list. Every objection and PM question in §4, §5 and §6 below stands as written, except where §A4 items 1–7 correct them.

### A9. v1.2 AMENDMENT — the r4 fix pass (review r3's three changes)

> **Third amendment (r4 fix pass, a seventh agent — not the builder, not any of the three reviewers, not the r1/r2/r3 fixers).** `rebuild/lanes/b/reviews/B1-REVIEW-r3.md` returned **ACCEPT WITH CHANGES** with the PM's three acceptance conditions at `DECISIONS:103` item 2 all **MET by execution**. Three changes followed; all three are applied. Two are docs-and-cells only. The third moves **one engine byte**, and this amendment exists to say exactly what that byte is and what it is not.

**1. C-r3-1 — how the mutant matrix must be stated (docs; nothing measured moved).** The package may not say "33 of 33 caught" unqualified. It says, in the same sentence as the count, what §A5 above already says: **32 behavioural kills by a committed artifact, plus `D10-2` — behaviourally unkillable — carried by a declaration-scoped source assertion.** This binds `BUILD-REPORT-B1.md` §9.3 (corrected in place), **the verdict file, and the sealed C7 artifact**. Two numbers were corrected with it: §9.2's `1/33` → the measured **12/33** (§A5's "Consequence" paragraph and BUILD-REPORT §10.4), and the `BRIEF-IMPORT-GUARDS.md:88` citation → **`:89`** (§A4 item 5).

**2. C-r3-2 — `D21`'s fall-back years are committed (cells only; no engine byte).** §2 D21 has always named three delta cells and said mutant (3) is killed by *"the two other fall-back years"*. Only `2026-11-01` was in the repository. Review r3 measured the gap: a `D21-3` that special-cases the 2026 date survived all 23 cells, 6/6 carriers and all 45 laws including D22's frames parity. **`2027-11-07` and `2025-11-02` are now cells** in `rebuild/engine/test/b1-delta-cells.cjs` (`B1-D21-the-fall-back-anchor-holds-on-every-fall-back-year`), with both primitives, both good-night controls, an off-transition control, and review r2's `N1e` case folded in as a committed control. Measured: `clean` is `true` on base and `false` on the candidate for all three years; the mutant is now **CAUGHT**, by this cell and by nothing else. Residual risk 12 records what three dates do and do not prove.

**3. C-r3-3 — D27's `phaseArc(s)` is GUARDED inside B1. THIS IS A GUARD, NOT A SEMANTICS CHANGE.**

The route was checked on the tip first: `rebuild/lanes/REQUESTS.md` @ `5dc9254` carries lane B's own line offering to guard it inside B1 *"unless you route it to B3 (say so)"*, and neither `DECISIONS:108`/`:109` nor any `PM → B` line answers it. **Not routed → guarded here**, per the default lane B itself stated.

What it fixes: `today.cjs:309`'s `const arc = phaseArc(s);` was called bare from `theOneFix`, which `nowModelUncached:554` calls bare. `phaseArc` reaches `dietBreakState` → `daysBetween` → `mk(brk.end)` → `.split(...)`, so on a **stalled cut with a non-ISO `plan.brk.end` or `plan.brk.start`** the candidate threw `TypeError: s.split is not a function` out of both `theOneFix` and `nowModel` — Today's whole model — where the frozen engine returned advice. No law, carrier or cell caught it.

The hunk is one line, in the idiom B1 already uses two lines below for `sleepInfo`:

```js
const arc = (() => { try { return phaseArc(s); } catch (e) { return { key: "cut", weeks: weekDay().wk }; } })();
```

**Why the fall-back is `{ key: "cut", weeks: weekDay().wk }` and not something neutral.** Before B1 this gate read `const stalled = !sealed && cr.measured && cr.scale < floor;` (no phase term) and `const longCut = weekDay().wk >= 10;`. That fall-back reproduces both expressions exactly. So **in the region where `phaseArc` cannot answer, B1's behaviour is byte-for-byte the frozen engine's**, and D27's repair applies only where `phaseArc` does answer. Nothing in §2 D27's *Proposed behaviour*, its delta cells, its *Must NOT change* list or its law changes. A neutral default would have **moved** the advice on the reviewer's own fixture (`break` → `hold`) and would have been an unbriefed second behaviour change; this one is not.

**It is committed as a cell and as a mutant.** `B1-D27-a-phase-that-cannot-be-derived-leaves-the-pre-D27-reading-standing` asserts no-throw plus the base's own `rung`/`title`/`move.kind` on three malformed shapes, and carries the D27 controls that make it RED on the pre-B1 base. `D27-4 drop-the-phaseArc-guard` is an **r4-authored mutant beyond §2's 32**, labelled as such wherever it is counted, and it is caught by that cell and by nothing else.

**The bar is unchanged and still met**, re-executed on the guarded candidate (BUILD-REPORT §10.4, §10.5):

```
45 laws            : 10 rows moved (D8,D10,D16,D17,D19,D21,D23,D24,D25,D27); 35 byte-identical;
                     every moved row RED-frozen/GREEN-candidate on the candidate
D22 frames parity  : TRUE — byte-identical row on both trees, absent from both AUDIT-FAIL sets
33 mutants of the bar, detectors = COMMITTED ARTIFACTS ONLY:
                     32 BEHAVIOURAL kills · D10-2 by the source assertion · 0 HARNESS
                     (+ 2 r4 additions, D21-3b and D27-4, both CAUGHT — 35 run, 35 caught)
without the cells  : 12 of 33 CAUGHT · 21 survive
B1 carrier         : 6/6 PASS · 20 substitutions · witness files byte-identical to their pins
delta cells        : 25/25 hold on the candidate; 0/25 on base
conform + 2nd gate : line-for-line identical to base, 0 differing in all four streams;
                     conform assertion lines 33/33 identical, tally {BAD:6, OK:26, INFO:1} both
```

**Engine bytes after this amendment** — three unchanged, one moved, and the one that moved is this guard:
...
today.cjs  f8d0397abd75c02dd570741191124c32e26ab850702607826943d4394fda2e00
        -> 0df0fd169e05cc7659f34f36ea37f2d7965fd4f2db4d568a285bd04fb05c80e9   the D27 guard
```

If the PM would rather the guard went to B3, it is one line to revert and its cell goes with it; say so in `REQUESTS.md` and lane B will take it out.
