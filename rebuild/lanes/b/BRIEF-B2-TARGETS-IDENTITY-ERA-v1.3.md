# EARNED — LANE B · PACKAGE B2 — TARGETS, IDENTITY & TECHNIQUE ERA — BEHAVIOUR/DELTA BRIEF **v1.3, POST-REVIEW-r2 AMENDMENT** (PROPOSED, NOT ACCEPTED)

## v1.3 amendment (post-review r2) — READ THIS FIRST
...
### v1.3-B1. The Q2 hunk, FINAL FORM — `volume.cjs:159` reads the whole NAME FAMILY
...
**Exact hunk, final form** (`volume.cjs` pre-image `:159`; post-image `:162–164` plus one delegate at
...
```
This is now **character-for-character the owner test `_volDeltas` runs** (`progression.cjs:228–236`), modulo
...
| two lifts whose NAME FAMILIES collide, `[cur,old]` / reversed | `["cur"]` / `["cur"]` | `["cur"]` / `["cur"]` | `["cur"]` / `["cur"]` | `["cur"]` / **`["old"]`** | `B2-Q2j` |
...
3. **The one new order-dependence the former-name term introduces.** When two lifts' name families collide
...
## v1.2 amendment (post-review r1) — READ THIS SECOND, BEFORE THE v1.1 NOTE BELOW
...
**Still open for the PM, unchanged:** every question Q1–Q9 of §7. Q2 now has a **corrected** hunk on a
revertable commit and Q8 now has the executed inertness finding beside it; neither is answered here.
**r2's recommendation on Q2 is KEEP, with required change 1 applied** — which is what §v1.3-B1 does; the
PM's alternative of carrying Q2 to B3 is still available for the cost of one `git revert`.

---

*Everything from here to the end of the file is `BRIEF-B2-TARGETS-IDENTITY-ERA-v1.1.md` carried WHOLE (its own H1 replaced by the v1.3 line at the top). Section numbers and the PM questions Q1–Q9 in §7 are byte-for-byte v1.1, and where v1.2 corrects a sentence it is quoted and replaced in the amendment rather than rewritten below.* **v1.3 makes the ONE exception: §2's C3 and C6 are rewritten in place and marked `[corrected in v1.3]`, because `PLAN…:157` makes §2 the durable convention B3 implements and a B3 author reading the old text would reproduce a known regression. C1, C2, C4 and C5 below are still byte-for-byte v1.1. The superseded C3 sentence is quoted in §v1.3-B2.**
...
`PLAN…:157` requires this brief to state the convention so the engine does not end up with two. It is stated here as a **durable lift-identity scheme**, not a string trick, and it is deliberately reader-first so B2 can land it without touching `writers.cjs`, `merge.cjs`, `earn.cjs` or `migrate.cjs`. **C1, C2, C4 and C5 are reproduced verbatim from v1; only the three line citations inside them are re-pinned onto `87eddad`, marked `[re-pinned]`. C3 and C6 are `[corrected in v1.3]` — the superseded C3 sentence is quoted in §v1.3-B2 and the reason is r2's required change 2. B3 must implement the C3 BELOW, not the one in v1/v1.1/v1.2.**
...
- **VOLUME receipts** (`writers.cjs:2162`, `:2319` `[re-pinned from :2135, :2292]`) emit `VOLUME ±N — <MG> via <ex.n> (now <n> sets)`. Read the **tail** — everything after the first `"via "`. The producer appends **exactly one** `" (now <n> sets)"` suffix, but a lift's own NAME may contain the string `" (now "`; therefore the suffix is cut at the **LAST** `" (now "` in the tail, never the first. Ownership is an exact `===` against `_formerNames(ex)` of **either** (a) the tail with that last suffix removed, **or** (b) the whole tail unchanged — (b) is what carries a legacy row that was written with no suffix at all. Two exact comparisons over the whole name family; nothing else. A row with no `"via "` (e.g. `VOLUME PASSED`) has **no owner** and is skipped, never guessed.
  - *Executable form, identical in `_volDeltas` (`progression.cjs:232–236`) and — with Q2 — at `volume.cjs:159`:* `at = t.indexOf("via ")` → `tail = t.slice(at+4)` → `cut = tail.lastIndexOf(" (now ")` → `own = cut < 0 ? tail : tail.slice(0, cut)` → `_formerNames(ex).some(n => n && (own === n || tail === n))`.
  - *Known bound (see §v1.3-B3):* when the row carries **no** suffix, (a) and (b) can name **two different lifts**, and the convention does not make that receipt unambiguous. That is C4's standard failing against a legacy shape, not a licence to loosen the comparison; the close is C2's writer-side `exId`, not a wider C3.
