# BRIEF — M2-H3-CLEAN-INIT · v1.6 (lane B, child of M2-B-NTC)
...
## 6 · HUNKS (sha256, `ce38aa3` → HEAD) — unchanged since v1.5; v1.6 moves no tracked byte but this brief
...
* **F-C — the grant is EXTENDED at `DECISIONS:146 (1)`, and the successor is a build this revision ATTEMPTED, EXECUTED IN SCRATCH, AND CANNOT DECLARE — for reasons that are now measured rather than predicted.** `:146` line sha256 **`3dc537652cdf8f6b9c012ac88096f8f8cc7d9156ce5dbbd3453a3936c2b9c2c0`** on `origin/rebuild/t2-client-core` (line 146), verified against all four predicates the runner applies: it names `M2-H3-CLEAN-INIT`, uses the word SUCCESSOR, stands on `:113`, and names all three support paths. The admitted gate set is **measured, not chosen**: `successorGates()` (`b-package.cjs:529/:542`) admits a parent `byChild` gate only when the carrier's own source closure contains the support path as a literal, and `rebuild/engine/writers.cjs` reaches all five inherited carriers (`constants.cjs` reaches none; `journey.test.mjs` reaches `b-ntc-journeys.cjs` only, which is not a `byChild` gate). So `support` is `rebuild/engine/writers.cjs` and the five gates are `migrate-source`, `merge-source`, `writers-source` → `source-carriers`; `witnesses-2`, `witnesses-5`, `migrate-differential` → `inherited-carriers`; `witnesses-7` → `defect-witnesses`; `writers-differential`; `second-gate`.

  **WHAT THE BUILD DID, AND THE THREE MEASURED REFUSALS.** v1.5's shape was built and executed, in scratch only (`work/lane-b/h3fc-scratch`; no tracked byte written, `git status --porcelain` empty before and after). The parent's shared support module `b-ntc-successors.cjs` was read from the Git blob at the reviewed commit `9ad2ecab…` and compiled privately (`new Module()` + `_compile`, never `require`, nothing in `require.cache`), with its `node:fs` answered so that every path `packages/H3.json` declares changed is read at `9ad2ecab…` — B-NTC's own `archivedReader` mechanism extended to H3's declared changes and driven by H3's product map alone. `run('source-carriers')` then **refused**: `Exact product construction: rebuild/engine/writers.cjs` — `native-carriers-source.cjs:91`, inside `preflight()`'s second stage `support.verify(root)`, which reads the real disk by design and compares H3's `writers.cjs` against the carrier pin at `native-carriers-source.cjs:44`. With that pin re-targeted to H3's post in a scratch copy, the gate refuses **again**, one layer earlier: `Recovered ACCEPTED preimage rebuild/engine/writers.cjs` at `native-carriers-reference.cjs:55` — `Reference.create(root)`, which `run()` calls for every gate, inverse-applies 34 declared literal edits to the candidate bytes and pins the recovered preimage in `native-next-target-candidate/source-delta.cjs`. Behind those stand two more, read in the same bytes: `constants.cjs` is a RETAINED file (`native-carriers-source.cjs:13`) asserted byte-equal to `git show BASE:`, and the gate body itself re-reads every prior engine module against the declared carriers (`native-carriers-source-carriers.cjs:80`/`:89`) and pins the substitution order (`:138`). **The parent's inherited gates are byte-identity gates over the engine; H3 changes the engine; so each of them has to be RE-TARGETED, not merely re-hosted — and v1.5's "no text substitution at all" shape is refuted.**

  **AND THE ADMISSION SURFACE HAS NO TARGET FOR ANY OF THEM.** `:113 (1) (c)` admits one thing only — a pin re-target *enumerated verbatim in the package spec* — and `b-package.cjs:1603` requires every `sub.original` to be a **parent EXECUTION PIN** (`:969` additionally fixes its shape to `rebuild/m4/spec/*.cjs`). B-NTC's `executionPins` are its own ten thin wrappers plus `b-package.cjs`, `packages/B-NTC.json`, the brief and three test files — enumerated and **measured**. Not one of the files the re-targets must live in is among them: `native-carriers-source.cjs`, `native-carriers-reference.cjs`, `native-next-target-candidate/source-delta.cjs`, `native-carriers-changes.json` and the gate bodies are neither B-NTC execution pins **nor** B-NTC `product` paths; `b-ntc-successors.cjs` — whose `SUPERSEDED` const would have to name H3's paths — is a `product` path but **not** a pin, and that is the blocker v1.5 disclosed, now seen to be the smallest of five. A successor that applied these changes *without* declaring them would be refused by `SUCCESSOR-SUBSTITUTION-TABLE-DISAGREES-WITH-THE-SPEC` (`b-package.cjs:1663`), correctly; a successor that declared them is refused by `:1603`. **There is no admissible re-target, so there is no successor to build.** The anchor v1.5 proposed — the Git blob at `9ad2ecab…` for the unpinned support module — was built and is sound as far as it goes; it is simply not enough, because four more unpinned files stand behind it.

  **A SECOND REFUSAL, INDEPENDENT OF THE FIRST, AND WHAT WOULD UNBLOCK F-C.** Even if every pin above were in reach, `b-package.cjs:1657` refuses all five originals on its own: the copy test requires the original to carry **at least eight** trimmed lines of 40+ characters, and each B-NTC gate wrapper is nine lines carrying exactly **seven** — `SUCCESSOR-ORIGINAL-TOO-SHORT-TO-PROVE-A-LOAD`, measured for `source-carriers`, `inherited-carriers`, `defect-witnesses`, `writers-differential` and `second-gate` alike. The floor is calibrated to B-NTC's *own* originals, the large `native-carriers-*.cjs` programmes; no child of B-NTC can ever meet it, because B-NTC's carriers are wrappers. **Both refusals are runner and parent-pin facts, not spec truth**, so lane B stopped there: `b-package.cjs` was not edited, `packages/B-NTC.json`'s posts were not rewritten to H3's bytes, no `coverage.successors` block was fabricated, and no scratch file was promoted into the tree. `coverage.successors` stays `null`, the five inherited carriers still refuse, and `--ci` still ends `B PACKAGE H3 FAIL` exit 1 at `Required child source-carriers` — the same line v1.5 reported, now with its cause read to the bottom. What would unblock it, for the PM and the tooling lane to choose between: **(a)** B-NTC's seal, or a successor ruling, pins the files a child of B-NTC must re-target (`native-carriers-source.cjs`, `native-carriers-reference.cjs`, `source-delta.cjs`, `native-carriers-changes.json`, `b-ntc-successors.cjs`) so `:113 (1) (c)` has a target; **(b)** `:113 (1) (c)` is widened to admit a re-target over a **parent-transitive** original anchored through the parent's own artifact chain, and the `>= 8` line floor is re-expressed against the original's body rather than its long-line count. Lane B has the authority to do neither, and says so rather than choosing.
