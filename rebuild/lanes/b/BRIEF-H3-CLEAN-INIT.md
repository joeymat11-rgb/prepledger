# BRIEF — M2-H3-CLEAN-INIT · v1.8 (lane B, child of M2-B-NTC)
...
## 6 · HUNKS (sha256, `ce38aa3` → HEAD) — v1.8 re-measures every row on the merged tip (`origin/rebuild/t2-client-core` @ `ec80cbe`) and adds the six files the supersession and the `sleep.needed` fold bring
...
## 7 · CELLS — `h3-clean-init.test.cjs` 14/14 and the five `h3-supersede-*.test.cjs` 16/16, exit 0 each
...
| `second-gate` | the independent second reading: the public record does not move | `h3-sup-second-gate` — both compositions (accepted writers from the blob, H3's from disk) over the **frozen seed record** across a 15-reader projection, compared as one string, plus `applyRead` itself: byte-identical; the one input they differ on is a state with no trend, which the frozen record cannot hold | 1 of 3 cells |
  ...
  **THE LINE THE PM MUST WRITE, VERBATIM.** The grant is a TOKEN and nothing else (r10 F1 measured that prose about a supersession admitted one). It must stand **alone in its own `·`-delimited clause** on a `RULED` ledger line — a word or mark before or after it inside the same clause frees nothing, so a line may discuss, quote or refuse the token safely:

```
GATE-SUPERSESSION M2-H3-CLEAN-INIT source-carriers,inherited-carriers,defect-witnesses,writers-differential,second-gate
```

  Until it lands, `coverage.superseded.rulingLineSha256` is the placeholder `null` and `--ci` refuses **`B PACKAGE H3 FAIL GATE-SUPERSESSION-RULING-NOT-CITED`**, exit 1 — the expected pre-ruling state, reached with all nine declared children green and nothing else wrong before it. **PASS 2 IS PROVEN, not predicted**: in a `--shared` scratch clone with that exact token on a probe chain branch and `CHAIN_REF` re-pointed at it (exactly one line of the real runner differs, asserted), the run prints `COVERAGE 0/19 original gate(s) covered by 0 executed child(ren) (0 inherited…); 9 SUPERSEDED`, `SUPERSESSIONS 5 byte-identity carrier(s) of B-NTC SUPERSEDED over 9 gate(s) … located on refs/heads/probe-chain BY ITS OWN SHA256`, five `SUPERSEDED <carrier> <- <gates>` lines and five `SUPERSEDED EVIDENCE` lines, then `CI REVIEW-PENDING: 2 open obligation(s)` (theme, brief) and **exit 2**. The probe clone wrote nothing in this worktree and is deleted.
