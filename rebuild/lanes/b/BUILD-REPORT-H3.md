...
**ONE HOLD-OUT, NAMED IN THE OPEN.** `food.test.mjs` is **not** added. Two of its cells execute the PRE-H3 engine as their own expectation — `N1.11` ("the engine cannot produce a plan for a clean-init athlete yet", `food.test.mjs:574-583`, which the file itself calls "the fact H3 will change") and `D2.1` ("the engine holds no figure for him yet") — so they are RED by design on any tree carrying H3; and its build cells `N1.15`/`N1.17` race `view.test.mjs`'s `R3` over one build output inside a shared `node --test` invocation (the 12-file argv measures 495 tests, and the fail count is **non-deterministic: 3–5** — `N1.11` and `D2.1` every run, plus one to three of the build cells; r3 first wrote a flat "5 fail", corrected here per H3-REVIEW-r3 F3 from four runs, the reviewer's three (4/4/3) and lane B's one (5)). Naming it would ship a red step. Lane C updates those cells and H3 then names the file; H3 does not edit another lane's test. The step comment carries the reason in full. The PM's confirming word was asked in the lane-B `REQUESTS.md` entry of 19:05 ET, which stands on the chain branch at `d7f5b7b` / `c40136d` and **not** on this branch — H3-REVIEW-r3 F5 is right that the citation does not resolve at `fffc668`. It was given at `DECISIONS:178`.
...
# §r3b — H3-REVIEW-r3 APPLIED, RE-SEALED ON THE `:179` TIP

`rebuild/lanes/b/reviews/H3-REVIEW-r3.md` @ **`3bb7612`** — **ACCEPT WITH CHANGES**, blind delta review of `e994c10..fffc668`, no private fixture opened. Its four required changes are applied here; §r3 above stands as the record of that round, and the values it names (`d7af7525…`, `0bcc14fb…`, `3cdc1ab8…`, commit `24b7073`) are superseded by this section.

**F1, the merge-gating one.** Chain tip **`c5552f5`** (`:178` confirming variant (i), `:179` the settings display) merged `--no-ff` at **`0633099`**; `merge-tree` clean, no conflict. The tip carries `rebuild/m3/w7-preview/today/test/machine-settings-ui.test.mjs`, which `DECISIONS:177 (1)` names and which no workflow named — the reviewer's probe (iv) measured `H3/13` RED on exactly that file. It is now named by exact path in the today step: **12 named**, `food.test.mjs` still the one hold-out. Measured here: ms-ui alone **51/51, exit 0**; the exact 12-file argv **490 tests, 490 pass, 0 fail, exit 0**, three consecutive runs — unlike `food.test.mjs`, ms-ui does not race the shared build; `setup.test.mjs` alone 157/157. `H3/13` needed **no logic change** — it reads the named set off the step line — and `h3-clean-init.test.cjs` is byte-unchanged at `69e20eb4…`. Proven both ways on this tree: with ms-ui named **14/14, exit 0**; with ms-ui removed from the step line and nothing else touched, **1 of 14 fail, exit 1** on the directory/named-set assertion.

**F2** is the PM's, and was given: `DECISIONS:178` confirms variant (i) — the hold-out stands, this `rebuild.yml` hunk is lane B's, FREEZE withdrawn. **F3** and **F5** are applied in §r3 above (the race count is now the measured 3–5, and the `REQUESTS.md` citation now names its chain-branch commits); F5's second half is applied in `packages/H3.json`'s `notes`, which said the brief errata stand "because the brief bytes are pinned by `:166`" and now cites `:169` — `notes` is not one of the 21 keys `proposed()` writes into the sealed artifact, so that correction moves the spec sha256 and nothing else. **F4** is applied in `rebuild/lanes/b/VERDICT-H3.md`, refreshed to these values.

**Re-pinned:** `.github/workflows/rebuild.yml` `3cdc1ab8…` → **`61fb1a86…`** (10,010 B); spec `packages/H3.json` `0bcc14fb…` → **`1a57a770…`**. Nothing else moved. All ten declared children green: 4 / 3 / 3 / 3 / 3 supersede cells, engine-files differential exit 0 over 27 files, `h3-cells` 14, `a0-journeys` 23, `today-suites` 321 at its unchanged eight-file argv and needle, `ntc-provider-cells` 39.

**The artifact**, own commit **`521a004`**: sha256 **`2b6e579eec4fadc2165e6edadd7f7a3a2ab18ae76950063634596598a2825262`**, 36,069 B, parent B-NTC `87f4848c…` unchanged; it differs from `d7af7525…` in three hashes only (spec twice, the rebuild.yml post). `review-h3-clean-init.json` stays `{version:1, status:"PENDING", receipt:null}` — `:172`'s receipt names `61bd6d3f…`, two artifacts back, so these bytes need a new receipt line.

**The runs.** `--ci --package H3` → `B PACKAGE H3 ENVELOPE PENDING artifact=2b6e579e… spec=1a57a770… runner=4482bb8a…`, ten of ten children `OBSERVED; exit 0`, one `B PACKAGE H3 OPEN independent exact-artifact acceptance PENDING`, then `B PACKAGE H3 PUBLIC CI EVIDENCE PASS — public evidence only, NOT the package verdict; …`, **exit 0**. `--full --package H3` with the private fixture → `PRIVATE ORACLE PRESENT; verdict-only reporting`, `FULL EVIDENCE: 10 of the 19 original gates re-executed, 0 carried … and 9 SUPERSEDED under DECISIONS:160; second gate included`, `B PACKAGE H3 POSTFIX PACKAGE REVIEW-PENDING: 1 open obligation(s); independent exact-artifact acceptance required`, **exit 2**. Same 10 + 9 matrix as both earlier seals. No private value, count or hash is recorded here.

---

# §r3c — THE B-NTC STANDING CI STEP RETIRED, AS `ce38aa3` RETIRED ITS PARENT'S

The integrator held the merge: on the r3b tree `--ci --package B-NTC` — the step `rebuild.yml` still carried — refuses `B PACKAGE B-NTC FAIL SUCCESSOR-BLOCK-KEYS-NOT-CLOSED; required evidence missing or failed; local diagnostics withheld`, **exit 1**. That is real but it is not the cause; it fires in `spec()` and masks the cause. Measured on a throwaway branch by filling the successor block with its nine truthful keys — `ruling` `DECISIONS:113` located by its own sha `e4ca487f…`, `support` `rebuild/m4/workout/engine-runtime.cjs`, `wrapper` `rebuild/m4/spec/native-carriers-package.cjs`, `reviewFile` `B-NTC-REVIEW-r2.md` `e27278aa…` — the shape check passes and the next line is `B PACKAGE B-NTC FAIL SEALED-PROFILE-RECOMPUTATION; required evidence missing or failed; local diagnostics withheld`, **exit 1**.

**Root cause, two sentences.** B-NTC's sealed, receipted artifact `87f4848c…` (ACCEPTED at `DECISIONS:141`) pins `spec.sha256` `05a5aa1f…` — `packages/B-NTC.json` as it stands at the chain tip and at H3's `sourceBase ce38aa3` — and `runner.sha256 eedabccd…`; H3's tree carries `c4a8cf92…`, because H3 re-pins that spec onto the r10 runner `4482bb8a…`. A cumulative child that re-pins its parent's spec cannot also leave the parent's gate standing, so the keys were never the bug, and filling them would only trade a shape refusal for one saying an accepted artifact no longer equals its spec.

**The precedent, and the hunk.** `git diff ce38aa3^1 ce38aa3 -- .github/workflows/rebuild.yml` shows B-NTC doing exactly this to *its* parent: it deleted `- name: Cumulative extracted-engine native-carrier and legacy-census evidence` / `run: node rebuild/m4/spec/native-carriers-package.cjs --ci` and put its own line in that position. The child's gate **is** the standing step; the parent's is retired, not kept beside it — H3 kept B-NTC's, and that was the defect. Fixed in the same position, same job, same `os: [ubuntu-latest, windows-latest]` matrix, one command changed: `- name: Cumulative H3 clean-init, native-carrier and legacy-census evidence` / `run: node rebuild/lanes/b/tooling/b-package.cjs --ci --package H3`, plus eight comment lines citing `ce38aa3`. Nothing else in the file moved — the today step, its twelve names and the `food.test.mjs` hold-out are untouched. No `run:` line in any of the five workflow files now executes `--package B-NTC`; exactly one runs `b-package.cjs`, at line 90.

**Custody checked before editing.** `git grep -l -- '--package B-NTC'` and `git grep -l 'Cumulative B-NTC native-carrier'` over `rebuild/` and `.github/`: **every hit outside `rebuild.yml` itself is Markdown** — ledger, lane reports, reviews. No test cell, preflight or `.cjs/.mjs/.json` pins the step's name or command, so nothing frozen had to be edited and none was. The three code files that *read* `rebuild.yml`: H3/13 in `h3-clean-init.test.cjs` reads only the today step (14/14 here); `rebuild/m3/w6/test/local-witnesses.test.mjs` names it in a comment only; `rebuild/conform/v4/postfix/test/ci-second-gate.test.cjs` asserts the whole workflow equals an `a777f64` blob with one substitution and was measured **RED before this edit** (34 pass, 1 fail, exit 1) — stale since long before H3, as H3/13's header has recorded since r1, and not made newly red here.

**Measured, re-pinned, re-sealed.** Twelve-file today argv **490/490, exit 0**; `setup.test.mjs` 157/157; `h3-clean-init.test.cjs` 14/14; all ten children green. `.github/workflows/rebuild.yml` `61fb1a86…` → **`19e861df…`** (10,692 B) and spec `packages/H3.json` `1a57a770…` → **`0253fed8…`**; nothing else moved. Artifact, own commit `5a5a965`: **`d3c760c3862262beef5d10f6004a5a96e25f1219b23c8da98a653289c0a50cce`**, 36,069 B, parent `87f4848c…` unchanged, three hashes different from `2b6e579e…`. The envelope goes **back to PENDING / `receipt: null`**: `DECISIONS:180` names `2b6e579e…` at `521a0049`, and an envelope is void the moment the artifact bytes move, so these bytes need a new receipt line. `rebuild/lanes/b/tooling/receipts/H3.json` is left in place as the record of the `2b6e579e…` run only — the runner consults it solely under an AUTHORIZED envelope and reports any mismatch as `AUTHORIZED STEP UNAVAILABLE`, never a refusal.

**The runs.** `--ci --package H3` → `ENVELOPE PENDING artifact=d3c760c3… spec=0253fed8… runner=4482bb8a…`, ten of ten children `OBSERVED; exit 0`, one `OPEN independent exact-artifact acceptance PENDING`, `PUBLIC CI EVIDENCE PASS`, **exit 0** — and this is now the standing CI step. `--full --package H3` with the private fixture → `PRIVATE ORACLE PRESENT; verdict-only reporting`, `FULL EVIDENCE: 10 of the 19 original gates re-executed, 0 carried … and 9 SUPERSEDED under DECISIONS:160; second gate included`, `POSTFIX PACKAGE REVIEW-PENDING: 1 open obligation(s)`, **exit 2**. Same 10 + 9 matrix, no private value recorded.

---

# §r3d — `DECISIONS:186`: THE LAST HOLD-OUT CLOSED, EVERY FILE NAMED

**Why, and the hunk.** Head `b111615` already carried the tip `d028c54`, still an ancestor of HEAD, so r3d needs no merge — the delta is one `rebuild.yml` name and the H3/13 flip. `food.test.mjs` was held out through r3/r3b/r3c for two measured reasons: `N1.11` and `D2.1` executed the PRE-H3 engine as their own expectation, RED by design on any tree carrying H3; and its build cells `N1.15`/`N1.17` raced `view.test.mjs`'s `R3` over one build output in a shared invocation, 3–5 fails per run. Lane C's `:185` made the suite engine-version-aware and un-raced the directory — measured here before naming it, **57/57, exit 0** — so `:186` orders it named and H3 names it: `rebuild/m3/w7-preview/today/test/food.test.mjs` by exact path, **thirteen named, which is every file in the directory**, one invocation, both matrix runners, no glob and no exception. The step's comment is rewritten so it no longer describes a hold-out and states the rule in full. Measured with the exact argv read back off the step line, **three consecutive runs: 547 tests, 547 pass, 0 fail, exit 0** each — agreeing with lane C's own two runs of the whole directory; `setup.test.mjs` 157/157.

**H3/13**, id kept, no other cell touched. `HELD_OUT` is gone entirely, and with it the two clauses that existed only to police it (no `run:` line executes the hold-out; a comment names it). What remains is the simpler law: `assert.deepEqual(onDisk, NAMED)` — directory and named set equal in both directions — plus every named file exists and no glob. **Red-first, measured:** with `b111615`'s `rebuild.yml` restored in place (`19e861df…`, the twelve-name step) the cell fails on *the today step names EXACTLY the files in rebuild/m3/w7-preview/today/test/ — every one of them, nothing else, no glob and no hold-out (DECISIONS:186)*, 1 of 14 fail, exit 1; on the r3d bytes 14/14, exit 0.

**Re-pinned, re-sealed.** `.github/workflows/rebuild.yml` `19e861df…` → **`34bec65a…`** (10,610 B); `rebuild/m4/workout/test/h3-clean-init.test.cjs` `69e20eb4…` → **`7eac2e17…`** (49,398 B); spec `packages/H3.json` `0253fed8…` → **`6c9eca87…`**. All ten children green, `today-suites` unchanged at its eight-file argv and `# pass 321` needle. Artifact, own commit `5f0c378`: **`b457b539a384d8c72531b880cd771e996c6b231f034a49272e899c1fba61e61f`**, 36,069 B, parent `87f4848c…` unchanged, five hashes different from `d3c760c3…`. Envelope back to PENDING / `receipt: null`: `DECISIONS:183` named `d3c760c3…` and its authorized rerun genuinely passed, but `:186` supersedes those bytes, so a fourth POSTFIX-ACCEPTANCE line is needed; `receipts/H3.json` `29c29e1b…` stays as the record of the `d3c760c3…` run and is not evidence for these bytes.

**The runs.** `--ci --package H3` → `ENVELOPE PENDING artifact=b457b539… spec=6c9eca87… runner=4482bb8a…`, ten of ten children `OBSERVED; exit 0`, one `OPEN independent exact-artifact acceptance PENDING`, `PUBLIC CI EVIDENCE PASS`, **exit 0** — and since r3c that command is the standing CI step. `--full --package H3` with the private fixture → `PRIVATE ORACLE PRESENT; verdict-only reporting`, `FULL EVIDENCE: 10 of the 19 original gates re-executed, 0 carried … and 9 SUPERSEDED under DECISIONS:160; second gate included`, `POSTFIX PACKAGE REVIEW-PENDING: 1 open obligation(s)`, **exit 2**. Same 10 + 9 matrix, no private value recorded.
