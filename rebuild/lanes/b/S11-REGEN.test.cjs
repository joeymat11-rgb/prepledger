...
for (const [label, note, head] of DRAFT_CLAIMS) {
  test('S11-REGEN S11 D-S11P-2: --write refuses while ' + label + ' is carried, and writes nothing', () => {
    const w = sealed(world(), 'ACCEPTED'); w.allowWrite = true;
    ...
    assert.match(r.out, /carried note\(s\) still cite the S10 candidate or a draft-time claim/);
  });
}
test('S11-REGEN S11 D-S11P-2: a dry run over the draft layout flags [0] and the four draft-time notes [4]-[7], and neither the regenerated [1]-[3] nor a carried debt note [8]', () => {
  const w = world();
  editSpec(w, (s) => {
...
test('S11-REGEN S11 D-S11P-2 CONTROL: a carried note with no candidate id and no draft-time claim is kept, unflagged, and --write proceeds', () => {
  const w = sealed(world(), 'ACCEPTED'); w.allowWrite = true;
