...
The154/184 snapshot at64f59af checked current registration separately: exactly one B1-B2 cumulative workflow command, both operating systems, all13 existing Today test files, and every other standing command. At that head N2 was unadmitted and the complete successor profile remained pending. The232/236 composition below supersedes that construction status.
...
The separate232 grant authorizes the five current replacement families. It does not transfer H3's execution results or reuse its160 token. Every family needs its own executed red-first child, all24 laws, combined original witnesses, U/FG writer cells, full engine-file comparison and original public census. The closed registry requires those exact children and executable targets; it refuses missing, duplicate or extra children, mode aliases, sourceBase overrides and old H3 evidence selection. A present malformed B1-B2 profile in Today's setup check refuses; historical lookup is used only when it is absent.

Three repaired witness routes are separate children: original witnesses1/3/4 at their unchanged SHA256, their exact ordered12/9/5 reviewed substitutions and their literal `--witness-1/3/4` modes. Each must retain its original10/5/5 cases and complete original terminal before its own final summary. The runner checks actual carrier ownership and mode mapping, full nonzero output and separate execution. They add three covered gates, disjoint from the nine superseded and seven original executions. `coverage.moves` stays empty. Default combined execution still preserves all four original witness programs,31 cases and35 substitutions, including D12 exactly once; its stdout now includes all original case lines and terminals before its unchanged summary.

The ten new workout evidence files preserve the original H3 four-hunk construction, literal forward/inverse proof of eight runtime changes, full unchanged inventory,15 reader projections and whole applyRead results. The H3 wrapper retains14 tests and its seven mutation controls; only eight enumerated S1 expectation substitutions are admitted, with inverse whole-body equality. Original native/census programs and inputs are not replaced by the synthetic45-law evidence. A failed native side or unapproved field difference fails the child.

DECISIONS236 admits exactly Cbfc293573e1010559e0119bdc7ff666e89f740cc/source60e24a8:12 source/test blobs plus the preserved builder report, fixed by13 post-image hashes in the runner and profile. The Today command explicitly keeps all13 standing files and adds sleep.test.mjs. C owns those implementation bytes; B owns only the232 setup lookup hunk. D2's prior scoped acceptance does not establish the result in this new engine context; a named combined-head D2 review remains required.

Node TAP children must report exactly one complete tests/pass total matching the declared positive count, and zero fail/cancelled/skipped/todo. New native expectations are marked UNMEASURED in the profile: source5, inherited5, defects5, writers152, second4, retained H3 14, Today630=553+77. The PM's236 ordering permits prospective static counts before review. Actual later authorized execution must match them; mismatches are recorded failures and any changed expectation requires affected review, with no automatic adjustment.
...
## B1-B2 R3 Today repair registration (PM262)

The R2 section above records its original head and scope. The R3 engine/evidence source is T=`797e4cf39fac39148ccae784116999669b82caf3`, following S=`48a3063a23528ed240eb2356226d237d2793a9da`. The profile retains112 product pins,26 literal child commands, the same15 public command licences and9+3+7 coverage. The runner, modes, executable allowlists, parent receipt and held authority bindings are unchanged.

The source map retains the original M-to-R eight-file/65-hunk record and the R-to-S four-file/10-hunk record with five evidence files. Its additive S-to-T record contains exactly three `today.cjs` hunks and four changed existing engine evidence files. The historical SUCCESSOR inventory describes R/S; the separate REPAIR inventory pins the complete disk/HEAD/T inventory. Native comparisons and the public-census terminal now name M/T with an empty admitted-delta list.

Public-law admission accepts complete M, R, S and T public module manifests, including the additive `REPAIR_SOURCE_MANIFEST`. T requires the exact Today postimage `80d4196cfe50637ed373dcf5fa0eab1c2ef549b957aea7c032ab518d2a67ba91`; mixed historical/current modules refuse. The existing registration test retains its M/R/S refusal controls and adds the T manifest, one-file boundary, preimage/postimage and unchanged-module controls. Its two source mutants expose a comparison that checks only the changed files. This extends the existing test body, keeping the nine-file tooling cohort's prospective count at267.

R3 registers323 U/FG tests and the existing public audit terminals. The builder reported323/323 and63/63 current behavioral faults at `2f764600`; the parent task reruns its licensed public commands on final T bytes. Source62 and writers327=4+323 remain prospective and unexecuted here; inherited5, defects5, second4, H3 14, Today630, A0 23 and NTC39 retain their scoped holds. The new tooling bodies and complete267-test cohort are also unexecuted by the registration author.

Registration authoring checks syntax and metadata only. The protected seed pin is retained from parent metadata without reading seed bytes. No package CLI, native/source-family child, frozen/private/FULL gate, workflow, receipt, integration or phone result follows. The mechanical authority amendments remain a separate held pre-gate requirement. R3 remains local and unpushed pending the required independent review and authorized evidence.

Final R3 parent verification: all14 licensed public commands pass on final T within their declared scope, including the unchanged45-row public-law comparison. Final R3 nine-file tooling cohort267/267 PASS on Node22.23.2 ate3025161841892a7628dbfe0dd916a879676a7f1,374491.4785ms,exit0,zero fail/cancel/skip/todo. .tmp/r3-tooling/cohort.tap,220584bytes,SHA256bed657645ead14300ff5ae5a0c06a26d065cbbf2e77bb91d65dfdb18b134ccf3; input manifest24d41adab186cb6c0a6c235fdf3a2b32970da3f273e8b693343d1b8f7cd3b45e. Before/after public input bytes matched. Exact profile tooling-cohort.argv; actual declaration/continuity functions in synthetic repositories, no real package CLI. Profile notes and this README are finalized afterward; final registration is rerun on those bytes. Full evidence and the separately held next steps are in `../BUILD-B1B2-TOOLING-154-184.md`.

## B1-B2 R4 captured-date registration (PM282)

The preceding records keep their original heads and scope. R4 uses engine/evidence source U=`cbcebb3d26bf8892adbe84fbbe0a7c5e85856013`. The additive `changes.r9` record starts at T=`797e4cf39fac39148ccae784116999669b82caf3` and declares exactly one literal `today.cjs` hunk plus four modified existing public evidence files. The hunk derives the workout date from the captured `tISO`. Every original M/R, R/S and S/T record remains unchanged. Historical SUCCESSOR and REPAIR inventories describe R/S and S/T; the separate R9 inventory binds disk/HEAD/U. Candidate loaders, migration sides and census comparisons now name M/U, with no admitted native deltas.

`R9_SOURCE_MANIFEST` adds the exact T preimage and U Today postimage `180cdfd01be61de258477bf78c5de27f4deecaee748710c613976a303b29998f`. Public-law admission and its existing registration test retain complete M/R/S/T checks and add the U manifest boundary, exact pins and mixed-module refusals. A third source mutant detects admission based only on the changed Today file. The profile retains112 product pins,26 literal children,15 public command licences,9+3+7 coverage and every standing authority hold.

R4 registers331 U/FG tests (131+200), source94 and writers335=4+331. Source94 and writers335 remain prospective and unexecuted. The registration extension adds no test declaration, so the nine-file tooling expectation remains prospective267 until the parent runs that exact cohort. The parent reports331 public positives,64 current behavioral kills, historical34 behavioral kills plus one alias assertion and the separate phaseArc probe. Those public results do not supply native or source-family credit.

Metadata authoring exposed an inherited historical blocker: `validateSuccessor` required R to be an ancestor of S, while the complete Git graph reports their exact common base as M. The original refusal and the transient, reverted proposal remain recorded separately. PM288 then approved the exact correction: the complete `git merge-base --all R S` result must equal only M. Fixed M/R/S identities, historical record hashes, the entire R/S delta and inverse proof, S/T and T/U ancestry, and every later ancestry duty remain binding. Bounded metadata reconstruction verifies all four source stages and the exact current public disk/HEAD/U bytes; it executes no native source module or source-family test.

Source94, writers335 and tooling267 are declared expectations; the final parent evidence belongs in `../BUILD-B1B2-R4-R9.md`. This registration does not claim a current tooling result or source-family, native/frozen, census, H3, private/FULL, package CLI, workflow, receipt, integration or phone execution. The seed pin remains metadata only. All work is local and unpushed.

## B1-B2 amendment authority (PM300/304)

Only `M2-B1-B2` requires `authorizations.amendments`: `repair-and-availability`, `reference-factory`, `today-clearance`, `captured-date` and `source-graph`. The runner fixes the five real PM line hashes and six ordered document path/hash pairs from PM300; the profile cannot nominate different authority. Each claim retains the existing exact four-field shape and Astra PM issuer. Ledger numbers remain explanatory coordinates; exact unique line bytes, issuer/handover and shared-chain ancestry authenticate the claim. Duplicate decoded JSON keys, missing/extra IDs or fields, changed paths/hashes and reordered documents refuse.

The same checker runs in inventory/spec, authority, artifact construction and envelope entry/terminal paths. Every authority call reads Git again; the accepted envelope additionally reads the documents at its actual receipt context. FULL entry and terminal bind all six documents at candidate HEAD and on disk. The authorized envelope also binds them at the reviewed commit and candidate HEAD/worktree, including CI, using the fixed document pins after product/execution declarations are collected. The artifact's existing `authorizations` field carries the complete claims and documents without changing other packages' artifact shape. The original owner, contract, theme, brief, parent and supersession checks remain.

PM304 extends only the existing continuity fixture constructor: five synthetic claims and six synthetic documents, with exactly five named identity substitutions (`CHAIN_REF`, `PM_HANDOVER`, `B1B2_PARENT`, `B1B2_SUPERSESSION_LINE`, `B1B2_AMENDMENTS`). Exact inverse reconstruction proves that this fixture adaptation changes no guard or function body. Registration tests reuse that already licensed constructor before its tests and exercise real issuer/Git/shape/binding functions, restored positives and separately identified guard-removal mutants. Synthetic unsealed entry/terminal results remain `ABSENT`; they supply no accepted package or native evidence.

The immutable runtime remains R4/U. SourceBase remains M, with112 product keys,26 unchanged child commands,24 D-ids and9+3+7 duties. This preparation updates only changed tooling pins and its measured tooling test total. The exact execution results and hashes belong in `../BUILD-B1B2-AMENDMENT-BINDING.md`; previous267 remains the R4 result at its recorded head. Final shared assembly, affected consumer proof, native comparison and individual native-field expectation admissions remain separate. `nativeFieldDeltas` is still unmeasured; source94/writers335 and native/frozen/family/census/H3/second/FULL, actual artifact/receipt/reverify/currentCI and phone/history requirements remain held.
