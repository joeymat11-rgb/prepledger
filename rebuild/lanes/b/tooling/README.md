...
Metadata authoring exposed a retained historical blocker: `validateSuccessor` requires R to be an ancestor of S, while the complete Git graph reports their exact common base as M. The original ancestry assertion and its refusal remain unchanged pending a named PM ruling. A proposed exact common-base assertion was evaluated transiently for metadata reconstruction and reverted; it is not adopted evidence. Independent T/U Git-delta, literal forward/inverse and current public disk/HEAD/U checks pass without changing that acceptance predicate. No source-family, native/frozen, census, H3, private/FULL, package CLI, workflow, receipt, integration or phone execution is claimed here. The seed pin remains metadata only. All work is local and unpushed.
