...
## September 11 candidate: exact B-NTC successors and child authority anchors

This amendment supersedes the earlier statement that every authorization must
exist at the parent receipt base. Parent owner/contract citations and parent
acceptance remain anchored exactly there. Only child theme and brief citations
may add `commit`, a full immutable 40-character Git SHA. It must descend from
the parent receipt base and be an ancestor of both HEAD and the fixed
`refs/remotes/origin/rebuild/t2-client-core`. Role remains `cowork`; no arbitrary
path or role override exists. Omitted commit retains the old parent-base rule.
The final exact-artifact receipt and all existing accepted-chain checks remain
required. A local PM ledger commit alone cannot qualify as a chain anchor.

`b-ntc-successors.json` is a source definition, not an approval. The runner pins
its exact SHA, reads it from disk and HEAD, and limits recognition to the exact
B-NTC parent, nine inherited gate/name pairs, fifteen child declarations, product
map and complete child/parent source closure at the named builder commit. Both
original and current source pins must hold in Git and on disk. Actual execution
still comes solely from `children()` and its validated Node target list. The
unchanged original thirteen profile-refusal tests and four actual-child controls
remain distinct declared children. Archived parent acceptance is not child
acceptance. Source/needle-only wrappers cannot substitute for the pinned files.
...
The policy is pinned again in a proposed successor artifact. `coverage.moves`
stays `{}` and `MOVES_RULING` stays null in every package; other packages receive
no successor exemption. Changing this candidate mapping or source closure requires
a new reviewed policy and exact PM disposition.

Focused controls: `node --test rebuild/lanes/b/tooling/test/successor-authority.test.cjs`.
They create public-only isolated Git fixtures and use the existing Node runtime.
For the real successor child, existing dependencies are linked from
`B_NTC_SOURCE_ROOT` (default: sibling worktree `astra-b-ntc`); no install occurs.
The fixture does not construct a package acceptance artifact or receipt. It tests
component source/authority/execution behavior, not a complete CI/FULL verdict.
