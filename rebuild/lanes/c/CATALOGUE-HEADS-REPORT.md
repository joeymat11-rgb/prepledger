...
## THE ONE FILE OUTSIDE THE DISPATCH'S CUSTODY LINE, AND WHY

The dispatch named `exercise-catalogue.mjs` + its test only. One more line was unavoidable: `starter-week.mjs:145` copied each credit as `{mg, lend}`, and `setup.test.mjs` S32 already required the proposed tags to **equal the catalogue entry's credits**. Narrowing the copy would have dropped the head the catalogue had just resolved and turned that existing cell RED (measured: setup 156/1). The copy is now `({ ...s })` - eight lines with their comment, no new promise, and it keeps S32's assertion exactly as it was written. Everything else is inside custody; no engine, client, m4, w6, conform or `.github` byte is touched.
...
catalogue **55** (43 + **12** new cells, >= 12 asked) / setup **157 unmoved** / today step **164** / copy **36** / gym **64** / check-in **28** / W6 **552**, all 0 fail.
