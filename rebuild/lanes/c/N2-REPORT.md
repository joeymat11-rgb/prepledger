...
## D2 ROUND 2 - SIX FINDINGS, ANSWERED

FINAL REJECT at `7d2fdab` (`rebuild/lanes/d2/reviews/N2-REVIEW.md` + annex @ `c25bfcb`). D2's annex is executable, so it was run first: all seven of its defect probes assert the WRONG outcome, and all seven now fail, which is the reproduction and the proof in one. Base is the tip carrying the :185 test-hygiene merge; nothing conflicted.

| # | Finding | Fix | Cell |
|---|---|---|---|
| 1 | the stale-correction precondition was checked OUTSIDE the transaction, so two concurrent corrections both committed | the expectation now TRAVELS with the write (`payload.supersedes`) and is enforced by the producer's own `validate`, which the accepted client calls synchronously on the envelope it has built, immediately before the one transaction that writes it (`rebuild/client/index.cjs:228-233`). Its reader is by op id, and on this device an op id is `op-<device>-<sequence>`, so `currentNightOp` walks every earlier operation this device wrote and answers from the generation being committed. The second of two concurrent corrections finds the first already accepted and is refused with nothing written | `N2-01 - D2 round 2 finding 1: two concurrent corrections, and the loser refuses AT THE COMMIT` |
| 2 | a real boot with a stored night, and a save completing after Back, both left the real gym projection stale | the workout entry is rebuilt when the sleep lane attaches at boot (if it holds anything), and after a late save - because a committed night is a fact about the DEVICE: ownership governs painting and navigation, never the record. The rebuild refuses to swap in a different installation (athlete, namespace, database and lease must match) and carries the gym draft | `N2-09 - D2 round 2 finding 2: a stored night at boot, and a late save, both reach the REAL gym host` |
| 3 | a second visit to the check-in restored the ORIGINAL draft and lost the edits made in the replacement | the rebound model is RETAINED as the active check-in; a later visit with nothing changed reopens that same sheet, and a later rebind carries from it. The carry is one-shot per rebind | `N2-07 - D2 round 2 finding 3: a second visit to the check-in keeps what was typed on the first` |
| 4 | the real A3 stores `{value, unit}`, the offer required a number, so `Use these hours` was hidden for every real check-in | both shapes are read; the offer is restricted to the check-in for the morning AFTER this night and to an `entered` answer; the saved night carries that op as its authenticated source | `N2-08 - D2 round 2 finding 4: the REAL check-in hours are offered, and the saved night cites that op` |
| 5 | an 8-to-5 correction with a failed read displayed 8; a lost acknowledgment with a failed reconciliation said "Nothing was recorded" | the acknowledgment OUTRANKS the stale projected row (and the stale operation's shape and stamp are not claimed with it); reconciliation matches the night that was ATTEMPTED rather than any new last op; and when the reconciling read itself fails the outcome stays UNKNOWN - "Checking whether sleep was saved.", the save fenced against a duplicate, the read offered - because "nothing was recorded" is a claim only the log may make | `N2-05 - D2 round 2 finding 5: a committed correction is shown, and an unknown outcome is never called nothing`; `N2-05 - ... a lost acknowledgment with an unreadable log stays UNKNOWN` |
| 6 | the actual coach reported 8 h after a 1 h night, even after reopen | the companion is written, as :167 (2) provides for once the row cannot pass without it. `local-world.mjs` opens the sleep lane on the COACH'S OWN era and replays it into `today` before the gym host, the check-in and every tool are composed from that state; the check-in's night is read when it is ASKED for, through A3's own `sleepNightFor` over the current state, rather than frozen at world-open. `sleepOnLocalEra` is false and the coach has no night to state when the lane cannot open | `N2-12 - D2 round 2 finding 6: the REAL coach reports the night this device recorded` |

**D2's own annex, re-run at this head:** R2-1 through R2-7 all report FIXED (`{acknowledged:[true,false], codes:[null,"SLEEP_STALE_NIGHT"], ops:2, winner:8}`; `useHidden:false`; only `Updated synthetic detail` survives; `5 h Save time not recorded.`; `Checking whether sleep was saved.` with the save fenced; `rebuilt:true, gymHasNight:true`; boot `gymHasNight:true`). Their commit-abort boundary still refuses with ops and outbox unchanged, and their two actual `today_checkin` calls now report 1 h after save AND after reopen.

**A PAGE-BREAKING DEFECT ON THE TIP, FOUND AND FIXED HERE.** `plain-copy.cjs` is bundled into the shipped page and, since the :185 test-hygiene merge, read `__dirname` at module scope. In a browser that is a ReferenceError before the first paint: **the built page did not boot at all** (msedge: `pageerror: __dirname is not defined`). No suite could see it, because the guard that uses the value runs only at build time. The derived spelling is kept for the build (it is what lets the dash guard be proved RED against a copy) and a fallback is used where `__dirname` does not exist. This is in today/** under lane C's standing licence, is not N2's own file, and is called out here because it affects the page for every lane.

**Counts at this head.** whole today directory in ONE glob invocation **597 pass, 0 fail, run twice**; sleep alone **50** (43 -> 50); coach **201**; W6 **552** (journey 51); host 23; B-NTC gate ALONE exit 0 with the tracked tree clean (H3 has not merged on this tip, so the gate is still B-NTC); `build.mjs` PASS, 113 inputs.

**Mutants: 28 written, 27 killed.** M04 - removing the HOST's pre-read stale check - SURVIVES and is disclosed as EQUIVALENT: with the enforcement now inside the commit, the producer refuses the envelope and the host's post-refusal read still names it `SLEEP_STALE_NIGHT`, so no observable behaviour changes. That is precisely what finding 1 asked for: the pre-read is no longer what enforces anything.

**Browser check** (msedge, extended): **4 real `taskkill /F /T` kills**, each verified dead; the same-page check-in; the gym return path walked for real; and a fresh boot on a device that ALREADY HOLDS a night (Today -> `Resume UPPER BODY` -> Today, truthful). The repeated-visit check-in journey could not be driven there - at that state the sheet shows no visible free-text box - so the check says so in its own output and that journey is proved over the real check-in entry in `N2-07` and by D2's own R2-3 probe.

**Still not proved:** a process kill between commit and acknowledgment, clock rollover in a browser, a physical iPhone, and Start/set bytes across the rebind beyond the draft (they are durable and in the log, which is why they are not carried). N2-11 still asserts the pre-H3 `model.read()` throw; H3 has not merged on this tip.
