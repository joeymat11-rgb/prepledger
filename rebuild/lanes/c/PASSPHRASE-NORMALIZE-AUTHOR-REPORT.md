...
The fix is one pure helper, stated once and read by both decoders:
    ...
    rebuild/m3/setup/port/passphrase.cjs  normalisePassphrase(typed)
      NFKD  ->  lower case  ->  every run of [ECMAScript \s (U+FEFF in,
      U+0085 out), hyphen-minus, U+2010 to U+2015, U+2212, underscore,
      comma, full stop] becomes ONE
...
* **0 words contain a separator character** (the whole class: ECMAScript \s,
  including U+FEFF and excluding U+0085, `-`, U+2010 to U+2015, U+2212, `_`,
  `,`, `.`). So the canonical
...
probe words; the WHOLE rendered text of the refused page is equal, character
for character, and so is the refusal record, for the two wrong phrases tested.
C-PN-29 also measures equal refusal textContent and refusal
records for a wrong phrase and a malformed phrase. The passphrase box on each
of the two refused pages still holds exactly the string typed into it,
inherited from the base. The input values
