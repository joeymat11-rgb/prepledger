...
Custody: 26 changed files vs `964f183`, **zero outside `rebuild/coach/**` and `rebuild/lanes/c/**`** (the two I flagged in round 1, `C6-INDEX.md` and `REPORT-A-PROBLEM-BRIEF.md`, are inside `lanes/c` and now explicitly in scope). Coach **201/201** (wave1-demo **28**, parity **31**, machine-settings 23, C5 65); setup **157**, catalogue **43**, `build.mjs` **PASS**. **C1 CLOSED**: I enumerated all 19 advertised tools and drove each through `dispatch` - **19 served, 0 `NOT_IN_LIST`**, every result's tier equal to its declared tier, and the unknown-name refusal still names the attempt and now lists all 19 (`__proto__`, `constructor`, `hasOwnProperty` included). No tool writes `sets`, `hi` or a day kind: 0 such keys in the module with comments stripped. **C3 CLOSED** (65), **C4 CLOSED** (28, off the floor). **The save-path fix is real and I verified it against the merged screens myself**: `setup-app.mjs:524` calls `onDone({setup, tags})` and `today-entry.mjs:141` forwards that one argument to `host.save()`, while `setup-commands.mjs:87 envelopeOf` accepts BOTH spellings - so byte equality alone could not have caught a coach left on the old two-argument call, which is exactly why this needed a deliberate fix. My own driver: **6/6** fixtures give `argc=1` with keys exactly `["setup","tags"]` equal to the screens' envelope, and **6/6** stored ops byte-equal against the screens' one-argument save. **The CI residual I carried through four rounds is withdrawn**: `.github/workflows/rebuild.yml:111` now runs `node --test "rebuild/coach/test/*.test.cjs"`, and `:82` the B-NTC gate. Only round-1 condition C2 remains, and it is closed too (annex refreshed).
**B-NTC gate, diagnostic for lane B, not a condition here** (`rebuild/coach` touches nothing it audits): run ALONE on a clean tree, it **PASSES**. Terminal line: `B PACKAGE B-NTC PUBLIC CI EVIDENCE PASS - public evidence only, NOT the package verdict; the 19 original gates, the private oracle and independent exact-artifact acceptance remain separate, and POSTFIX PACKAGE PASS is unavailable on this mode at any time`. The builder's `AUDIT RED-FIRST FAIL ... 97/104 mutant executions DETECTED` is an **interior LAWS-accounting line**, not the verdict; I reproduced it identically on three runs and the run still ends PASS, so his worktree and the integrator's agree and only the reading differed. On the stale-stat question: `rebuild/engine/*.cjs` really does show modified **during** the run (`git diff --numstat` gives a genuine `1 1` on `today.cjs`, `progression.cjs`, `acceptance-step-efficacy.json` at different moments) because the gate writes mutants and restores them; **after** completion `git status` is clean and `git diff` empty. Real bytes, transient, not a stale stat.
