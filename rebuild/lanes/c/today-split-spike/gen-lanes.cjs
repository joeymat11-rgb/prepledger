...
const HOOKS = [
  ...
  ["WRITE", "/* THE TWO GUARDED ENTRIES (spec E.6). The subject list is REACH.md's rule -"],
  ["WRITE", "   guarded if and only if it reaches a durable PUT and no paint root reaches it -"],
  ["WRITE", "   and in THIS file that is exactly two: recordIntake reaches foodLane.save and"],
  ["WRITE", "   recordSleep reaches sleepLane.save. E.6's other seven subjects are not this"],
  ["WRITE", "   round's: five are gym-app.mjs's, and submitWeighIn and recoverWorkout are"],
  ["WRITE", "   released seams that no sealed guard can reach. The build report says so. */"],
  ["WRITE", "recordIntake:      gesture(\"recordIntake\", (save, cal, pro, error) => { foodSaving = recordIntake(save, cal, pro, error); return foodSaving; }),"],
  ["WRITE", "retrySleepRead:    () => { sleepSaving = retrySleepRead(); return sleepSaving; },"],
  ["WRITE", "recordSleep:       gesture(\"recordSleep\", (map) => { sleepSaving = recordSleep(map); return sleepSaving; }),"],
  ...
  ["WRITE", "bindSleepDraft:    (draft) => { sleepDraftHeld = draft; },"],
  ["WRITE", "/* E.6's shim. Every listener the released view installs comes through here, so"],
  ["WRITE", "   the two entries above can tell a gesture from a paint. The fence asserts that"],
  ["WRITE", "   the released file holds ZERO remaining addEventListener, which is what makes a"],
  ["WRITE", "   listener this build missed a RED ROW rather than a silent hole. */"],
  ["WRITE", "listen:   (el, type, fn) => el.addEventListener(type, wrapFor(type, fn)),"],
  ["WRITE", "unlisten: (el, type, fn) => el.removeEventListener(type, wrapFor(type, fn)),"],
];
