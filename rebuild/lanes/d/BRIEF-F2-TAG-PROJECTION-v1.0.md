# BRIEF-F2-TAG-PROJECTION v1.0

Status: BRIEF-READY for PM acceptance by name; implementation custody and semantics ruled at :155. Lane D does not author acceptance lines.
...
Authority: DECISIONS:148(3b), :155 (snapshot/credits/unresolved policy/custody), :159 (candidate pre-build), :163/:164 (F1 predecessor brief/token); A4b seam: :149.
...
1. Add a pure, constructor-independent module `m4/workout/setup-tags.cjs`. Proposed factory: `createSetupTagProjector({taxonomy:{muscles:ENGINE_MG,regions:REGION_MG}})`, returning `validateSetupTags(setup,tags)` and `projectSetupTags(state,{setup,tags,op_id,date})`. C binds one frozen taxonomy from its approved catalogue muscle/region exports, not from a setup operation or a duplicate authored label list. The caller first invokes the existing `createCleanInitState({setup})`; the projector returns a fresh frozen state. Keep the original closed setup and exercise contracts unchanged.
...
4. Project `head`, a cloned `secondary` list, and a runtime-only provenance marker `volumeTags:{profile:'earned/setup-volume-tags/v1',op_id,date,regionsByMuscle}` onto each corresponding engine exercise. These names/profile are DECLARED implementation vocabulary, not stored schema. regionsByMuscle is derived from C's injected REGION_MG mapping for split muscles only; it is runtime metadata so the engine can budget coarse helpers without importing or duplicating the catalogue. No other exercise field changes. Preserve ids, families, order, sets, loads, forks, native carriers, readings, sessionLog and every operation/outbox byte.
...
Quarter-credit and the catalogue's anatomy assignments become numerically active for the first time. They remain declared programming estimates, not newly validated science. This brief proposes consuming the athlete's saved values verbatim after validation. PM ruled that activation and unresolved-region policy at :155; Lane D supplies no new physiological claim. A later catalogue correction changes future setup snapshots only.
...
PM handoff: accept v1.0 by name and supply this package's own gate-supersession token under :153/:155(5). Current evidence is in F2-CANDIDATE-REPORT.md; C companion, cumulative FULL/private run and real phone journey remain outstanding. F1/F2 pre-build on B1+B2 begins when B names the branch; final seal order is unchanged. No D citation/ruling/acceptance/receipt line.

Implementation disclosure: first projection enriches an empty constructor basis only; same-snapshot re-projection may preserve later observations, but imported pre-setup history or conflicting/mixed provenance refuses. Null/absent tags return the original state. New coach-context qualification copy ("region unspecified; no regional band or proposal") is INVENTED UI wording, not a new evidence claim.
