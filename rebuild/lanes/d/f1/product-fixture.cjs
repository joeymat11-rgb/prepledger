'use strict';
// PUBLIC SYNTHETIC F1 inputs. Every identity, date, exercise and number here is
...
function loadProduct(ref = process.env.F1_SOURCE_REF || null) {
  if (ref !== null && ref !== BASE) throw new Error('F1_BASE_REF_NOT_ALLOWLISTED');
  const cache = new Map();
  function load(rel) {
    if (!ALLOWED.has(rel)) throw new Error('F1_SOURCE_NOT_ALLOWLISTED');
    if (cache.has(rel)) return cache.get(rel).exports;
    const filename = path.join(ROOT, rel);
    const source = ref === null ? fs.readFileSync(filename, 'utf8') :
  ...
  const fail = () => { throw new Error('F1_UNAVAILABLE_PROVIDER_REACHED'); };
...
function setup(training = { 1: 'U', 4: 'L' }, exercises) {
  return { athlete_label: 'synthetic-f1-public-proof',
    split: { from: '2026-09-07', map: map(training) },
    exercises: exercises || [exercise('f1-upper', 'U', 3, 'chest'), exercise('f1-lower', 'L', 2, 'quads')],
...
function orderedState(product) {
  const s = state(product, { 1: 'F', 3: 'U', 4: 'F', 6: 'L' }, [
    exercise('f1-u-a', 'U', 3, 'chest'), exercise('f1-l-a', 'L', 2, 'quads'),
    exercise('f1-u-b', 'U', 2, 'back'), exercise('f1-l-b', 'L', 4, 'hams'),
    exercise('f1-u-tail', 'U', 1, 'delts'),
  ]);
  s.exOrder = { U: ['f1-u-b', 'f1-u-a'], L: ['f1-l-b', 'f1-l-a'] };
  return s;
}
