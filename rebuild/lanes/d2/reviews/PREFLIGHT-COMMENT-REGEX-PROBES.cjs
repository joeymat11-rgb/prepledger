...
const cases = [
  ...
  ['script-closing-attribute', 'html', '<script>/* comment </script data-x><p>—</p> */</script>', 'MUST-REFUSE-UI'],
  ['style-closing-attribute', 'html', '<style>/* comment </style data-x><p>—</p> */</style>', 'MUST-REFUSE-UI'],
  ['style-closing-solidus', 'html', '<style>/* comment </style/><p>—</p> */</style>', 'MUST-REFUSE-UI'],
];
const outcomes = [];
try {
  ...
  for (const [name, extension, source, expected] of cases) {
    ...
    if (expected !== 'MUST-REFUSE-UI') assert.equal(line, expected, name);
    outcomes.push({ name, expected, actual: line });
    fs.unlinkSync(path.join(scratch, file));
  }
  const defects = outcomes.filter(x => x.expected === 'MUST-REFUSE-UI' && x.actual === 'PREFLIGHT FAIL CI-UNVERIFIED');
  assert.equal(defects.length, 3, 'all three R1 omission witnesses reproduce on the named head');
  console.log(JSON.stringify({ case: 'D2-PREFLIGHT-BOUNDARIES', controls: 10, defectObservations: defects, outcomes }));
} finally {
