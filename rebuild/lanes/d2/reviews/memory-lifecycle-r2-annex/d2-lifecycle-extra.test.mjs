...
test('D2 independent entry edits do not prevent confirmed settings Save clearing its unchanged editor',async()=>{
 ...
 try{
  ...
  fs.writeFileSync('.tmp/d2-lifecycle-race-settings.json',JSON.stringify({editor:p.draft.settingsEditor,entry:p.draft.entry,effort:p.draft.effort?.label,counts:p.mounted.refreshState().counts,opsBefore:Object.keys(before.ops).length,opsAfter:Object.keys((await collections(kit)).ops).length},null,2)+'\n');
...
test('D2 independent editor edits do not hide an acknowledged set log or suppress successful-log clearing',async()=>{
 ...
 try{
  ...
  fs.writeFileSync('.tmp/d2-lifecycle-race-log.json',JSON.stringify({modelPhase:'saved',visibleUndo:!!p.phone.querySelector('[data-action="undo"]'),visibleLog:!!p.phone.querySelector('[data-slot="log"]'),changedCalls:changed,entry:p.draft.entry,effort:p.draft.effort?.label,editor:p.draft.settingsEditor,counts:p.mounted.refreshState().counts,opsBefore:Object.keys(before.ops).length,opsAfter:Object.keys((await collections(kit)).ops).length},null,2)+'\n');
