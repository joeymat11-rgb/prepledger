...
test('D2 R2 editor row addition preserves the real submitted set payload and acknowledges it exactly once',async()=>{
  ...
  try{
    ...
    const before=await collections(kit);p.click('[data-slot="log"]');await pause.reached;const snapshot=p.mounted.refreshState();
...
test('D2 R2 actual set-write refusal remains visible after the independent editor repaints the active card',async()=>{
  const kit=await device(),pause=gate();let p,result;
  try{
    assert.equal((await kit.model.start()).ok,true);
    const model=createGymModel({gymHost:wrap(kit.gymHost,'executeResumedWorkout',async(input,original)=>{pause.enter();await pause.wait;result=await original();return result;})});
    ...
    kit.fault.state.mode='quota';kit.fault.state.armed=true;pause.release();await until(()=>p.mounted.refreshState().counts.busy===0,'actual refused set write');kit.fault.state.armed=false;
    const after=await collections(kit);preserved(before,after,0);assert.notEqual(result.acknowledged,true);
    const currentError=p.phone.querySelector('#gym-error').textContent;
    const modelRead=await model.read();
    fs.writeFileSync('.tmp/d2-lifecycle-r2-refused-set.json',JSON.stringify({result,phase:modelRead.phase,message:modelRead.message,currentError,detachedError:oldError.textContent,currentLogVisible:!!p.phone.querySelector('[data-slot="log"]'),counts:p.mounted.refreshState().counts,opsBefore:Object.keys(before.ops).length,opsAfter:Object.keys(after.ops).length},null,2)+'\n');
    assert.ok(currentError.trim(),'the current owning card must disclose that its actual set write failed');
    assert.equal(p.draft.settingsEditor.draft.rows.length,2);assert(p.draft.effort);
