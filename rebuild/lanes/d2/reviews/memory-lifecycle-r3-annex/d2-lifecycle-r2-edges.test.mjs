...
test('D2 R2 actual pending set outcome reaches the current owning card after an independent editor repaint',async()=>{
  const kit=await device(),pause=gate(),observation={preparations:[],execute:null};let p,result,changed=0;
  try{
    assert.equal((await kit.model.start()).ok,true);
    ...
    const model=createGymModel({gymHost:wrap(observed,'executeResumedWorkout',async(input,original)=>{observation.execute=input.resumeId;kit.fault.state.mode='quota';kit.fault.state.armed=true;try{result=await original();}finally{kit.fault.state.armed=false;}pause.enter();await pause.wait;return result;})});
    p=await page(kit,{model,onChanged:()=>changed++});await editor(p);p.input('#gym-weight','35');p.input('#gym-reps','7');p.choose();const before=await collections(kit),oldError=p.phone.querySelector('#gym-error'),entry={...p.draft.entry},effort=p.draft.effort;
    ...
    // Delay only delivery of an already completed REAL storage-refusal result.
    // The real client command has run; this does not reorder its enqueue behind
    // a new continuation preparation or synthesize a refusal result.
    ...
    fs.writeFileSync('.tmp/d2-lifecycle-r2-refused-set.json',JSON.stringify({result,phase:modelRead.phase,message:modelRead.message,currentError,detachedError:oldError.textContent,currentLogVisible:!!p.phone.querySelector('[data-slot="log"]'),currentOwns:p.mounted.refreshState().owns,entry:p.draft.entry,effort:p.draft.effort,editorRows:p.draft.settingsEditor.draft.rows,changed,counts:p.mounted.refreshState().counts,opsBefore:Object.keys(before.ops).length,opsAfter:Object.keys(after.ops).length,outboxBefore:Object.keys(before.outbox).length,outboxAfter:Object.keys(after.outbox).length,observation},null,2)+'\n');
    assert.equal(p.draft.settingsEditor.draft.rows.length,2);assert.equal(p.mounted.refreshState().owns,true);
    if(result.acknowledged===true){
      await until(()=>changed===1&&settled(p),'successful outcome');preserved(before,after,1);assert(p.phone.querySelector('[data-action="undo"]'));
    }else{
      preserved(before,after,0);assert.deepEqual(p.draft.entry,entry);assert.equal(p.draft.effort,effort);assert.equal(changed,0);assert(result.code);
      assert(currentError.includes(result.code),'the actual refusal must be visible on the current owning card, not only its detached predecessor');
    }
