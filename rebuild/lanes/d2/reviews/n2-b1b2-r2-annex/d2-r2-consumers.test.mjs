...
for (const [label, night, expectedClean, band, score] of [
  ...
  // PM246/R7 changes only this unavailable-target contribution expectation.
  ...
  try {
    ...
    assert.deepEqual(E.fiveLevers(state).sleep,
      { label: 'SLEEP', state: 'quiet', detail: night?.date === NIGHT ? 'sleep target not recorded' : 'current sleep not recorded' });
