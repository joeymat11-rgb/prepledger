...
const problems=[],faviconDiagnostics=[],cases=[],killProof=[];let context=null;
...
async function open(profile,configuration,width=390) {
  ...
  page.on('console',m=>{if(m.type()!=='error')return;const where=m.location()?.url||'';if(where===url+'favicon.ico'&&/404/.test(m.text()))faviconDiagnostics.push({message:m.text(),url:where});else problems.push({kind:'console',message:m.text(),url:where});});
...
async function kill(profile) {
  ...
  for(const pid of pids){try{execFileSync('taskkill.exe',['/F','/T','/PID',String(pid)],{cwd:root,stdio:'ignore',timeout:30000});}catch{/* A child can already be gone with its verified parent. Check all remaining processes below. */}}
...
try {
  ...
  fs.writeFileSync('.tmp/d2-r3-browser-primary-error.txt',String(error.stack||error)+'\n');
  throw error;
} finally {
  if(context){try{await context.close();}catch(error){console.log('Browser cleanup: '+error.message);}}await new Promise(resolve=>server.close(resolve));
