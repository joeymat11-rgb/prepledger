...
const allowed=new Set(JSON.parse(fs.readFileSync('../review-n2-b1b2-r2/.tmp/d2-combined-closure.json')).files.map(f=>f.path));
const bad=[],seen=new Set();
const guard=absolute=>{const file=rel(absolute);if(file.startsWith('../')||deny.some(re=>re.test(file))||!allowed.has(file)){bad.push(file);throw new Error('UNLICENSED_DEPENDENCY_BEFORE_READ '+file);}return file;};
