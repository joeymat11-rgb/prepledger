...
function fullBasis(target,rows=[]) {
  ...
  // owedLedger asks for the three exact preceding DATES, not a row count.
  // A date-only current basis row deliberately has NO observed hours. Real N2
  // saves below overlay it; missing-hour controls leave it unknown. The two
  // initial fixture attempts stopped at this prerequisite and remain uncredited.
  const dated=new Map([{d:'2030-02-01',h:8},{d:'2030-02-02',h:8},{d:NIGHT},...rows].map(r=>[r.d,r]));
  input.sleep.nights=[...dated.values()].sort((a,b)=>a.d.localeCompare(b.d));
