...
| `quality/teeth.py` | the executable mutation list: 92 rows, each one change applied to a scratch copy and run through the scratch copy's own gate, asserted to fail for the stated reason (or, for the honest controls, asserted NOT to fail the named check); the line under its head line says how many rows were enumerated, how many ran and how many this platform did not run | what proves the two gates can still refuse |
...
the exact refusal. Ninety two rows: every row of the audit's mutation table, plus a dropped RIR
...
minus sign doing a dash's job, a spaced hyphen whose spaces are a raw TAB in an attribute value
and an ordinary space, a visible target behind an inert clip, and a minus sign pressed against a
letter), and u1 to u9, v1 to v24, w1 to w11 and y1 and y2 are the second teeth audit's one list.
...
visibility hidden, and a right edge moved 6 px by two 3 px moves. v1 to v24: fixed copy and a
...
`teeth.py` itself, and the phone sheet asked for a state whose apply throws, for one that
applies, for one whose apply throws in one theme only, for one whose applied marker names
another state and for one that raises an error behind a correct marker. w1 to w11 are the minus
sign and spaced hyphen clauses (the honest openers, a comma, a TAB, a line feed and a carriage
return between two signed numbers, two real cells, the sign alone on its line, the no break
space, and the two ranges, with and without a space), each clause with a row of its own and with
...
- No dashes, no readiness words, no vendor names. Both gates sweep one string: the active
  ...
  stands at the start of a line or a cell, and the nearest character in front of the sign which is
  not a space is not a digit (a digit across a space is a range); or it is the whole of its own line or cell in the swept string,
