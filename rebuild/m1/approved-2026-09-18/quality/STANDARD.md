...
pack's `app/` when that is what was measured, and a plain statement that an `EARNED_APP` build is
external and not digested here), and the versions that drew it. A green report that does not name
the build it measured is not evidence about that build, and the later tickets are judged with
`EARNED_APP` set.
...
| pressed state on every tappable surface | R | the three surfaces per screen named in gate.py's PRESSABLE, which is a named sample and not every tappable surface. The pointer is moved on and left to settle before the idle picture, so a hover style with no pressed style of its own FAILs. An element outside the viewport is a FAIL line, never a crash |
...
both margins, contrast in the same two tiers, and the primary it finds staying wholly inside every edge of the viewport, compared unrounded. A state whose primary is on the page and not drawn is not judged here: 29 of the 209 states draw a panel over the screen and leave the screen's primary in the markup behind it, so which states owe a drawn primary is a property of the state and the driver has to declare it, which is C-UI-GATES-2. It is then
