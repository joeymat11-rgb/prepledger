...
- **One spacing scale.** Gaps between blocks are 8 (inside a group), 12 (between related blocks), 14 to 16 (around the primary action), 20 to 24 (between sections). Small structural gaps of 4, 5, 9 and 10 exist in the header and title areas only. Anything else is flagged. The check measures the gaps between the children of one container, the scrolling body's own children and the fixed stack's own children, so a gap inside a nested card is a rule the eye holds and this check does not measure; widening it to every block is C-UI-GATES-2. *gate (gaps on the spacing scale, the one check besides the type scale that may WARN)*
...
- Sans weights 430 to 500 only. Titles in row cards and pill labels are 500; body 450 to 480; sub lines 430. The current automated admitted-weight set also includes 400, so review holds the narrower design rule until C-UI-GATES-2 adds the missing witness. *gate (type sizes and weights on the scale, which WARNs on a new weight); reviewer (the 430 floor)*
...
- Text nobody can see is not text. A line leaves the state record, and is not measured for contrast, when it is `display: none`, `visibility: hidden`, at an effective `opacity` of 0 (walked up its ancestors), drawn with no area or wholly outside the viewport, clipped to nothing by `clip-path: inset()` (read up to its `round`, because the radii after that word are a corner rounding and inset nothing), or carried off its own box by a `text-indent` at or below minus 1000 px on a block container (an indent does not move an inline box's own text, so it is not read there). Hiding a line then reads as removing it and the sheet says the visible text changed. The list names the mechanisms the gates check, not every way a line can be hidden; it grew each time a reviewer built one that was missing, and it grows the same way again.
- Contrast: primary text 4.5:1 or better against what is actually behind it; muted labels 3:1 or better. Measured on the rendered screenshot, not on the token. The lower tier is the pack's own `--muted` or `--faint` token wherever it is painted, the `--gold` token where the marker rule below gives it, a disabled control's label (section 10 calls it a muted label), text at 24 px or larger, or one of the classes the stylesheets paint muted AND painted with one of the quiet tokens those stylesheets give those classes, which are `--muted`, `--faint` and `--text-soft` and are listed with their line numbers in `quality/common.py`. A CLASS NAME ALONE NEVER LOWERS THE TIER: primary copy relabelled with a class off that list, and painted any colour at all, stays at 4.5:1. It is not "the colour differs from the body colour" either. *gate (contrast (measured behind the text), on both gates, at all three sizes)*
- Only two radii: 14 px for cards, buttons and chips; full round only for pills (the header pills, Save, Use 105 / Keep 115 and the Talk pill, as the boards draw them). The gate samples the top-left corner of its named card, button and chip classes; no pill is measured, so review holds the full-round half. If the code wants a third radius, the standard changes on the record. *gate (radii: 14 px for cards, buttons and chips; full round only for pills); eye (pills)*
...
- Every tappable surface has a pressed state: an instant change while the finger is down (deeper cream, lifted card, gold edge). The pointer is moved onto the surface and left to settle BEFORE the idle picture is taken, so a `:hover` style is not read as a pressed state; a phone has no hover at all. Measured on the approved app: zero `:hover` rules and twenty `:active` rules. The gate samples eight named surfaces: three on Today, three on Workout and two on Coach. *gate (pressed state on every tappable surface, over gate.py's PRESSABLE sample)*
...
forbidden change at a time to a scratch copy and asserts the exact refusal, in 102 rows. The line under its head line says how many rows it ENUMERATED, how many it RAN and how many this platform did not run, and names them, so no reader has to take "rows" for "rows proved here". A row that this machine cannot build, because the change does nothing here, prints the reason in words and counts as expected; a VOID row, whose anchor did not match, is never a pass. `--only` refuses in one line and exits 2 when it names an id the list does not carry, when it names one it carries and one it does not, and when it names nothing at all; a selection that names nothing used to run every row and report it as if it had been asked for.
...
Every ordinary report of all three scripts names, under its first line: its mode (ordinary, or
which accept), the scope it was narrowed to, the build under test (the URL, with a digest of the
pack's `app/` when that is what was measured, or the caller-supplied immutable build digest from
`EARNED_APP_DIGEST` when `EARNED_APP` transfers custody outside this pack), and the versions that
drew it. The external digest is labelled caller-supplied, not computed or verified by this pack.
An external run without a 64-digit SHA-256 identity is refused. A green report that does not name
the build it measured is not evidence about that build.

`gate.py`, 34 check names, 372 result rows on a green full run. Thirty three of them hold a rule; the thirty fourth is the diagnostic that catches a check which threw, and on a green run it is never emitted at all, which is why the run is 372 rows and not 373. R means the reference size 393x852 only; 3 means all three
of 393x852, 375x812 and 360x780. Every row FAILs unless it says WARN.

| check | sizes | what it holds |
|---|---|---|
| primary action in first viewport | 3 | the screen's primary is on the page, is drawn by the shared visibility test, and sits wholly inside every edge of the viewport, compared unrounded. A primary that is not on the page is named as missing; one that is on the page and not drawn is named as not drawn |
...
| touch targets >= 44 px | 3 | every tappable surface, hit area included: the focusable elements, the control roles, `label[for]`, and the classes the pack's own tap highlight rule declares tappable (app/app.css:625, listed in `quality/common.py`). A box that is positioned absolute or fixed AND clipped to nothing, which is how the pack hides an assistive label, is not a target; any other small box is, a box carrying a clip its positioning makes inert included. A box of zero width is skipped. This walk uses rendered layout plus that narrow clip exclusion, not the shared text-visibility test, so opacity-zero and visibility-hidden hit areas remain conservative failures. The side that failed prints two decimals |
| copy: no dashes, readiness words, vendor names | 3 | the owner's standing rules, word boundary matched, over the screen's text plus every visible placeholder, assistive label, tooltip, alternative text, filled in value and quoted string in ::before or ::after. A dash is every character of Unicode category Pd except the plain hyphen, plus U+2043 and U+2053, which Unicode files under Po; U+2212 unless it is the sign of a negative number or it is the whole of its own line or cell, which is a control whose entire label is the sign. It is the sign of a number when a digit follows it directly, AND the character directly in front of it is one of a closed list of openers, or it stands at the start of a line or a cell, AND the nearest character in front of the sign which is not a space is not a digit, because a digit across a space makes the pair a range. The openers are: a space (after the fold), an opening bracket, brace or quotation mark (Unicode's Ps and Pi, plus the two ASCII quotes), a colon, a slash, an equals sign, the multiplication sign, and a currency symbol (Unicode's Sc). Everything else directly in front of the sign is a dash and fails: a letter (a word), a digit (a range), a comma, a full stop. For this rule alone a TAB separates cells. The DOM sweep adds that boundary only between adjacent numeric elements that markup whitespace separates; "3" and "\u22125" in those separate numeric spans are two values and pass. An element boundary inside a word, adjacent spans without separating whitespace, and "3 \u22125" in one normal-flow text node remain one rendered phrase and fail. A source-formatting newline collapsed by normal-flow HTML remains a space, while literal TAB/LF/CR in a rendered attribute or preformatted cell retain their rendered boundary. A hyphen with a space each side, read after every character Python's str.isspace() calls whitespace, except the newlines that separate the swept string's lines, plus U+2060, has been folded to an ordinary space (the TAB folds here, so a TAB beside a hyphen is a spaced hyphen); a character of category Cf is a problem of its own, named by its code point, and the word and vendor sweeps read the flat string with those removed, NFKC normalised and casefolded, so splitting a forbidden word across elements does not bypass it |
...
| RIR chips are the five locked values | 3 | 0, 1, 2, 3+, unsure in order, labels 0, 1, 2, 3+, Unsure, and every one of them visible by the shared visibility test, so a chip kept in the list and hidden by opacity, by an inherited visibility, by a clip or by being pushed off the side is named |
...
| type sizes and weights on the scale | 3 | WARN on a size or weight off the scale. The admitted weights are gate.py's ALLOWED_WEIGHTS, which includes 400. It is measured on the three base screens only, not on the 209 drawn states, which carry sizes of their own (20 px and 34 px are drawn there and this check never sees them) |
...
| pressed state on every tappable surface | R | the eight surfaces named in gate.py's PRESSABLE (three Today, three Workout, two Coach), which are a sample and not every tappable surface. The pointer is moved on and left to settle before the idle picture, so a hover style with no pressed style of its own FAILs. An element outside the viewport is a FAIL line, never a crash |
...
| the gate finished this screen | 3 | the diagnostic: a check that threw is a FAIL row naming the exception, never a crash that ends the run. It is emitted only when something threw, so it is absent from a green run |
...
both margins, contrast in the same two tiers, and the primary its frozen state contract requires. The contract names 180 states that require #start, #log, .mic-button or .panel-primary and 29 intentional no-primary states. A required control must exist, pass the shared drawn-visibility test and stay wholly inside every unrounded viewport edge; removing or hiding it cannot redefine the expectation. It is then
...
on Windows and judged by a full Linux run, all 418 renders came back with the worst rect edge in
the whole set moved 0.04 px of the 3 allowed, at W-05 ink element 22 "Unsure" left, and every
colour 0.00 of 3. The thumbnail half is a raster, which between those
...
two decimals, so a size 0.51 px off its record fails; held to one decimal it read as exactly 0.50
and passed. Every font size the approved build draws is exact at one decimal, so no committed
record's value changes. A run never drops a problem in silence: the
