...
# the committed record: the screen's visible text, then every visible text bearing element in
# document order with its own text, its rounded rect, its computed colour, its first font family
# and its font size
JS_RECORD = """()=>{const ui=document.querySelector('.screen.is-active .ui');if(!ui)return null;
    ...
    return {text: norm(ui.innerText), els: els}}"""
...
def compare_record(sid, theme, rec, thumb):
    ...
    for i, (wa, wb) in enumerate(zip(a, b)):
        ...
        if max(abs(x - y) for x, y in zip(wa[2], wb[2])) > COLOUR_TOL:
    ...
    async with async_playwright() as p:
        ...
        for st in states:
            for t in ['ink', 'dawn']:
                ...
                bad = copy_problems(info['text'])
                if bad: problems.append('copy: ' + ', '.join(repr(x) for x in bad))
                xbad = set_x_problems(info['text'])
                ...
                if st['screen'] == 'workout' and re.search(r'\boptional\b', info['text'].lower()):
                ...
                if rec is None:
                    ...
                    problems += compare_record(st['id'], t, rec, thumb)
...
def write_report(rows):
    ...
    head = f'STATE SHEET: {len(rows)} renders, {len(bad)} with problems' + (f', {len(rows)} SET' if ACCEPT else '')
    ...
    rep = '\n'.join(lines)
    print(rep)
    name = 'states-report' + ('-' + ONLY.rstrip('-') if ONLY else '') + '.txt'
...
def refuse(msg):
    ...
    print(line)
    try:
        with open(os.path.join(RUN, 'states-report.txt'), 'w', encoding='utf-8') as f:
            f.write(line + '\n')
    except Exception:
        pass
    sys.exit(2)
