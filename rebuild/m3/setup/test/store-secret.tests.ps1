...
#
# Status: NOT RUN in the integrator's Linux cloud session (no PowerShell there). The revision-1 file ran 38/38 on
# cowork's Linux PowerShell 7.4.6; this revision has not been run anywhere yet. The next run's summary line goes
# into the report.
...
# ---- an in-memory facade with the same four operations the real one has -------------------------------------
...
function New-MemoryFacade([hashtable]$seed = @{}, [hashtable]$ProcessView = $null) {
  ...
  return [pscustomobject]@{
    ...
    List       = { @($store.Keys | ForEach-Object { [string]$_ }) }.GetNewClosure()
  }
}
function NullProc { return { param($n) $null } }
...
$p = Parse-StoreSecretArgs @("--verify-fresh-proof", "EARNED_FRESHPROOF_0123abcd"); Check "args: --verify-fresh-proof NAME" ($p.Mode -eq "verify" -and $p.ProofName -eq "EARNED_FRESHPROOF_0123abcd")
$p = Parse-StoreSecretArgs @("--verify-fresh-proof");      Check "args: --verify-fresh-proof without NAME (auto)" ($p.Mode -eq "verify" -and $p.ProofName -eq "")
$p = Parse-StoreSecretArgs @("--cleanup-fresh-proof", "EARNED_FRESHPROOF_0123abcd"); Check "args: --cleanup-fresh-proof NAME" ($p.Mode -eq "cleanup" -and $p.ProofName -eq "EARNED_FRESHPROOF_0123abcd")
$p = Parse-StoreSecretArgs @("--cleanup-fresh-proof");     Check "args: --cleanup-fresh-proof without NAME (sweep)" ($p.Mode -eq "cleanup" -and $p.ProofName -eq "")
$p = Parse-StoreSecretArgs @("--verify-fresh-proof", "SENTINEL-not-a-name-7Q9"); Check "args: verify with bad NAME → error 5, not echoed" ($p.Mode -eq "error" -and $p.ExitCode -eq 5 -and (NoEcho $p.Message "SENTINEL"))
...
# ---- derived dummy value ------------------------------------------------------------------------------------
$e1 = Get-FreshProofExpected "EARNED_FRESHPROOF_0123abcd"; $e2 = Get-FreshProofExpected "EARNED_FRESHPROOF_0123abce"
Check "derived: deterministic" ($e1 -eq (Get-FreshProofExpected "EARNED_FRESHPROOF_0123abcd"))
Check "derived: differs per name" ($e1 -ne $e2)
Check "derived: carries the marker and the suffix" ($e1 -like "EARNED-FRESHPROOF-OWNED-0123abcd-*")
Check "derived: bad name → null" ($null -eq (Get-FreshProofExpected "PATH"))
...
$bad = [pscustomobject]@{ Get = { param($n) "something-else" }; Set = { param($n, $v) }; GetProcess = (NullProc); List = { @() } }
...
$thrower = [pscustomobject]@{ Get = { param($n) $null }; Set = { param($n, $v) throw "access denied (dummy)" }; GetProcess = (NullProc); List = { @() } }
...
# ownership guard (1): SAME PREFIX, DIFFERENT VALUE at cleanup → NOT deleted
$script:n = 0
$store = @{}
...
# setter WRITES then throws: still set-failed, landed value cleaned up (attempted flag set before Set)
$script:n = 0
$seen2 = @{}
...
# cleanup itself throws → cleanup-failed
$script:n = 0
$seen3 = @{}
...
# read-back throws after the write → truthful ambiguity
$script:n = 0
$calls = @{ get = 0 }; $seen4 = @{}
...
# read-back mismatch → unverified
$script:n = 0
$script:written = $false
$mismatch = [pscustomobject]@{ Get = { param($n) if ($script:written) { "not-what-we-wrote" } else { $null } }; Set = { param($n, $v) $script:written = ($null -ne $v) }; GetProcess = (NullProc); List = { @() } }
...
# ---- fresh-process proof: arm → verify → cleanup (NAME only; value derived from the NAME) ---------------------
$script:n = 0
...
Check "fresh: arm message carries NAME but not the value" (("$($a.Message)" -like "*$($a.Name)*") -and (NoEcho $a.Message "EARNED-FRESHPROOF-OWNED"))
Check "fresh: stored value equals the derived expected value" ($f.Store[$a.Name] -ceq (Get-FreshProofExpected $a.Name))
$v = Invoke-FreshProofVerify -Name $a.Name -Facade $f
Check "fresh: verify(NAME) ok in a fresh process" ($v.Ok -and $v.Outcome -eq "ok")
Check "fresh: verify message carries no value" (NoEcho $v.Message "EARNED-FRESHPROOF-OWNED")
$v2 = Invoke-FreshProofVerify -Name "" -Facade $f
Check "fresh: verify() auto-detects the single armed variable" ($v2.Ok -and $v2.Name -eq $a.Name)
$c = Invoke-FreshProofCleanup -Name $a.Name -Facade $f
Check "fresh: cleanup(NAME) removed" ($c.Ok -and $c.Outcome -eq "removed" -and -not $f.Store.ContainsKey($a.Name) -and $f.Store["UNRELATED"] -eq "keep-me")
$c2 = Invoke-FreshProofCleanup -Name $a.Name -Facade $f
Check "fresh: second cleanup(NAME) → already-gone, ok" ($c2.Ok -and $c2.Outcome -eq "already-gone")
$c3 = Invoke-FreshProofCleanup -Name "" -Facade $f
Check "fresh: cleanup() with nothing armed → ok, already-gone" ($c3.Ok -and $c3.Outcome -eq "already-gone")
...
$v = Invoke-FreshProofVerify -Name $a.Name -Facade $stale
...
$v = Invoke-FreshProofVerify -Name "" -Facade $stale
Check "fresh: stale process, auto-detect → still not-visible" ((-not $v.Ok) -and $v.Outcome -eq "not-visible-in-this-process")

# process inherited a DIFFERENT value than expected → FAIL process-value-differs
$script:n = 0
$diff = New-MemoryFacade @{} @{}
$a = Invoke-FreshProofArm -Facade $diff -Rng $fixedRng
$diff.GetProcess = { param($n) "an-older-value" }
$v = Invoke-FreshProofVerify -Name $a.Name -Facade $diff
Check "fresh: process value differs → not ok, outcome process-value-differs" ((-not $v.Ok) -and $v.Outcome -eq "process-value-differs")

# foreign value under an armed name (same prefix, not the derived value) → verify FAIL foreign-value; cleanup refuses; sweep keeps it
$script:n = 0
$f = New-MemoryFacade
$a = Invoke-FreshProofArm -Facade $f -Rng $fixedRng
$f.Store[$a.Name] = "EARNED-FRESHPROOF-OWNED-" + $a.Name.Substring(18) + "-not-the-derived-tail"
$v = Invoke-FreshProofVerify -Name $a.Name -Facade $f
Check "fresh: foreign (same-prefix) value → verify not ok, outcome foreign-value" ((-not $v.Ok) -and $v.Outcome -eq "foreign-value")
$c = Invoke-FreshProofCleanup -Name $a.Name -Facade $f
Check "fresh: foreign (same-prefix) value → cleanup(NAME) refuses, value kept" ((-not $c.Ok) -and $c.Outcome -eq "foreign-value-not-removed" -and $f.Store.ContainsKey($a.Name))
$c = Invoke-FreshProofCleanup -Name "" -Facade $f
Check "fresh: foreign value → sweep keeps it and reports" ((-not $c.Ok) -and $c.Outcome -eq "foreign-value-not-removed" -and $f.Store.ContainsKey($a.Name) -and $c.Kept -contains $a.Name)
$v = Invoke-FreshProofVerify -Name "" -Facade $f
Check "fresh: only a foreign value present, auto-detect → not-armed" ((-not $v.Ok) -and $v.Outcome -eq "not-armed")

# absent
$v = Invoke-FreshProofVerify -Name "EARNED_FRESHPROOF_0000ffff" -Facade (New-MemoryFacade)
Check "fresh: never armed (NAME) → not ok, outcome not-armed" ((-not $v.Ok) -and $v.Outcome -eq "not-armed")
$v = Invoke-FreshProofVerify -Name "" -Facade (New-MemoryFacade)
Check "fresh: never armed (auto) → not ok, outcome not-armed" ((-not $v.Ok) -and $v.Outcome -eq "not-armed")

# two armed variables → auto-detect is ambiguous; sweep removes both owned ones
$script:n = 0
$f = New-MemoryFacade
$a1 = Invoke-FreshProofArm -Facade $f -Rng $fixedRng
$a2 = Invoke-FreshProofArm -Facade $f -Rng $fixedRng
$v = Invoke-FreshProofVerify -Name "" -Facade $f
Check "fresh: two armed, auto-detect → ambiguous" ((-not $v.Ok) -and $v.Outcome -eq "ambiguous")
$v = Invoke-FreshProofVerify -Name $a2.Name -Facade $f
Check "fresh: two armed, explicit NAME → ok" ($v.Ok)
$c = Invoke-FreshProofCleanup -Name "" -Facade $f
Check "fresh: sweep removes both owned variables" ($c.Ok -and $c.Removed.Count -eq 2 -and $f.Store.Count -eq 0)
...
# arm: setter writes then throws → set-failed, cleaned
$script:n = 0
$seen5 = @{}
$armThrows = [pscustomobject]@{
  ...
  GetProcess = (NullProc); List = { @($seen5.Keys) }.GetNewClosure()
}
...
# arm: read-back mismatch → unverified, cleanup attempted (exact value only, so the foreign value stays)
$script:n = 0
$script:w2 = $false
$armMismatch = [pscustomobject]@{ Get = { param($n) if ($script:w2) { "not-what-we-wrote" } else { $null } }; Set = { param($n, $v) $script:w2 = ($null -ne $v) }; GetProcess = (NullProc); List = { @() } }
$a = Invoke-FreshProofArm -Facade $armMismatch -Rng $fixedRng
Check "fresh: arm read-back mismatch → not ok, outcome unverified" ((-not $a.Ok) -and $a.Outcome -eq "unverified-readback-mismatch")
