...
## Mechanical coverage after the key checkpoint

This follow-up retains `8e7e1b00658f273874c9fe840aadb233a6a83693` as its base and changes tests/evidence only. The new `test/frame-browser-migration.mjs` case runs through the existing frame browser runner. Its real versionchange abort leaves database1 and the complete active/previous v1 pair unchanged. Database2 before conversion refuses ordinary reads/writes; conversion abort after the active request succeeds preserves both old records and consumes the attempted capability. Fresh preparation/retry converts the complete T2 operations/outbox plus unknown collection/history, retains the exact legacy active as previous, and stays unproven18. Missing active refuses both load modes without fallback or reseeding. A disposable omitted legacy-predecessor write fails the raw-pair assertion before unseal; the restored source is rebuilt and the complete matrix reruns successfully.

`test/frame-nonce-browser.mjs` also passes through that runner: two actual pages prepare from the same pair, one wins and the other receives retryable stale refusal. Consumed capabilities cannot write. The losing page reloads into a fresh realm, reads the winning pair and prepares a new attempt. An actual request-success abort and repository reopen preserve the complete body/pair/U; a fresh retry draws again without advancing the aborted revision. Six preparations yield six observed frame nonce/body IV draw pairs; record bytes are matched to those draws. Missing RNG permits authenticated reading but refuses preparation3 without a durable effect. Deterministic per-realm test draws are not a production RNG, global uniqueness proof or per-key budget; six preparations do not imply six frame-encrypt calls. Both new cases passed on installed Chrome151.0.7922.174, with unchanged product bytes and all required local regressions passing.

The accepted proposal's section7 is the coverage index below; existing cases are retained, not relabelled as new work. Exact commands/verdicts and current run boundaries are in the report.

| Section7 obligation | Named executable evidence | Remaining boundary |
|---|---|---|
| 1: cipher/profile/parser/dependencies | `frame-codec.test.mjs`, `strict-json.test.mjs`, `FRAME-BROWSER`, `clean-build.mjs` | Same-library Node/browser parity, not independent cipher implementation or security audit. |
| 2: proof ownership/read/predecessor/CAS | `frame-repository.test.mjs` retained-field, caller-mutation, missing-validator/key and same-revision cases; `FRAME-PREVIOUS` | All ten predecessor fields are mutated on read; CAS specifically covers nonce/malformed records, not all ten fields. Production semantic validators stay open. |
| 3: nonce/key lifecycle | `frame-codec.test.mjs` attempt-burn/window cases; `FRAME-KEY-BROWSER`; `FRAME-NONCE-BROWSER` | Actual-browser CAS/reload/abort/reopen draw routing now executed; operational revision windows still do not bound all encryption attempts, and the helper-level epoch boundary is not a whole-repository boundary run. |
| 4: final sample/batch/refusal | `frame-repository.test.mjs` later-sample, actual full-batch, control, evidence-rollback and dispatch cases; browser public sink | Synthetic finalizer/epochs are not production 24h/64 eligibility or sufficient server-time bounds. |
| 5: migration/faults | `FRAME-MIGRATION-BROWSER`, `FRAME-OLD-TAB`, Node conversion/key/control failures | Actual IDB abort and repository reopen, not OS-crash/iPhone proof. Reopening a basis already at18 does not prove newly learned knowledge persisted. |
| 6: effective mutations | Partial charge, reader predecessor comparison, historical unsealing, legacy predecessor publication and earlier bridge early-ack witnesses | Other promised binding/unknown-data/body-frame/nonce-reuse/epoch/old-sample/control-body source faults remain unproved; input rejection and import-pin refusal are distinct evidence. |

NEXT: same W6 claim and draft PR32; migration and nonce-lifecycle coverage now executed and ready for independent exact-candidate review. Further mechanical work is the explicitly outstanding effective-fault/CAS-field/whole-repository epoch-boundary coverage above, not repeated completed cases. K1/T1/P1/R1/W4 and physical/private blockers remain separate; no complete FRAME, CLOCK, W6, M3 or workout-use verdict follows from this map.
