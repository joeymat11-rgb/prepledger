...
//   4. PARTIAL ERASURE (the device key record removed under the page) is
...
try {
  ...
  /* 6. PARTIAL ERASURE. The device key record is removed under the page — the
  ...
  console.log(`W6 LOCAL-TODAY-BROWSER PASS — ${cases}/5 real IndexedDB cases against THE BUILT PAGE `
    ...
    + `into ONE sealed generation, one lease, one checkpoint; partial erasure is RESTORE_REQUIRED by name `
    + `and never a re-enrolment; localStorage holds nothing.`);
