...
try{
 ...
 const checks=await page.evaluate(async({lease,key,scopeDigest,identityKey,remote,expectedSourceState})=>{
  ...
  ok(C.fullEqual(assembledCopy.collections.sync.snapshot,before.generation.collections.sync.snapshot)&&C.fullEqual(assembledCopy.metadata.authorityLease,before.generation.metadata.authorityLease),'native admission does not change plan or grant a new lease');
