...
if(process.argv.includes('--bite')||process.argv.includes('--profile-bite')||process.argv.includes('--ordinal-bite')){
 const ordinalBite=process.argv.includes('--ordinal-bite'),profileBite=process.argv.includes('--profile-bite')||ordinalBite;
 ...
 for(const name of ['recovery-stage.mjs','recovery-profile.mjs','test/recovery-stage/profile-vectors.mjs','test/recovery-stage/stage.test.mjs','test/recovery-stage/browser.mjs','test/run-recovery-stage.cjs'])names.add('rebuild/m3/w6/'+name);
 ...
 const sourceName=profileBite?'recovery-profile.mjs':'recovery-stage.mjs',file=path.join(out,'rebuild/m3/w6',sourceName),original=fs.readFileSync(file),text=original.toString(),needle=ordinalBite?text.split('\n').find(line=>line.startsWith(' for(let n=1;n<=metadata.seq;n++)')):profileBite?"if(!await publicVerifier.verifyDisposition(h))fail('DISPOSITION_SIGNATURE');":'const decision=validateContext();';
 if(text.split(needle).length!==2)throw Error('Exact recovery mutation cut missing');fs.writeFileSync(file,text.replace(needle,profileBite?'/* disposable original-signature bypass */':'const decision=null;'));
 const args=['--test','--test-reporter=tap',...profileBite?[]:['--test-name-pattern','known standing loss refuses'],path.join(out,'rebuild/m3/w6/test/recovery-stage/stage.test.mjs')],env={...process.env,EARNED_ROWS_R1_ROOT:source};
 ...
 if(red.status!==1||!(ordinalBite?/not ok \d+ - coherent signed log gap defeats counts alone/.test(red.stdout):profileBite?/not ok \d+ - original disposition signature forged/.test(red.stdout):red.stdout.includes('not ok 1 - known standing loss refuses')))throw Error('Recovery cut mutation was not effective; '+red.stdout+red.stderr);
