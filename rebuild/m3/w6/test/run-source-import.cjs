...
for(const name of ['index','dates','constants','seed','plan','progression','sleep','energy','policy','today','volume','migrate','earn','merge','writers'].map(n=>'rebuild/engine/'+n+'.cjs')
  .concat(['rebuild/m4/import/prepare.cjs','rebuild/m3/w7-preview/fixtures.cjs'],process.argv.includes('--browser')?[]:['rebuild/m4/import/reading-replay.cjs','rebuild/m4/import/daily-history.cjs']))
