...
export function sleepNightsIn(generation, profile = PROFILE) {
  ...
  return Object.values(collections.ops || {})
    ...
    .map((op) => Object.freeze({
      ...
      ...(Object.hasOwn(op.payload, 'supersedes') ? { supersedes: op.payload.supersedes } : {}),
...
export function checkInSourceFault(generation, night) {
  ...
  if (!hours || typeof hours !== 'object' || hours.unit !== 'h' || hours.value !== night.hours) return 'SLEEP_SOURCE_WRONG_HOURS';
  ...
  try {
    ...
    const handle = {
      ...
      async all() {
        ...
        return sleepNightsIn((await bindings.repository.load()).generation, PROFILE);
      },
      async forDate(date) {
...
export default { createSleepHost, sleepNightsIn, checkInSourceFault,
  PROFILE, OP_CLASS, OP_KIND, SLEEP_SCHEMA_VERSION };
