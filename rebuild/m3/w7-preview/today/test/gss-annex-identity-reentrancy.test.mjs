...
async function make(){
  ...
  const lane=createGymSettingsLane(doc,phone,model,settings,Object.freeze({repaint:()=>repaintHook()}));
  ...
  return{dom,phone,save,log,held,entered,calls,outcomes,lane,token,raw,click,
    armRepaint(callback){repaintHook=()=>{repaintHook=()=>{};return callback();};},
...
for(const revision of [0,1])test('GSS review: revision '+revision+' saved outcome preserves exact editor token with one raw capture', {timeout:5000},async()=>{
  const u=await make();let reads=0;
  try{
    u.lane.hooks.bindSettingsSave(u.token,()=>{reads+=1;return u.raw(revision);},outcome=>u.outcomes.push(outcome));
    ...
    assert.equal(reads,1,'GSS-SINGLE-RAW-CAPTURE');
    ...
    assert.equal(u.outcomes[0].editorRevised,undefined,'lane does not invent revision outcomes');
    u.lane.hooks.bindSettingsSave(u.token,()=>u.raw(revision),()=>{throw Error('STALE_TOKEN_DELIVERY');});
    u.save.disabled=false;u.click(u.save);await tick();
    assert.equal(u.calls.length,1,'retired editor token cannot bind another write');
  }finally{await u.close();}
});

for(const invalidation of ['replace','leave','dispose'])test('GSS review: post-save repaint '+invalidation+' cannot deliver or retire a new editor', {timeout:5000},async()=>{
  const u=await make();let reads=0,repainted=0,replacement=null,dispose=()=>{},replacementOutcomes=[];
  try{
    dispose=u.lane.hooks.bindSettingsSave(u.token,()=>{reads+=1;return u.raw(0);},outcome=>u.outcomes.push(outcome));
    u.armRepaint(()=>{
      repainted+=1;
    ...
    assert.equal(reads,1,'GSS-SINGLE-RAW-CAPTURE');assert.equal(repainted,1,'post-save callback reached');
    assert.equal(u.calls.length,1,'original admitted save only');
    assert.equal(u.lane.facade.settingsBusy(),false);
    assert.equal(u.outcomes.length,0,'GSS-POSTSAVE-'+invalidation.toUpperCase()+'-STALE-DELIVERY');
...
test('GSS review: post-save repaint stays under synchronous refusal and pending reservation',{timeout:5000},async()=>{
  const u=await make();let reads=0,repainted=0,originalPending;
  try{
    u.lane.hooks.bindGymAction('logSet',()=>({load:'1',reps:'1',effort:{reserve:{tag:'unknown'}}}),()=>{});
    u.lane.hooks.bindSettingsSave(u.token,()=>{reads+=1;return u.raw(0);},outcome=>u.outcomes.push(outcome));
    u.armRepaint(()=>{repainted+=1;u.save.disabled=false;u.click(u.save);u.click(u.log);assert.equal(u.lane.api.pending(),originalPending);assert.equal(u.lane.facade.settingsBusy(),true);});
    u.click(u.save);originalPending=u.lane.api.pending();await bounded(u.entered.promise);await u.complete();
    assert.equal(reads,1,'GSS-SINGLE-RAW-CAPTURE');assert.equal(repainted,1);assert.equal(u.calls.length,1,'nested callbacks cannot admit a writer');
    assert.equal(u.outcomes.length,1);assert.equal(u.lane.facade.settingsBusy(),false);
  }finally{await u.close();}
});

test('GSS review: single raw capture rejection releases reservation without starting a command',{timeout:5000},async()=>{
  const u=await make();let reads=0;
  try{
    u.lane.hooks.bindSettingsSave(u.token,()=>{reads+=1;throw Error('SYNTHETIC_RAW_CAPTURE');},o=>u.outcomes.push(o));
    u.click(u.save);await assert.rejects(bounded(u.lane.api.pending()),/SYNTHETIC_RAW_CAPTURE/);await tick();
    assert.equal(reads,1,'GSS-SINGLE-RAW-CAPTURE');assert.equal(u.calls.length,0);assert.equal(u.outcomes.length,0);assert.equal(u.lane.facade.settingsBusy(),false);assert.equal(u.save.disabled,false);
  }finally{await u.close();}
});
