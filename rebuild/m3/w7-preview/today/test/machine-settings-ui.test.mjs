...
/* -----------------------------------------------------------------------
   P-MEASURE v1 ROUND 2 - closing the reviewer's findings on
   rebuild/lanes/c/P-MEASURE-REVIEW-R1.md (sha 28d3b4a). Each block below is
   headed with the finding it closes.
   ----------------------------------------------------------------------- */
import { createMeasureHost, waistRowsIn, measureMarkersIn } from '../measure-host.mjs';
import MeasureCommandsMod from '../measure-commands.cjs';
import BaselineMod from '../measure-baseline.mjs';
import { admittedLocalSourceState, admittedLocalSourceBasis } from '../local-source-basis.mjs';

const { markersOf } = MeasureCommandsMod;
const { baselineWeeksFromState, baselineWeeks } = BaselineMod;

/* FINDING 1 - nothing recorded. Executed over the REAL encrypted repository
   (fake-indexeddb), the SAME durable client every other lane in this file
   uses: a save() writes ONE operation with an op_id and a device_seq, and
   all() reads it back durably - not a plain object nobody records. */
test('P-MEASURE R2 finding 1 - the waist entry is recorded through a real op, with op_id and device_seq', async () => {
  const fault = faultDatabase();
  const host = await createMeasureHost({ day: DAY, indexedDB: fault.indexedDB, crypto: webcrypto });
  const saved = await host.save({ date: '2026-01-05', in: 34.5 });
  assert.equal(saved.ok, true, saved.code || '');
  assert(typeof saved.op_id === 'string' && saved.op_id, 'no op_id came back from a successful save');
  ...
  assert(Number.isInteger(rows[0].device_seq) && rows[0].device_seq >= 1, 'no device_seq on the durable row');
  /* A correction is a NEW op, not an update: projectWaist (round 2 fix) picks
     the LATEST date, and here both ops share a date so the durable read shows
     both while the projector still resolves one winner. */
  const corrected = await host.save({ date: '2026-01-05', in: 35 });
  assert.equal(corrected.ok, true);
  assert.notEqual(corrected.op_id, saved.op_id);
  const rows2 = await host.all();
  assert.equal(rows2.length, 2, 'a correction appends, it does not update');
  host.close();
});

/* FINDING 1, continued - the store-level producer refuses independently of
   the screen: a future-dated waist value never reaches the durable log even
   if a caller bypassed measure-model.mjs's own refusal. */
test('P-MEASURE R2 finding 1 - the producer refuses a future-dated waist entry at the store', async () => {
  const fault = faultDatabase();
  const host = await createMeasureHost({ day: DAY, indexedDB: fault.indexedDB, crypto: webcrypto });
  const result = await host.save({ date: '2099-01-01', in: 34 });
  assert.equal(result.ok, false);
  const rows = await host.all();
  assert.equal(rows.length, 0, 'a refused write left nothing in the log');
  host.close();
});

/* FINDING 8 - the 3-4 markers are a fixed pick STORED ON DEVICE, chosen once. */
test('P-MEASURE R2 finding 8 - the markers pick is recorded once and refused a second time', async () => {
  const fault = faultDatabase();
  const host = await createMeasureHost({ day: DAY, indexedDB: fault.indexedDB, crypto: webcrypto });
  assert.equal(await host.markers(), null, 'no pick yet');
  ...
  const second = await host.saveMarkers(['Overhead Press', 'Row', 'Pull-up']);
  assert.equal(second.ok, false);
  assert.equal(second.code, 'MEASURE_MARKERS_ALREADY_RECORDED');
  assert.deepEqual(await host.markers(), ['Back Squat', 'Bench Press', 'Deadlift'], 'the first pick still stands');
  host.close();
});
test('P-MEASURE R2 finding 8 - the producer refuses fewer than 3 or more than 4 markers', () => {
  assert.throws(() => markersOf(['Only', 'Two']));
  assert.throws(() => markersOf(['A', 'B', 'C', 'D', 'E']));
  assert.throws(() => markersOf(['A', 'A', 'B']), 'no duplicate marker');
  assert.deepEqual(markersOf(['A', 'B', 'C']), ['A', 'B', 'C']);
});

/* FINDING 3 - the baseline never touches local-source-basis. Executed over a
   SYNTHETIC admitted import (C2b shape: one committed marker, one derived
   view, three identical copies of one basis), the same generation shape
   admittedLocalSourceBasis itself requires, proving an imported window comes
   back non-empty and is what the baseline actually reads. */
test('P-MEASURE R2 finding 3 - the baseline is derived from a synthetic admitted import', async () => {
  const trialStart = '2026-03-02';
  const first = new Date(Date.UTC(2026, 0, 5));                 // 56 days before trial start
  const reads = [];
  for (let i = 0; i < 56; i++) {
    const d = new Date(first.getTime() + i * 86400000).toISOString().slice(0, 10);
    reads.push({ d, w: 200 - i * 0.1 });
  }
  const waist = [{ d: '2026-01-12', in: 36 }, { d: '2026-02-09', in: 35.4 }];
  const state = { exercises: [], athlete_label: null, reads, waist };
  const basis = { installation_id: 'ns-1', imported_at: '2026-02-20' };
  const view = { ready: true, pending: false, issues: [], state, basis };
  const derived = { view, basis };
  const marker = { core_complete: true, selection_id: 'sel-1', basis };
  const sources = { active: 'sel-1' };
  const generation = { metadata: { localSourceApplication: marker, localSources: sources },
    collections: { derived: { localSource: derived } } };

  const direct = admittedLocalSourceBasis(generation, { athleteLabel: null, namespace: null });
  assert(direct, 'the synthetic import was not admitted');
  assert.equal(direct.reads.length, 56);

  const weeks = baselineWeeksFromState(direct, trialStart, 8, ['Back Squat']);
  assert.equal(weeks.length, 8);
  assert.equal(weeks[0].weightAvg, 199.7, 'weightAvg of the imported window, first week');
  assert.notEqual(weeks.every((w) => w.weightAvg === null), true, 'an imported window with no baseline data');
  assert.equal(weeks[1].waist, 36);

  const setup = { host: { repository: { load: async () => ({ generation }) }, namespace: null },
    athleteLabel: () => null };
  const viaJoin = await baselineWeeks(setup, trialStart, 8, ['Back Squat']);
  assert.deepEqual(viaJoin, weeks, 'the full join (local-source-basis -> baseline) matches the direct call');

  /* No admitted import (P3 not landed) - the baseline stays empty, unchanged. */
  const none = await baselineWeeks({ host: { repository: { load: async () => ({ generation: {} }) } } },
    trialStart, 8, ['Back Squat']);
  assert.deepEqual(none, []);
});

/* FINDINGS 5, 6, 7, 9, 10, 11 - a fixture WITH gaps, exercising every null
   branch, adapted from the reviewer's own R1-R11 synthetic cells. */
test('P-MEASURE R2 - a fixture with gaps exercises every null branch (finding 5)', () => {
  const START2 = '2026-02-02';
  const day2 = (n) => new Date(Date.UTC(2026, 1, 2 + n)).toISOString().slice(0, 10);
  const reads2 = [];
  for (let n = 0; n < 84; n++) {
    if (n === 9 || n === 11 || n === 13) continue;               // week 2 gaps
    if (n === 40) continue;                                      // week 6, one missed day
    reads2.push({ date: day2(n), lb: Math.round((181 - n * 0.1) * 10) / 10 });
  }
  const waist2 = [];
  for (let w = 0; w < 12; w++) {
    if (w === 3) continue;                                       // week 4 never measured
    waist2.push({ date: day2(w * 7 + 1), in: Math.round((34 - w * 0.1) * 10) / 10 });
  }
  const sets2 = [];
  for (let w = 0; w < 12; w++) {
    sets2.push({ date: day2(w * 7), marker: 'Squat', load: 225 + w * 5, reps: 5 });
    if (w !== 5) sets2.push({ date: day2(w * 7 + 2), marker: 'Bench', load: 155 + w * 2.5, reps: 8 });
  }
  const MK2 = ['Squat', 'Bench'];
  const wk2 = (i, extra = {}) => computeWeek({ startDate: START2, index: i, reads: reads2, waistRows: waist2,
    sets: sets2, markers: MK2, sessionsCompleted: 4, sessionsPlanned: 4,
    foodDaysLogged: 7, energyDaysLogged: 5,
    nights: [{ clean: true }, { clean: true }, { clean: false }, { clean: true }, { clean: true }],
    ...extra });

  assert.equal(wk2(0).weightAvg, 180.7);
  assert.equal(wk2(1).weightAvg, null, 'week 2, 4 of 7 reads');
  assert.equal(wk2(5).weightAvg, null, 'week 6, 6 of 7 reads');
  assert.equal(wk2(0).waist, 34);
  assert.equal(wk2(3).waist, null, 'week 4, never measured');
  assert.equal(wk2(4).waist, 33.6);
  assert.equal(wk2(4).waistTrend4Week, -0.4);
  assert.equal(wk2(7).waistTrend4Week, null, 'window start (week 4) is missing');
  assert.equal(wk2(5).marker1RM.Bench, null, 'no Bench set that week');
  assert.equal(wk2(0).marker1RM.Squat, 262.5);

  /* FINDING 9 - run in is a WEEK NUMBER, not an array position: weeks 3-12
     handed straight through must never read as run in. */
  const later = [wk2(2), wk2(3)].map((w) => ({ ...w }));
  const view = buildComparisonView({ trialWeeks: later, markers: MK2 });
  assert.deepEqual(view.trial.map((w) => w.runIn), [false, false],
    'weeks 3 and 4 handed at array positions 0 and 1 must not read as run in');

  /* FINDING 10 - two waist entries in one week: the LATER date wins. */
  const two = [{ date: day2(0), in: 34.5 }, { date: day2(3), in: 34 }];
  assert.equal(MeasureModel.weeklyWaistValue(two, START2, 0), 34);

  /* FINDING 11 - a blank value with a date is NOTHING, not OUT_OF_RANGE. */
  assert.equal(waistRefusalFor({ date: day2(0), in: '' }, day2(0)), WAIST_REFUSALS.NOTHING);

  /* FINDING 6 - a partial (install-day) week: 3 of 3 elapsed days logged is
     100%, not divided by a hardcoded 7. */
  const partial = wk2(11, { foodDaysLogged: 3, energyDaysLogged: 3, today: day2(11 * 7 + 2) });
  assert.equal(partial.foodAdherencePct, 100, 'got ' + partial.foodAdherencePct + ' for 3 of 3 elapsed days');

  /* FINDING 7 - an adherence percentage never exceeds 100. */
  const over = wk2(0, { foodDaysLogged: 8 });
  assert(over.foodAdherencePct <= 100, 'got ' + over.foodAdherencePct);
  assert.equal(over.foodAdherencePct, 100);
});

/* FINDING 12 - the "no path outside the device store" cell was vacuous: it
   grepped two files that touched no store at all. Now measure-host.mjs and
   measure-commands.cjs DO write to the store, this checks them for real: no
   network call, and the ONLY store touched is the one accepted repository
   (era.client.hostBindings - never a second indexedDB.open). */
test('P-MEASURE R2 finding 12 - the store-touching modules open no path outside the device store', () => {
  const names = ['measure-host.mjs', 'measure-commands.cjs', 'measure-baseline.mjs'];
  for (const name of names) {
    const code = codeOf(fs.readFileSync(path.join(ROOT, 'rebuild/m3/w7-preview/today', name), 'utf8'));
    assert(!/fetch\s*\(|XMLHttpRequest|sendBeacon|WebSocket/.test(code), name + ': no network call');
    assert(!/https?:\/\//.test(code), name + ': no network reference');
    assert(!/indexedDB\.open\s*\(/.test(code), name + ': no second store opened directly');
    assert(!/localStorage|sessionStorage/.test(code), name + ': no browser storage of its own');
  }
  /* measure-host.mjs opens through the SAME extension point sleep-host.mjs
     uses, never a repository of its own. */
  const host = codeOf(fs.readFileSync(path.join(ROOT, 'rebuild/m3/w7-preview/today/measure-host.mjs'), 'utf8'));
  assert.match(host, /hostBindings\(\{\s*workoutCommands/, 'measure-host.mjs must bind through the accepted client, not a repository of its own');
});

/* FINDING 2 - nothing renders. Structural: a real mount export, real DOM, and
   a route today-app.cjs actually reaches (code, comments stripped). */
test('P-MEASURE R2 finding 2 - the comparison view mounts, builds DOM, and Today reaches it', () => {
  const view = codeOf(fs.readFileSync(path.join(ROOT, 'rebuild/m3/w7-preview/today/measure-view.mjs'), 'utf8'));
  const app = codeOf(fs.readFileSync(path.join(ROOT, 'rebuild/m3/w7-preview/today/today-app.cjs'), 'utf8'));
  assert.match(view, /export function mountMeasureComparison/, 'no mount export');
  assert.match(view, /createElement|replaceChildren|appendChild|data-slot/, 'no DOM built');
  assert.match(app, /measure-(model|view)/, 'today-app.cjs never imports measure-model or measure-view');
  assert.match(app, /"measure"/, 'no measure route in today-app.cjs render()');
});

/* Round-2 REGRESSION on round-1's own R17/R19 shape, without depending on a
   built dist (the A1 build check runs separately): a waist-related string
   appears outside the measure-*.mjs files themselves. */
test('P-MEASURE R2 - a waist entry path exists outside measure-*.mjs (finding 19 regression)', () => {
  const app = fs.readFileSync(path.join(ROOT, 'rebuild/m3/w7-preview/today/today-app.cjs'), 'utf8');
  assert.match(app, /waist/i);
});
