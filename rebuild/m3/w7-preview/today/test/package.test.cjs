...
test("no athlete data and no credential is shipped in the bundle", async () => {
  ...
  // The one synthetic label left in the page says what it is in its own name.
  assert(app.includes("synthetic-preview-identity-not-a-credential"));
  /* A2 review B2 — since the device mints its own enrolment there is no key
     LITERAL in the page at all: both private keys are generated by WebCrypto on
     the device and are non-extractable, so nothing that could be copied off the
     device is ever written down. */
  ...
  for (const call of [/generateKey\(/, /"AES-GCM"|'AES-GCM'/, /"ECDSA"|'ECDSA'/]) {
    assert(call.test(app), "the page generates its own keys: " + call);
  }
  // Neither generated key is extractable — the third argument of both calls is false.
  assert(/generateKey\(\s*\{\s*name:\s*["']AES-GCM["'],\s*length:\s*256\s*\},\s*!1|false/.test(app)
    || /generateKey\([^)]*,\s*(?:!1|false)\s*,/.test(app), "the generated keys are non-extractable");
});
