...
## Current v0.56 actual engine capture and original-input reconstruction

Current integration state, September9: workout/engine-capture.cjs now builds a validated immutable capture from the actual genSession/rirPlan readers and resolves its slot layout by read-only reconstruction from the ORIGINAL engine inputs. The static earned/engine-workout-capture/v1 profile defines this producer's within-lift tuple IDs and ordering; it does not rename arbitrary IDs from other producers. The resolver requires original producer/basis and exact complete capture equality. Current engine/clock/state is not a substitute for the original source.

The adapter preserves each supplied prescribed load vector, selected debut vector, target, effort and setup/reason. A baseline prompt's placeholder0 is not a numeric rep prescription; its load/reps are explicitly not_prescribed. Missing confidence stays unknown. Missing/ambiguous load mapping refuses instead of flattening. A known actual boundary omission is repaired: genSession drops holdFlag, so the adapter passes the governing exercise's flag into the existing rirPlan reader. The reached one-set held case was0 before,2 after, following that reader's actual existing guard. This is a call-site binding correction, not new physiology or a globally changed policy.

Actual W6 Start→Set→early Close→correction→fresh reopen now resolves layout through this engine reconstruction. Later plan changes to60lb/3sets cannot replace the original40lb/2set capture; original/current/edit values remain, and the proposed reader accounts360 from actual corrected9reps. The test-owned immutable original-input map, identity, schema2 lease and IndexedDB adapter are explicitly synthetic.44/44 combined capture/history/order tests (10new adapter cases), original conformance99/99/29/70 plus rig185 and SELFTEST PASS. Accepted static engine bytes and W6 runtime unchanged; no qualification-package rerun.

Still required: production authentication/custody/recovery of the complete original engine/clock/input snapshot, actual immutable import/source activation, qualified current state projection and rule applicability/caller publication. The test input map is not a production store or permission grant. In particular the existing rirPlan history policy still has its legacy opener-history reader; this adapter does not claim that corrected rich facts already drive every policy. No installed app producer or private/phone/scientific acceptance follows from a mechanically faithful capture. These are the next real dependencies in the single ordered NEXT at outputs/EARNED-DELIVERY-STATUS.md.

W6 remains4eba747 with actual authenticated callback/Start-reopen evidence34mapping/372composition/44native and conformance/selftest at its cut. Recovery31PASS already proved the existing authenticated index feeds M4: no extra receipt plumbing. R1 remains02de388 over reviewed342 runtime; capacityOPEN. APM owns corrected frozen-lock Linux setup and any one authorized original witness, with no root memory experiment or duplicate dispatch. Accepted numeric3e908/artifactd21 Review149 and actual integrated ledger receipt pending remain unchanged.

Full PRODUCT-GOALv3/CURRENT-PRODUCT-DECISIONSrev3/P1–P6/all eight first-use groups, complete approved design, richer optional check-in, early qualified daily macros and later same-record Coach remain. No logger-only substitute, new review campaign, merge/private activation/release or demand to complete all future automation first. Prior v0.55 brief/report preserved in work/delivery-history/M4-{BRIEF,REPORT}-before-v056.md and2e36bbb. Historical queues below are superseded by the shared checkpoint.

New source workout/engine-capture.cjs SHA91b2fe60d8eb0620ab6facebdad437679230f30c605fa36d07854a97fb6e82c6; test workout/test/engine-capture.test.cjs uses actual accepted source construction/factories and actual W6 capture validator/client/store. Ten cases: explicit numeric zero versus missing load, held-original versus current clock/alarm reconstruction, actual vector/target/effort translation, complete original-input/capture binding, baseline placeholder semantics, held one-set reader binding, incomplete vector refusal, selected debut vector/ambiguous queue handling, rest-day absence and actual Start/correction/reopen. Final combined44PASS/0FAIL/0skip in .tmp/engine-capture-final.log; earlier clock43 and combined42 cuts remain preserved. Red .tmp/engine-capture-red.log detects actual held target0 versus required existing-reader2. One integration fixture init token typo produced ENROLLMENT_UNPROVEN; fixed to the fixture's declared synthetic token, then7PASS before selected-debut case was added. No product enrollment rule changed. Focused logs retained. R1 .tmp/engine-capture-final-{conform,selftest}.log uses prepared engines/oracle and M4 adapters, SUITE CONSISTENT99/99/29/70+rig185 and SELFTEST PASS. No private/frozen/golden/installed-engine change; no extra scientific verdict from parity.

Actual clock-advance case: the original injected clock sees the synthetic pulse alarm and returns effort[2,1]; three days later the actual bodyAlarm reader returns null and current capture[2,0]. Replaying the old day/state through that current engine fails whole-capture agreement. Reconstruction with the held original engine/clock succeeds unchanged before and after current production. A saved day string alone is not clock custody; the real import/producer caller must retain/recreate the complete original engine clock context. This is reader behavior/source preservation, not personal safety qualification.

Explicit numeric0 prescribed load is retained as specified0lb; it is distinct from a missing baseline load. This is source-value preservation, not an inference about effective bodyweight or a new prescription. Negative/nonfinite and negative-zero inputs remain invalid.

The module is ordinary internal candidate code but is not configured as the app's qualified producer. It requires real original/current input ownership outside this adapter, and the exact held original engine/clock context for reconstruction. Known unqualified policy applicability, accepted-state/consent/current-safety and actual complete-history mapping remain. The existing host supports the future connection; the test's original-input Map cannot stand in for the missing import/controller. All final independent/integrator/private/device/resource gates remain.
