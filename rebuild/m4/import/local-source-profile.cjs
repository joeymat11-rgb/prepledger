...
function createProducerRegistry(entries,{hash}={}){
 ...
 function qualify({context,materialDigest}={}){
  ...
  const handle=Object.freeze({profile:'earned/source-engine-context/v1'}),clock=clockAt(mapping.gate.clock,12,cal);
  contexts.set(handle,freeze({mapping,execution,clock,clockAt:(day,hour)=>clockAt(day,hour,cal),digest:digest(hash,'earned/local-source-engine/v1',{mapping,execution})}));
  return handle;
 }
