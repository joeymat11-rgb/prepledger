...
/* THE TRIP-WIRE, the coach's own, AND THE SEALING WINDOW IT HAS TO SURVIVE.
 *
 * The standing package is whatever rebuild.yml's own --package flag names. Once
 * that package is SEALED its receipt is on disk and the constant is recomputed
 * from it: a reseal that moves either goes red here until the successor ticket
 * updates rebuild/coach/engine-revision.cjs. But the sealing FULL run writes
 * that receipt at its END, so between the flip of the CI step to the new
 * package and the seal itself there is no receipt to read, and the old shape
 * went ENOENT - no reseal declaring this cell as a child could ever be sealed.
 * The invariant in that window is not "no check": it is that the constant still
 * names the PARENT seal, because the constant only ever moves AFTER the new
 * receipt exists (DECISIONS:465-467, :495). Both sides are one function, and
 * both are executed below against fixtures, so no cell here depends on whether
 * the real tree happens to be mid-window. */
const REVISION_RULE =
...
/* THE ONE READING OF THE RULE, over a repo root: the real tree in P3-M3, a temp
   fixture in the cells that execute its red sides. Every refusal names the
   state it refused in. */
...
function fakeRoot({ standing, spec, receipts }) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'erw-map-'));
...
test('P3-M3 - the mapping\'s engine revision names the standing package\'s sealed '
  + 'receipt on disk, or - inside the sealing window - its parent\'s', () => {
  const seal = checkRevision(Mapping.ENGINE_REVISION, REPO_ROOT);
  assert.ok(seal.state === 'sealed' || seal.state === 'window', seal.state);
  assert.equal(Mapping.ENGINE_REVISION.split('@')[0], receiptId(seal.receipt),
    'the revision package id does not match the receipt the rule points at');
});

/* THE SEALED SIDE'S RED, EXECUTED: the trip-wire this ticket must not weaken.
   A receipt for the standing package on disk still pins the constant by sha
   AND by package id, and anything else is refused. */
test('P3-M3A - SEALED: a receipt for the standing package pins the constant, and '
  + 'a wrong sha or a wrong package id is red', () => {
...
/* THE WINDOW ITSELF, EXECUTED: no receipt for the standing package, a
   BRIEF-ACCEPTED spec naming a sealed parent, and the constant must be the
   PARENT's - a stale grandparent and a random string are both red. */
test('P3-M3B - SEALING WINDOW: with no receipt for the standing package the '
  + 'constant must name the PARENT seal, and nothing else', () => {
...
test('P3-M3C - no spec, a spec that is not BRIEF-ACCEPTED, a spec with no parent '
  + 'and a parent with no receipt each fail by name', () => {
