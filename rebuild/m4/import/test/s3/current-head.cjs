...
Module._resolveFilename=function(request,parent,isMain,options){
 ...
 const absolute=path.resolve(result),real=fs.realpathSync.native(absolute),relative=path.relative(root,absolute).split(path.sep).join('/');
 if(entries.has(relative)&&same(absolute,real))return result;
 if(inside(run,real)&&real.split(path.sep).includes('node_modules'))return result;
