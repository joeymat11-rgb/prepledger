...
const FILES=Object.freeze(['rebuild/m3/w6/test/local-owner-entry.test.mjs',
...
for(const file of FILES)assert.equal(sha(fs.readFileSync(path.join(root,file))),spec.product[file]?.post,'Pinned actual owner-entry/calendar test '+file);
