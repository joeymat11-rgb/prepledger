...
function failure(error){const blocked=codes.includes(error?.code);return {exit:blocked?2:1,line:blocked?'LOAD PACKAGE BLOCKED '+error.code:'LOAD PACKAGE FAIL; required evidence missing or failed; local diagnostics withheld'};}
