...
try{
 ...
 console.log('LOAD WRITE LEGACY DIFFERENTIAL: '+runs+'/3 Date/trap modes PASS; all original assertions/roster/aliases/instance checks intact; exact approved frozen-source expectation; PACKAGE receipt PENDING');
}catch(_){console.error('LOAD WRITE LEGACY DIFFERENTIAL FAIL in '+mode+'; protected context withheld');process.exitCode=1;}
