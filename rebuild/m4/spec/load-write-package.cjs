...
const context=Profile.verify(),{root,parent:a,manifest:m}=context;
...
const logDir=path.join(root,'.tmp/load-write-package');fs.mkdirSync(logDir,{recursive:true});
...
}catch(_){console.error('LOAD PACKAGE FAIL; required evidence missing or failed; local diagnostics withheld');process.exitCode=1;}
