...
const root=path.resolve(__dirname,'../../..'),S=require('./load-write-source.cjs'),Expected=require('./load-write-expectations.cjs'),NativeDate=Date;
...
try{
 ...
 Expected.verifySource(source);console.log('LOAD WITNESS EXPECTATIONS SEALED: 10 complete source-derived witness executions; no candidate loaded');
 S.verify(root);
 for(const mode of ['frozen','native']){phase=mode;const rows=run('candidate',mode);assert.equal(JSON.stringify(rows)===sealed.get(mode),true,'Exact complete witness results/input mutations/receipts');
  console.log('LOAD WRITE WITNESS '+mode+': 5/5 complete comparisons PASS; 3 exact changed predicates; D42/D44/D45 parent consequences unchanged');
 }
 console.log('LOAD WRITE WITNESSES: 10/10 complete comparisons PASS; original frozen files unchanged; PACKAGE receipt PENDING');
}catch(_){console.error('LOAD WRITE WITNESSES FAIL in '+phase+'; protected context withheld');process.exitCode=1;}
