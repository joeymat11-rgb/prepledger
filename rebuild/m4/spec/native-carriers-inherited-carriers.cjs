...
const A=require(path.join(P,'acceptance.cjs')),Parent=require(path.join(P,'legacy-carriers.cjs')),Step=require(path.join(P,'legacy-step-efficacy-carriers.cjs')),S=require('./native-carriers-parent-source.cjs');
...
function run(id,mode){
 ...
 try{
  ...
  console.log('NATIVE INHERITED '+id+' '+mode+': PASS; '+(step?11:cases.length)+' original/accepted cases; exact existing expectation substitutions');
 }finally{globalThis.Date=NativeDate;process.argv=oldArgv;}
}
let phase='preparation';
try{for(const id of ['migrate-differential','defect-witnesses-5','defect-witnesses-2'])for(const mode of ['frozen','native']){phase=id+'/'+mode;run(id,mode);}console.log('NATIVE INHERITED CARRIERS: 6/6 PASS; import/D12 original scope retained; PACKAGE receipt PENDING');}
catch(_){console.error('NATIVE INHERITED CARRIERS FAIL at '+phase+'; protected context withheld');process.exitCode=1;}
