...
function orderWorkoutStarts(history, generation) {
  ...
  for (const id of starts.keys()) {
    ...
    dependencies.set(id, need);
  }
  const ordered = [], remaining = new Set(starts.keys());
  while (remaining.size) {
    const ready = [...remaining].filter(id => [...dependencies.get(id)].every(parent => !remaining.has(parent)));
    ...
    const next = ready[0]; ordered.push(next); remaining.delete(next);
  }
