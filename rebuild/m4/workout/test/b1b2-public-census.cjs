...
function pinned(file) {
  ...
  const bytes=H.disk(file),object=crypto.createHash('sha1').update('blob '+bytes.length+'\0').update(bytes).digest('hex');
...
function runOriginal(side,stage,mode) {
  assert.ok(['M','R'].includes(side));assert.ok(['main','worker'].includes(stage));
...
if(process.env.B1B2_CENSUS_STAGE!==undefined) {
  ...
  for(const side of ['M','R']) {
    ...
    runs.push({side,status:result.status,signal:result.signal,error:!!result.error,stdout:H.sha(result.stdout||''),stderr:H.sha(result.stderr||'')});
  }
  ...
  for(const mode of ['main','frozen','unfrozen']) {
    ...
    const before=load('M'),after=load('R');assert.ok(before.length&&after.length,'nonempty complete native frames');
  ...
  console.log('B1B2 PUBLIC CENSUS: 2 complete original public runs; 3 complete M/R frame comparisons; '+JSON.stringify(comparisons));
}
