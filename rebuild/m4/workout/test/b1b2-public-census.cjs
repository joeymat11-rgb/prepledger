...
function runOriginal(side,stage,mode) {
  assert.ok(['M','S'].includes(side));assert.ok(['main','worker'].includes(stage));
...
if(process.env.B1B2_CENSUS_STAGE!==undefined) {
  ...
  for(const mode of ['main','frozen','unfrozen']) {
    ...
    const before=load('M'),after=load('S');assert.ok(before.length&&after.length,'nonempty complete native frames');
  ...
  console.log('B1B2 PUBLIC CENSUS: 2 complete original public runs; 3 complete M/S frame comparisons; '+JSON.stringify(comparisons));
}
