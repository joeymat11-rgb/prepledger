...
if(process.env.B1B2_MIGRATION_STAGE!==undefined) {
  assert.equal(process.env.B1B2_MIGRATION_STAGE,'worker');assert.ok(['M','U'].includes(process.env.B1B2_MIGRATION_SIDE));assert.ok(['frozen','native'].includes(process.env.B1B2_MIGRATION_CLOCK));
...
function constants(ref) {
  ...
  H.reconstructR9();H.candidateSource(file);
  m.require=()=>{throw Error('CONSTANTS_UNDECLARED_DEPENDENCY');};m._compile(H.blob(ref,file).toString(),m.filename);return m.exports();
}
test('B1B2/INHERITED-1 complete candidate inventory, separate M/R, R/S, S/T and T/U proofs, original H3 construction',()=>{
  H.original('h3-supersede-inherited-carriers.test.cjs');H.closedEngineInventory();H.reconstructRuntime();H.reconstructSuccessor();H.reconstructRepair();H.reconstructR9();H.reconstructH3();
});
test('B1B2/INHERITED-2 original H3 SUP-6 constants export and four-label differential',()=>{
  const accepted=constants(H.H3_BASE),ours=constants(H.U);
...
test('B1B2/INHERITED-3 original H3 SUP-7 red-first region heads and complete producer scan',()=>{
  const ours=constants(H.U),accepted=constants(H.H3_BASE),keys=['back_lats','back_upper','back_traps','back_lower'];
