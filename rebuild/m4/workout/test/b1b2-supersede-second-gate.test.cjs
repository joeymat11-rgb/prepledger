...
// independent of the additional M/S reader projection below. Native: PC only.
H.loadOriginal('h3-supersede-second-gate.test.cjs');
test('B1B2/SECOND-1 every original reader and whole applyRead M/S output',()=>{
  H.closedEngineInventory();
  const a=H.nativeBrowser(H.M),b=H.nativeBrowser(H.S),state=structuredClone(a.SEED);
