...
// independent of the additional M/U reader projection below. Native: PC only.
H.loadOriginal('h3-supersede-second-gate.test.cjs');
test('B1B2/SECOND-1 every original reader and whole applyRead M/U output',()=>{
  H.closedEngineInventory();
  const a=H.nativeBrowser(H.M),b=H.nativeBrowser(H.U),state=structuredClone(a.SEED);
