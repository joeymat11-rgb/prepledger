'use strict';
/* =====================================================================
   M2-S10 SUPERSEDES THE NATIVE-CARRIERS CARRIER `writers-differential`
   ...
   WHY S10 CANNOT CARRY IT. The gate reaches writers.cjs through the same
   ...
   and carried unmoved by M2-S3-COMPANION and by the parent M2-S9-UI-PINS.
   SUP-11 measures both distances. M2-S10 moves writers.cjs by exactly one
   owner-worded clause at :227 (DECISIONS:631 O-1a), further from both, so it
   can neither reproduce the frozen side nor re-target the pin.

   WHAT S10 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME DIFFERENTIAL.
   ...
   nothing here has acquired a clock dependence. The things S10 moves outside this
   engine - the Today split's sealed siblings, the gym-settings writer, the
   capture clause and the two view files it RELEASES - are not in it; inside it
   S10 moves today.cjs and writers.cjs by one clause each, and completeSession
   is not among the readers compared here. SUP-13 asserts that no OTHER
   parent-pinned file this package edits lives under rebuild/engine/.
...
test('S10/SUP-11 - writers-differential: the gate\'s own subject is the parent post plus one declared clause, and RED-FIRST it is not the reconstruction', () => {
  /* The parent's post plus exactly the one owner-worded clause at :227, declared edited. */
  assertEppClauseOnly(WRITERS);
  ...
  assert.notEqual(diskSha(WRITERS), pin.after, 'RED: nor the post-image ' + pin.after.slice(0, 12) +
    '; the tree stands at ' + diskSha(WRITERS).slice(0, 12) + ', moved by an ancestor and by one declared clause here');
});

test('S10/SUP-12 - writers-differential: 3/3 Date/trap modes - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  const results = {};
...
test('S10/SUP-13 - writers-differential: S10 edits exactly the two EPP engine files, and RED CONTROL - one removed export is detected in all three modes', () => {
  ...
  assert.deepEqual(touched.filter(f => f.startsWith('rebuild/engine/') && SPEC.product[f].pre !== null).sort(), Object.keys(EPP_CLAUSES).sort(),
    'of the parent-pinned engine files exactly the two EPP files are edited; the rest of the engine is carried');
  for (const f of Object.keys(EPP_CLAUSES)) assertEppClauseOnly(f);
