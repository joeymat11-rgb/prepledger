'use strict';
/* =====================================================================
   M2-S11 SUPERSEDES THE NATIVE-CARRIERS CARRIER `second-gate`
   (gate second-gate)
   ...
   WHY S11 CANNOT CARRY IT. Same wall, same measurement: the reconstruction
   refuses on this tree - first at its closed inventory, which the native-load.cjs
   this package adds stands outside, and past that at files this package's
   ancestors moved, one of which (writers.cjs) S11 moves further by its FG01
   hunks, and at progression.cjs, which S11's FG02 takes off the construction
   (s11-supersede-source-carriers SUP-1, SUP-2). The parent
   M2-S10-TODAY-SPLIT stood in the same position and retired the gate at
   DECISIONS:829; S11 retires it again, under its own line and its own evidence,
   because a retirement is never inherited - only the gate list is.

   WHAT S11 PUTS IN ITS PLACE, HERE, EXECUTED. The claim the second gate makes,
   ...
   nothing may move; S11's FG01 moves completeSession's governor without
   changing a byte of it and adds one exported member, and FG02 changes a reader
   progression.cjs reaches only on a native row, so any difference here would
   have to be FG02's declared reading and would show red, never be absorbed.
   The census log itself is proved unmoved by this package's own `--ci` census
...
test('S11/SUP-14 - second-gate: the reader surface is real, and both compositions expose it', () => {
  ...
  assert.deepEqual(added, ['updateOpenerHold'], 'this package adds exactly one engine member, the FG01 governor updateOpenerHold');
  assert.deepEqual(removed, [], 'and removes none');
});

test('S11/SUP-15 - second-gate: over the FROZEN RECORD the accepted engine and this tree agree BYTE FOR BYTE', () => {
...
test('S11/SUP-16 - second-gate: RED-FIRST - the gate cannot be TAKEN over this tree, and the comparison has teeth', () => {
  ...
  assert.equal(NC.ENGINE.includes('rebuild/engine/native-load.cjs'), false, 'first at the closed inventory the added native-load.cjs stands outside');
  for (const f of ['rebuild/engine/progression.cjs', 'rebuild/engine/today.cjs', 'rebuild/engine/writers.cjs', 'rebuild/engine/merge.cjs', 'rebuild/engine/constants.cjs'])
  ...
  const mutant = privately(f, real.replace(marker, marker.replace('return null;', 'return null; /* S11 RED CONTROL */')));
