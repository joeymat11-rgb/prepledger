'use strict';
/* =====================================================================
   DECISIONS:153 (ii) — THE NAMED-FILES ENGINE DIFFERENTIAL, FOR M2-S3-COMPANION
   The fifth required evidence slot of the standing gate-supersession role
   (DECISIONS:153; this package's own token line is the PM's to append, and
   packages/S3.json cites it by sha256 once it stands on the chain branch).
   Templated on H3's h3-engine-files-differential.cjs, with the two named
   files being S3's: merge.cjs (the nativeDate seam) and today.cjs (the
   session pool and the sessionMembership reader).
   ...
   and otherwise to the blob at the parent's own `sourceBase`. S3's brief names
   eighteen files under that root — the sixteen engine modules it carries and
   the two it edits (`merge.cjs` and `today.cjs`) — so the
   ...
   byte-identity reconstructions of the engine. S3 cannot reproduce two of the
...
assert.deepEqual(moved.slice().sort(), ['rebuild/engine/merge.cjs', 'rebuild/engine/today.cjs'],
