'use strict';
/* =====================================================================
   M2-S4-REAL-DAY SUPERSEDES THE NATIVE-CARRIERS CARRIER `defect-witnesses`
   (gate witnesses-7)
   ...
   WHY S4 CANNOT CARRY IT. The FROZEN side is not a file: it is built, and it
   ...
   reconstruction rebuilds were moved by S4's ancestors under their own token
   lines; S4 moves none of them and cannot mend the frozen side by standing
   still.

   WHAT S4 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME SHAPE. S4
...
function project(E, state) {
  ...
  diverged.reads.push({ d: DAY, w: 186.4, sealed: false, note: 'SYNTHETIC S4 witness divergence' });
  out.mergeState = JSON.stringify(E.mergeState(JSON.parse(JSON.stringify(state)), diverged));
  return out;
}

test('S4/SUP-8 - defect-witnesses: RED-FIRST - the FROZEN side is built through the reconstruction, and it refuses here', () => {
  ...
  /* AND IT IS NOT S4. Every file the reconstruction names stands at the parent post. */
...
test('S4/SUP-9 - defect-witnesses: THE COMPLETE COMPARISON - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  ...
  assert.deepEqual(added, [], 'S4 adds no engine member');
...
test('S4/SUP-10 - defect-witnesses: RED CONTROL - a single removed export moves the projection, so SUP-9 is a measurement', () => {
