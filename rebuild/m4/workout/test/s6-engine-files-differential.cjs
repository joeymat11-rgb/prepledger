'use strict';
/* =====================================================================
   DECISIONS:153 (ii) - THE NAMED-FILES ENGINE DIFFERENTIAL, FOR M2-S6-TODAY-CHILD
   ...
   Templated on S5's s5-engine-files-differential.cjs, which was templated on
   S4's, and it keeps S5's shape because S6 is in S5's position exactly: it
   ...
   THE PARENT HERE IS M2-S5-TODAY-CHILD, WHICH ITSELF MOVED NO ENGINE BYTE, so the
   comparison chains: S5's own differential proved all 45 identical to S4's
   post, and this one proves all 45 identical to S5's. A reseal child that pins
...
assert.deepEqual(moved, [], 'M2-S6-TODAY-CHILD moves no engine byte, so no named file moves either');
