'use strict';
/* =====================================================================
   M2-S6-TODAY-CHILD SUPERSEDES THE NATIVE-CARRIERS CARRIER `inherited-carriers`
   ...
   WHY S6 CANNOT CARRY IT. The frozen prior bytes come from the same sha-pinned
   list as `source-carriers`, and today.cjs and writers.cjs on this tree are no
   longer the declared post-image of that list - not because S6 moved them
   (it moves nothing: s6-supersede-source-carriers SUP-1) but because
   M2-S3-COMPANION and M2-H3-CLEAN-INIT did, under their own token lines, and
   the parent M2-S5-TODAY-CHILD inherited them unmoved and retired these gates
   again at DECISIONS:462 for the same reason this package now does.
   SUP-5 executes priorModule's own rule and shows exactly where it refuses.

   WHAT S6 PUTS IN ITS PLACE, HERE, EXECUTED. Both halves of the same claim,
   pointed at what S6 actually is - a RESEAL child (DECISIONS:455) that pins
   page bytes and touches no engine module at all. (a) EVERY prior engine module on this tree
...
test('S6/SUP-5 - inherited-carriers: RED-FIRST - priorModule\'s own rule, re-executed over the real carried modules', () => {
  ...
  for (const f of refused) {
    assert.equal(diskSha(f), parentWant(f), 'and the refused file stands at the PARENT\'s post, unmoved by S6: ' + f);
    console.log('  PRIORMODULE REFUSES ' + f + ' declared post ' + NC.CARRIED[f].after.slice(0, 12) + ', tree ' + diskSha(f).slice(0, 12));
  }
});

test('S6/SUP-6 - inherited-carriers: every prior engine module is byte-identical to the PARENT\'S OWN POST, and the instrument has teeth', () => {
  const rows = [];
...
test('S6/SUP-7 - inherited-carriers: the inherited capabilities are still THERE and still reached, executed', () => {
