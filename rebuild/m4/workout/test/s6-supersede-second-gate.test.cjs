'use strict';
/* =====================================================================
   M2-S6-TODAY-CHILD SUPERSEDES THE NATIVE-CARRIERS CARRIER `second-gate`
   (gate second-gate)
   ...
   WHY S6 CANNOT CARRY IT. Same wall, same measurement: the reconstruction
   refuses on this tree at files this package's ancestors moved, and S6 moves
   none of them (s6-supersede-source-carriers SUP-1, SUP-2). The parent
   M2-S5-TODAY-CHILD stood in exactly this position and retired the gate at
   DECISIONS:462; S6 retires it again, under its own line and its own evidence,
   because a retirement is never inherited - only the gate list is.

   WHAT S6 PUTS IN ITS PLACE, HERE, EXECUTED. The claim the second gate makes,
...
test('S6/SUP-14 - second-gate: the reader surface is real, and both compositions expose it', () => {
...
test('S6/SUP-15 - second-gate: over the FROZEN RECORD the accepted engine and this tree agree BYTE FOR BYTE', () => {
...
test('S6/SUP-16 - second-gate: RED-FIRST - the gate cannot be TAKEN over this tree, and the comparison has teeth', () => {
  ...
  const mutant = privately(f, real.replace(marker, marker.replace('return null;', 'return null; /* S6 RED CONTROL */')));
