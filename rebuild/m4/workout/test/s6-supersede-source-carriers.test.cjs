'use strict';
/* =====================================================================
   M2-S6-TODAY-CHILD SUPERSEDES THE NATIVE-CARRIERS CARRIER `source-carriers`
   ...
   WHY S6 CANNOT CARRY IT, AND WHY THAT IS NOT S6'S DOING. M2-S6-TODAY-CHILD
   ...
   reconstruction is nevertheless already false on this tree, because S6's
   ...
   long before B-NTC. The PARENT, M2-S5-TODAY-CHILD, moved none of them either -
   it is where this package inherits them from, unmoved - and it retired these
   same gates under its own token line DECISIONS:462 for exactly this reason.
   A gate that reconstructs bytes cannot be mended by a package that moves no
   bytes: S6 can neither reproduce the construction nor re-target the sha-pinned
   list without editing engine files its accepted brief does not name.

   THIS IS THE FOURTH GENERATION, AND THE POINT IS THAT NOTHING DECAYS. S3 was
   the child, S4 the grandchild, S5 the great-grandchild and S6 the fourth in the
   line; each retires the nine
   ...
   first of the nineteen gates refusing; :462 is the PARENT's token line and
...
const SPEC = JSON.parse(fs.readFileSync(path.join(REPO, 'rebuild/lanes/b/tooling/packages/S6.json'), 'utf8'));
...
test('S6/SUP-1 - source-carriers: M2-S6-TODAY-CHILD moves NO rebuild/engine byte, over the whole tracked inventory', () => {
...
test('S6/SUP-2 - source-carriers: RED-FIRST - the carrier\'s own verifier, EXECUTED here, refuses on this tree', () => {
  /* The construction itself is intact: the frozen BASE plus the 48 pinned
     ...
     it, and SUP-1 has just shown S6 is not what moved. */
...
test('S6/SUP-3 - source-carriers: RED-FIRST - the ORIGINAL gate migrate-source, EXECUTED, refuses at its first declaration', () => {
...
test('S6/SUP-4 - source-carriers: the pinned list cannot be extended, and the parent\'s own retirement is on the record', () => {
  ...
  /* THE PARENT IS S5, NOT S4, AND THAT IS THE WHOLE SHAPE OF THIS PACKAGE. S5
     ...
     (ii)-(iii) is asked of this package exactly as it was asked of S5. */
  assert.equal(PARENT.packageId, 'M2-S5-TODAY-CHILD', 'the bound parent is S5');
