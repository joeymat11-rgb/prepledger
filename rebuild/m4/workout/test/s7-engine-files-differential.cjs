'use strict';
/* =====================================================================
   DECISIONS:153 (ii) - THE NAMED-FILES ENGINE DIFFERENTIAL, FOR M2-S7-PORT-ADMISSION
   ...
   Templated on S6's s6-engine-files-differential.cjs, which was templated on
   S5's, and it keeps S6's shape because S7 is in S6's position exactly: it
   ...
   THE PARENT HERE IS M2-S6-TODAY-CHILD, WHICH ITSELF MOVED NO ENGINE BYTE, so the
   comparison chains: S6's own differential proved all 45 identical to S5's
   post, and this one proves all 45 identical to S6's. A reseal child that pins
   the import path's bytes has no engine claim of its own to make, and this file
   is how that is said as a number instead of as a sentence.
...
assert.deepEqual(moved, [], 'M2-S7-PORT-ADMISSION moves no engine byte, so no named file moves either');
