'use strict';
/* =====================================================================
   M2-S7-PORT-ADMISSION SUPERSEDES THE NATIVE-CARRIERS CARRIER `source-carriers`
   ...
   WHY S7 CANNOT CARRY IT, AND WHY THAT IS NOT S7'S DOING. M2-S7-PORT-ADMISSION
   ...
   reconstruction is nevertheless already false on this tree, because S7's
   ...
   long before B-NTC. The PARENT, M2-S6-TODAY-CHILD, moved none of them either -
   it is where this package inherits them from, unmoved - and it retired these
   same gates under its own token line DECISIONS:490 for exactly this reason.
   A gate that reconstructs bytes cannot be mended by a package that moves no
   bytes: S7 can neither reproduce the construction nor re-target the sha-pinned
   list without editing engine files its accepted brief does not name.

   THIS IS THE FIFTH GENERATION, AND THE POINT IS THAT NOTHING DECAYS. S3 was
   the child, S4 the grandchild, S5 the great-grandchild, S6 the fourth and S7
   the fifth in the line; each retires the nine
   ...
   first of the nineteen gates refusing; :490 is the PARENT's token line and
...
test('S7/SUP-1 - source-carriers: M2-S7-PORT-ADMISSION moves NO rebuild/engine byte, over the whole tracked inventory', () => {
...
test('S7/SUP-2 - source-carriers: RED-FIRST - the carrier\'s own verifier, EXECUTED here, refuses on this tree', () => {
  /* The construction itself is intact: the frozen BASE plus the 48 pinned
     ...
     it, and SUP-1 has just shown S7 is not what moved. */
...
test('S7/SUP-3 - source-carriers: RED-FIRST - the ORIGINAL gate migrate-source, EXECUTED, refuses at its first declaration', () => {
...
test('S7/SUP-4 - source-carriers: the pinned list cannot be extended, and the parent\'s own retirement is on the record', () => {
  ...
  /* THE PARENT IS S6, NOT S5, AND THAT IS THE WHOLE SHAPE OF THIS PACKAGE. S6
     ...
     (ii)-(iii) is asked of this package exactly as it was asked of S6. */
  assert.equal(PARENT.packageId, 'M2-S6-TODAY-CHILD', 'the bound parent is S6');
