'use strict';
/* =====================================================================
   M2-S7-PORT-ADMISSION SUPERSEDES THE NATIVE-CARRIERS CARRIER `writers-differential`
   ...
   WHY S7 CANNOT CARRY IT. The gate reaches writers.cjs through the same
   ...
   and carried unmoved by M2-S3-COMPANION and by the parent M2-S6-TODAY-CHILD.
   SUP-11 measures both distances. M2-S7-PORT-ADMISSION moves no engine byte at all,
   so it can neither reproduce the frozen side nor re-target the pin.

   WHAT S7 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME DIFFERENTIAL.
   ...
   nothing here has acquired a clock dependence. The things S7 does move - the
   local-source admission rule, the Import screen's copy and the Edit My Week
   companion beside them - are not in this engine at all: SUP-13 asserts that
   no file this package edits or adds lives under rebuild/engine/, over the
   package's own whole declared inventory.
...
test('S7/SUP-11 - writers-differential: the gate\'s own subject is carried at the parent post, and RED-FIRST it is not the reconstruction', () => {
...
test('S7/SUP-12 - writers-differential: 3/3 Date/trap modes - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  const results = {};
...
test('S7/SUP-13 - writers-differential: S7 edits no engine file at all, and RED CONTROL - one removed export is detected in all three modes', () => {
