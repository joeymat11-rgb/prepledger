'use strict';
/* =====================================================================
   DECISIONS:153 (ii) - THE NAMED-FILES ENGINE DIFFERENTIAL, FOR M2-S8-REAL-SHAPE
   ...
   Templated on S7's s7-engine-files-differential.cjs, which was templated on
   S6's, and it keeps S7's shape because S8 is in S7's position exactly: it
   ...
   THE PARENT HERE IS M2-S7-PORT-ADMISSION, WHICH ITSELF MOVED NO ENGINE BYTE, so the
   comparison chains: S7's own differential proved all 45 identical to S6's
   post, and this one proves all 45 identical to S7's. A reseal child that pins
...
assert.deepEqual(moved, [], 'M2-S8-REAL-SHAPE moves no engine byte, so no named file moves either');
