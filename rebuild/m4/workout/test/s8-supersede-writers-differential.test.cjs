'use strict';
/* =====================================================================
   M2-S8-REAL-SHAPE SUPERSEDES THE NATIVE-CARRIERS CARRIER `writers-differential`
   ...
   WHY S8 CANNOT CARRY IT. The gate reaches writers.cjs through the same
   ...
   and carried unmoved by M2-S3-COMPANION and by the parent M2-S7-PORT-ADMISSION.
   SUP-11 measures both distances. M2-S8-REAL-SHAPE moves no engine byte at all,
   so it can neither reproduce the frozen side nor re-target the pin.

   WHAT S8 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME DIFFERENTIAL.
   ...
   nothing here has acquired a clock dependence. The things S8 does move - the
   local-source admission rule, the lift correspondence helper, the capture
   layout law's readers and the Import screen's copy beside them - are not in
...
test('S8/SUP-11 - writers-differential: the gate\'s own subject is carried at the parent post, and RED-FIRST it is not the reconstruction', () => {
...
test('S8/SUP-12 - writers-differential: 3/3 Date/trap modes - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  const results = {};
...
test('S8/SUP-13 - writers-differential: S8 edits no engine file at all, and RED CONTROL - one removed export is detected in all three modes', () => {
