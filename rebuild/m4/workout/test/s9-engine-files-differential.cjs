'use strict';
/* =====================================================================
   DECISIONS:153 (ii) - THE NAMED-FILES ENGINE DIFFERENTIAL, FOR M2-S9-UI-PINS
   ...
   Templated on S8's s8-engine-files-differential.cjs, which was templated on
   S7's, and it keeps S8's shape because S9 is in S8's position exactly: it
   ...
   THE PARENT HERE IS M2-S8-REAL-SHAPE, WHICH ITSELF MOVED NO ENGINE BYTE, so the
   comparison chains: S8's own differential proved all 45 identical to S7's
   post, and this one proves all 45 identical to S8's. A reseal child that pins
...
assert.deepEqual(moved, [], 'M2-S9-UI-PINS moves no engine byte, so no named file moves either');
