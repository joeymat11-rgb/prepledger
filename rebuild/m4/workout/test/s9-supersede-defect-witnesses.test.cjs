'use strict';
/* =====================================================================
   M2-S9-UI-PINS SUPERSEDES THE NATIVE-CARRIERS CARRIER `defect-witnesses`
   (gate witnesses-7)
   ...
   WHY S9 CANNOT CARRY IT. The FROZEN side is not a file: it is built, and it
   ...
   reconstruction rebuilds were moved by S9's ancestors under their own token
   lines; S9 moves none of them - nor did its parent M2-S8-REAL-SHAPE, which
   retired this same carrier at DECISIONS:527 - and a package that stands still
   cannot mend a frozen side that four earlier packages left behind.

   WHAT S9 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME SHAPE. S9
...
function project(E, state) {
  ...
  diverged.reads.push({ d: DAY, w: 186.4, sealed: false, note: 'SYNTHETIC S9 witness divergence' });
  out.mergeState = JSON.stringify(E.mergeState(JSON.parse(JSON.stringify(state)), diverged));
  return out;
}

test('S9/SUP-8 - defect-witnesses: RED-FIRST - the FROZEN side is built through the reconstruction, and it refuses here', () => {
  ...
  /* AND IT IS NOT S9. Every file the reconstruction names stands at the parent post. */
...
test('S9/SUP-9 - defect-witnesses: THE COMPLETE COMPARISON - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  ...
  assert.deepEqual(added, [], 'S9 adds no engine member');
...
test('S9/SUP-10 - defect-witnesses: RED CONTROL - a single removed export moves the projection, so SUP-9 is a measurement', () => {
