'use strict';
/* =====================================================================
   M2-S9-UI-PINS SUPERSEDES THE NATIVE-CARRIERS CARRIER `second-gate`
   (gate second-gate)
   ...
   WHY S9 CANNOT CARRY IT. Same wall, same measurement: the reconstruction
   refuses on this tree at files this package's ancestors moved, and S9 moves
   none of them (s9-supersede-source-carriers SUP-1, SUP-2). The parent
   M2-S8-REAL-SHAPE stood in exactly this position and retired the gate at
   DECISIONS:527; S9 retires it again, under its own line and its own evidence,
   because a retirement is never inherited - only the gate list is.

   WHAT S9 PUTS IN ITS PLACE, HERE, EXECUTED. The claim the second gate makes,
...
test('S9/SUP-14 - second-gate: the reader surface is real, and both compositions expose it', () => {
...
test('S9/SUP-15 - second-gate: over the FROZEN RECORD the accepted engine and this tree agree BYTE FOR BYTE', () => {
...
test('S9/SUP-16 - second-gate: RED-FIRST - the gate cannot be TAKEN over this tree, and the comparison has teeth', () => {
  ...
  const mutant = privately(f, real.replace(marker, marker.replace('return null;', 'return null; /* S9 RED CONTROL */')));
