'use strict';
/* =====================================================================
   M2-S9-UI-PINS SUPERSEDES THE NATIVE-CARRIERS CARRIER `source-carriers`
   ...
   WHY S9 CANNOT CARRY IT, AND WHY THAT IS NOT S9'S DOING. M2-S9-UI-PINS
   ...
   reconstruction is nevertheless already false on this tree, because S9's
   ...
   long before B-NTC. The PARENT, M2-S8-REAL-SHAPE, moved none of them either -
   it is where this package inherits them from, unmoved - and it retired these
   same gates under its own token line DECISIONS:527 for exactly this reason.
   A gate that reconstructs bytes cannot be mended by a package that moves no
   bytes: S9 can neither reproduce the construction nor re-target the sha-pinned
   list without editing engine files its accepted brief does not name.

   THIS IS THE SEVENTH GENERATION, AND THE POINT IS THAT NOTHING DECAYS. S3
   was the child, S4 the grandchild, S5 the great-grandchild, S6 the fourth,
   S7 the fifth, S8 the sixth and S9 the seventh in the line; each retires the nine
   ...
   first of the nineteen gates refusing; :527 is the PARENT's token line and
...
test('S9/SUP-1 - source-carriers: M2-S9-UI-PINS moves NO rebuild/engine byte, over the whole tracked inventory', () => {
...
test('S9/SUP-2 - source-carriers: RED-FIRST - the carrier\'s own verifier, EXECUTED here, refuses on this tree', () => {
  /* The construction itself is intact: the frozen BASE plus the 48 pinned
     ...
     it, and SUP-1 has just shown S9 is not what moved. */
...
test('S9/SUP-3 - source-carriers: RED-FIRST - the ORIGINAL gate migrate-source, EXECUTED, refuses at its first declaration', () => {
...
test('S9/SUP-4 - source-carriers: the pinned list cannot be extended, and the parent\'s own retirement is on the record', () => {
  ...
  /* THE PARENT IS S8, NOT S7, AND THAT IS THE WHOLE SHAPE OF THIS PACKAGE. S8
     ...
     (ii)-(iii) is asked of this package exactly as it was asked of S8. */
  assert.equal(PARENT.packageId, 'M2-S8-REAL-SHAPE', 'the bound parent is S8');
