'use strict';
/* =====================================================================
   M2-S9-UI-PINS SUPERSEDES THE NATIVE-CARRIERS CARRIER `writers-differential`
   ...
   WHY S9 CANNOT CARRY IT. The gate reaches writers.cjs through the same
   ...
   and carried unmoved by M2-S3-COMPANION and by the parent M2-S8-REAL-SHAPE.
   SUP-11 measures both distances. M2-S9-UI-PINS moves no engine byte at all,
   so it can neither reproduce the frozen side nor re-target the pin.

   WHAT S9 PUTS IN ITS PLACE, HERE, EXECUTED, AND IT IS THE SAME DIFFERENTIAL.
   ...
   nothing here has acquired a clock dependence. The things S9 does move - the
   Today carry lane's four declarations, the passphrase normalisation lane's
   import path, the look tickets' screen files, and the two presentation files
   DECISIONS:536 RELEASES out of the sealed inventory altogether - are not in
   this engine at all: SUP-13 asserts that no file this package edits or adds
   lives under rebuild/engine/, over the package's own whole declared
   inventory. A released file is role "released", which is not "carried", so it
   is one of the files SUP-13 counts and then measures.
...
test('S9/SUP-11 - writers-differential: the gate\'s own subject is carried at the parent post, and RED-FIRST it is not the reconstruction', () => {
...
test('S9/SUP-12 - writers-differential: 3/3 Date/trap modes - the accepted engine (from Git) and this tree agree BYTE FOR BYTE', () => {
  const results = {};
...
test('S9/SUP-13 - writers-differential: S9 edits no engine file at all, and RED CONTROL - one removed export is detected in all three modes', () => {
