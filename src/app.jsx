...
/* ---------- tokens ---------- */
/* Two greys, two jobs — the distinction the old palette collapsed.
   `steel` is the ONLY secondary text colour (5.6:1 on ink, 5.2:1 on plate — both
   pass AA body). `dim` is decoration ONLY: hairlines, gridlines, disabled tracks,
   recessive chart marks. It computes to 3.0:1 on ink, which fails body text, so
...
/* ---------- type scale — six roles, one source ----------
   ...
   BELOW 11px. Two rules now hold. (1) 11px is a hard floor — nothing a user must
   read goes under it, because below that trips iOS legibility and, on this
   athlete's ADHD, glance-ability. (2) Fractional sizes are gone: 12.5 and 9.5
   land on half-pixels, which is what made the old text look soft rather than cut.
   Every step here is an integer, and both mono steps clear the floor.
   To make something bigger, move it to the right role — do not invent a number. */
const TS = {
  metric: 34,   /* Barlow Condensed 600 · lh 36 · hero number, ONE per screen */
  display: 26,  /* Barlow Condensed 600 · lh 28 · screen title               */
  title: 17,    /* Barlow 600          · lh 24 · card / section title        */
  body: 13,     /* Barlow 400          · lh 20 · prose, the coach's read     */
  label: 12,    /* IBM Plex Mono 500   · lh 16 · labels, units, axes (UPPER) */
  micro: 11,    /* IBM Plex Mono 500   · lh 16 · dense tags (measured, n=12) */
};
/* Line-heights on the 4px grid, paired to the roles above so callers stop
   guessing. Used as a multiplier-free px value: lineHeight: `${LH.body}px`. */
const LH = { metric: 36, display: 28, title: 24, body: 20, label: 16, micro: 16 };
...
/* ---------- motion — "measured / deliberate" ----------
   Decelerate-dominant, no bounce, no overshoot, nothing over 320ms. Motion
   confirms that state changed; it never rushes and never entertains. There are
   no countdowns in this app — a timer that races you is a pressure device, and
   pressure is not a measurement. Numerals count up on FIRST appearance only; on
   re-render they cut, so a number never animates while you are reading it. */
const MOT = {
  state: 80,        /* press / hover / select            */
  micro: 120,       /* toggles, tab underline            */
  standard: 200,    /* card expand, inline reveal        */
  deliberate: 300,  /* screen transitions, earned states */
  count: 320,       /* numeric count-up, first load only */
};
const EASE = {
  settle: "cubic-bezier(0.05,0.7,0.1,1)",   /* default — lands softly into place */
  standard: "cubic-bezier(0.2,0,0,1)",      /* entrances that also move          */
  exit: "cubic-bezier(0.3,0,0.8,0.15)",     /* dismissals                        */
  linear: "linear",                         /* counters, progress fills          */
};
/* The one transition string 95% of callers want. */
const TR = (prop = "all", d = MOT.standard, e = EASE.settle) => `${prop} ${d}ms ${e}`;
...
if (typeof document !== "undefined" && !document.getElementById("pl-gx")) {
  ...
  st0.textContent = "*{box-sizing:border-box;-webkit-tap-highlight-color:transparent} html,body,#root{max-width:100%;overflow-x:hidden} body{-webkit-text-size-adjust:100%} input,select,textarea{font-size:16px !important;max-width:100%} button{max-width:100%}"
    ...
    /* The in-app toggle mirrors it, for the athlete who wants it off regardless
       of what the phone says. Set data-reduce-motion on the root. */
...
/* The ± pair used to be 40px with a 6px gap. Both numbers were under spec: Apple
   HIG wants a 44pt hit area and WCAG 2.2 (2.5.8) wants ≥8px between adjacent
   targets, or a fat thumb on a moving train hits the wrong one. Now 44/8, with
   the value in tabular mono so it does not jitter as digits change. */
const stepBtn = {
  width: 44, height: 44, borderRadius: 8, fontSize: 18,
  border: `1px solid ${T.line}`, background: T.plate2, color: T.chalk,
...
const Num = ({ children, size = 30, c = T.chalk }) => (
  <span data-num style={{ fontFamily: mono, fontSize: size, fontWeight: 600, color: c, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums" }}>{children}</span>
);
        ...
        <Card key={ex.id} style={{ padding: 16, opacity: skipped[ex.id] ? 0.45 : 1 }} accent={ex.isDebutNow && !skipped[ex.id] ? T.orange : undefined}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 10, flexWrap: "wrap" }}>
            {(() => { const rp2 = rirPlan(s, ex, slp); return getReps(ex).map((r, i) => (
              <div key={i}>
                <div style={{ fontFamily: mono, fontSize: TS.label, color: T.steel, marginBottom: 3 }}>SET {i + 1} · <span style={{ color: rp2.plan[i] === 0 ? T.jade : T.steel, fontWeight: 700 }}>RIR {rp2.plan[i] ?? "—"}</span>{rp2.plan[i] === 0 ? <span style={{ color: T.jade }}> · TO FAILURE</span> : null}</div>
