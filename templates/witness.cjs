...
const {createResourceMeter}=require("C:\\Users\\joeym\\Documents\\Codex\\2026-09-06\\earned-process-proportionality-audit\\acceleration-trial\\MEMORY-PROFILE-02\\retaining-capture/meter.cjs");
...
async function run({onProgress=m=>console.log(m),runtimeFactory=createR1Runtime,populateFixture=populate,
  ...
  const CAP=global.__retainingCapture.controller;
  ...
  const save=()=>{evidence.resourceAcceptance=false;fs.mkdirSync(path.dirname(output),{recursive:true});fs.writeFileSync(output,JSON.stringify(evidence,null,2)+'\n');};

  async function request(route,body,subject='subject-first',label=route){return CAP.request(label,async()=>{
    const controller=new AbortController(),unregister=CAP.registerAborter(controller),timer=setTimeout(()=>controller.abort(),30000);
    try{
  ...
  async function measuredRequest(label,route,body,subject){
    ...
    const measured=await meter.measure(async()=>{try{return await request(route,body,subject,label);}catch(error){return {failure:errorText(error)};}}).catch(error=>{const stopped=CAP.stopped===true; if(!stopped)CAP.fail("CPU_MEASUREMENT_UNAVAILABLE");return {cpuMs:null,value:{failure:stopped?"DIAGNOSTIC_REQUEST_STOPPED":"CPU_MEASUREMENT_UNAVAILABLE"}};});
    const result=measured.value,entry={label,cpuMs:measured.cpuMs,cpuGuardMs:evidence.cpuGuardMs,
      guardedCpuMs:Number.isFinite(measured.cpuMs)?measured.cpuMs+evidence.cpuGuardMs:null,status:result.status??null,responseBytes:result.responseBytes??0,
  ...
  async function beginPhase(name){CAP.assertIssuance();CAP.setPhase(name);evidence.phase=name;const before=await meter.sampleProcess();await meter.begin();observing=true;
  ...
  try{
    ...
    for(let i=0;i<3;i++){
      const result=await meter.measure(()=>CAP.request('calibration-'+i,async()=>{const a=new AbortController(),unregister=CAP.registerAborter(a),timer=setTimeout(()=>a.abort(),30000);try{const r=await fetch(new URL('/__r1-meter-calibration',runtime.url),{signal:a.signal});return {status:r.status,value:await r.json()};}finally{clearTimeout(timer);unregister();}}));
    ...
    evidence.resourceAcceptance=false; evidence.diagnosticOnly=true;
    ...
    // Controller owns bounded resume/disable/session/runtime cleanup.
  }
}
