...
const MUTATIONS = [
  ...
  ["adj-id-order", "pin", "the undo door throws away the legacy writer's own embedded instant — same-day unstamped taps fall back to rid spelling and Undo reverses the wrong move (the rig123 wound, executed with the real f72dbf7 writer)", `    const c7 = (x) => (x && x.at) ? "2" + String(x.at) : (x && x.id) ? "1" + String(x.id) : "0" + String((x && x.ord) || "");`, `    const c7 = (x) => String((x && x.at) || "");   /* mutant: rid decides the day */`],
  ...
  ["adj-ord-not-minted", "pin", "the boot stops recovering array order for the id-less suggestion-era adjustments — their day falls back to rid spelling", `  if (Array.isArray(st.adjustments)) st.adjustments.forEach((a, i) => { if (a && typeof a === "object" && !Array.isArray(a) && !a.at && !a.id && a.ord == null) a.ord = String(i).padStart(4, "0"); });`, `  /* mutant: position is noise here too */`],
  ["ordersure-gone", "pin", "the undo door claims 'last' over an unproven same-day legacy order again", `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: !!a.at || day7.length <= 1 };`, `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: true };   /* mutant: every order is the athlete's */`],
