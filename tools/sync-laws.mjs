...
const MUTATIONS = [
  ...
  ["at-order-sid", "pin", "same-day decisions fall back to alphabetical sid order — the athlete's later tap silently loses at boot", `  const k9 = (x) => String((x && x.d) || "") + "|" + String((x && x.at) || "") + "|" + String((x && x.sid) || "") + "|" + _canonJ(x);`, `  const k9 = (x) => String((x && x.d) || "") + "|" + String((x && x.sid) || "") + "|" + _canonJ(x);   /* mutant: spelling order */`],
  ["adj-at-order", "pin", "the undo door falls back to rid order within a day — 'Last move applied' offers the older move", `    const ka = String((a && a.d) || "") + "|" + String((a && a.at) || "") + "|" + String((a && a.rid) || "") + "|" + String((a && a.id) || "");`, `    const ka = String((a && a.d) || "") + "|" + String((a && a.rid) || "") + "|" + String((a && a.id) || "");   /* mutant: rid order */`],
  ...
  ["undo-not-durable", "pin", "the one-tap undo goes back to hiding the control without reversing the effect", `  if (sugRow9 && sugRow9.decided === "approved") { sugRow9.undone = true; reconcileSuggestionEffects(s); }`, `  /* mutant: undone is a display state */`],
