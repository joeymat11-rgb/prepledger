...
const MUTATIONS = [
  ...
  ["adjustments-arrival", "pin", "the adjustment ledger keeps arrival order — lastUndoable offers a different move by merge direction", `  if (Array.isArray(s.adjustments)) s.adjustments = s.adjustments.slice().sort((a, b) => {`, `  if (false) s.adjustments = s.adjustments.slice().sort((a, b) => {   /* mutant: arrival order */`],
  ...
  ["sug-receipts-history", "pin", "the analyst-suggestion receipts stop re-deriving — an APPLIED line outlives the dismissal that reversed its effect", `    s.feed = s.feed.filter((f) => !(f && ((typeof f.op === "string" && f.op.indexOf("sug:") === 0) || f.t === SUGT9.approved || f.t === SUGT9.dismissed || f.t === SUGT9.noted)));`, `    /* mutant: the receipts are history */`],
  ...
  ["fork-ops-not-canonical", "pin", "the boot stops restating fork ops — a boot differs from its own self-merge in bytes no reconciler owns", `      const ops9 = Array.isArray(f9.ops) && f9.ops.length ? f9.ops : (f9.why ? [String(f9.why)] : []);`, `      const ops9 = Array.isArray(f9.ops) && f9.ops.length ? f9.ops : [];   /* mutant: no restatement */`],
