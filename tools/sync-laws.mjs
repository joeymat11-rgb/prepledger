...
const MUTATIONS = [
  ...
  ["ordersure-selected-row", "pin", "orderSure becomes a property of the selected row again — a stamped winner claims 'Last move applied' over a skew-dependent or mixed-writer order (the executed A6 contradiction)", `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: day7.length <= 1 };`, `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: !!a.at || day7.length <= 1 };   /* mutant: the winner's own clock is proof */`],
  ...
  ["adj-ord-not-minted", "pin", "the boot stops recovering storage order for adjustments without a derivable instant — their day falls back to rid spelling (re-anchored at SCALE-8: the mint mirrors _adjInstant)", `  if (Array.isArray(st.adjustments)) st.adjustments.forEach((a, i) => { if (a && typeof a === "object" && !Array.isArray(a) && _adjInstant(a) == null && a.ord == null) a.ord = String(i).padStart(4, "0"); });   /* SCALE-8 — the mint mirrors _adjInstant exactly: a row with an unparseable id needs its position as much as an id-less one */`, `  /* mutant: position is noise here too */`],
  ["ordersure-gone", "pin", "the undo door claims 'last' over an unproven same-day order again (re-anchored at SCALE-8: sureness is now set-derived)", `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: day7.length <= 1 };`, `    return { rid: a.rid, d: a.d, title: a.title, auto: !!a.auto, orderSure: true };   /* mutant: every order is the athlete's */`],
